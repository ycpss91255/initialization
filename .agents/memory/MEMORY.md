...
- [Coverage shards are unit-only](project-coverage-shards-unit-only.md) — AC-17 merged gate traces UNIT bats only; expect/integration smoke excluded, so cover new TUI screens with unit e2e-harness specs, not just .exp smoke
- [sh completeness program](project-sh-completeness-program.md) — post-rc3: finish the shell layer (bugs + security + hook specs + arch deepenings + full pyramid; legacy tool/small-tools = per-script triage) BEFORE returning to TUI (parked in tui-feedback-traceability.md)
