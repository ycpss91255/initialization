= README
Personal modular Ubuntu environment initialization tool (`init_ubuntu`): a bash engine + module catalog that installs, upgrades and removes my development environment, apt-style.
...
On first run the entrypoint checks the tool's own dependencies (`jq` / `curl` / `git`): if something is missing and sudo is available it asks once whether to `apt install` it (automatic with `-y`); without sudo it fails fast with explicit install instructions.

== Documentation

* link:doc/guide/cli-usage.md[`doc/guide/cli-usage.md`] — apt-style daily flows, exit codes, examples
* link:doc/guide/troubleshooting.md[`doc/guide/troubleshooting.md`] — doctor, JSONL logs, trace_id, common errors
* link:doc/guide/module-authoring.md[`doc/guide/module-authoring.md`] — write your own module
* link:doc/guide/archetype-cookbook.md[`doc/guide/archetype-cookbook.md`] — per-archetype recipes
* link:doc/module/INDEX.md[`doc/module/INDEX.md`] — auto-generated module catalog
* link:doc/prd/init-ubuntu.prd.md[`doc/prd/init-ubuntu.prd.md`] — product spec
