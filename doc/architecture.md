...
graph TB
    ...
    subgraph frontend["Front-end (共用同一個 Engine)"]
        ...
        tui["setup_ubuntu_tui.sh<br/>(TUI - fzf/whiptail)"]
...
│   ├── tui_backend.sh                  # shared data layer + whiptail Fallback tier(ADR-0024)
...
分層為 **fzf(Rich tier,雙欄導覽,優先)+ whiptail(Fallback tier,保證存在)**
(ADR-0024,取代 ADR-0023 的 gum;gum 不再是後端,僅保留為可安裝的工具
`setup_ubuntu install gum`)。`lib/tui_backend.sh` 是共用資料層 + whiptail
Fallback tier;`lib/tui_render_fzf.sh` 是 fzf Rich tier。契約對齊的是**行為/結果
(讀回 tag、exit code 語意),不是像素**(HTML+JS 類比,同 G4)。
`--backend fzf|whiptail` 可強制分層並**跳過偵測與安裝詢問**(invalid →
exit 2;`--backend gum` 會以 exit 2 提示改用 `setup_ubuntu install gum`),是
CI/QA 強測單一分層的槓桿(`just tui --backend whiptail`)。
