...
當 module 適用 `lib/module_helper.sh` 提供的標準 lifecycle 樣板時,**只需宣告資料欄位 + 呼叫一行 archetype macro**,macro 一次定義 8 個可預設的
lifecycle:`is_installed / install / upgrade / remove / purge / verify /
doctor / is_outdated`(ADR-0027;`detect` 與 `is_recommended` 仍由 module
自訂)。Macro 之後可重新宣告任一函式以覆寫(例:apt module 自行覆寫
`verify()` 跑 smoke test,或 docker 覆寫 `doctor()` 檢查 daemon)。
...
- `verify()` 預設(`module_default_verify`)**與** `doctor()` 預設
  (`module_default_doctor`)都會在 `is_installed` 成功後額外執行此命令當作
  runtime probe;任一失敗即回非 0(§4.1.1)
- 例:`docker run --rm hello-world` / `command -v nvim && nvim --version`
- 預設:空(此時 verify / doctor 只跑 `is_installed`)

---

## 4. Lifecycle 函式規範

### 4.1 Lifecycle 函式(10 個,全部 mandatory)

Lifecycle 契約是 **10 個函式,全部 mandatory**(ADR-0002):`install` /
`upgrade` / `remove` / `purge` / `verify` / `doctor` / `detect` /
`is_recommended` / `is_installed` / `is_outdated`。早期草案曾分「5 必要 + 5
選填」,但每一個「選填」欄位都會讓 CLI 行為隨 module 而異(「`setup_ubuntu
doctor neovim` 回 exit 2,只因為那個 module 沒實作」),對使用者是敵意的
UX。因此全部 mandatory。

**成本為零的原因**:archetype macro(`module_use_apt_archetype` 等,§3.2.1)
為它所發出的 **8 個** 函式提供 **可運作的預設值**
(`install` / `upgrade` / `remove` / `purge` / `verify` / `doctor` /
`is_outdated` + `is_installed`),module **可**覆寫其中任一個(bash 後宣告者
勝)。只有 `detect()` 與 `is_recommended()` 一律由 module 自行定義(判斷真的
因 module 而異)。手寫的 archetype D(custom)作者則需自行實作全部 10 個。

#### 10 個 mandatory 函式

| 函式 | 簽名 | 回傳 | 副作用 | 預設來源 | Idempotent |
|---|---|---|---|---|---|
| `detect()` | `() -> int` | 0=支援當前環境,非 0=不支援 | 無(只讀環境) | **module 自訂**(非 macro 發出) | 是 |
| `is_recommended()` | `() -> int` | 0=建議勾選,非 0=預設不勾 | 無 | **module 自訂**(非 macro 發出) | 是 |
| `is_installed()` | `() -> int` | 0=已裝,非 0=未裝 | 無 | archetype 預設 | 是 |
| `install()` | `() -> int` | 0=成功,非 0=失敗 | **裝套件、修改系統** | archetype 預設 | **必須** |
| `upgrade()` | `() -> int` | 0=成功,非 0=失敗 | **升級套件/重抓 latest** | archetype 預設 | **必須** |
| `remove()` | `() -> int` | 0=成功,非 0=失敗 | **移除套件,保留 config** | archetype 預設 | **必須** |
| `purge()` | `() -> int` | 0=成功,非 0=失敗 | **移除套件 + 清 config/PPA** | archetype 預設 | **必須** |
| `verify()` | `() -> int` | 0=驗證通過 | 無(只讀) | `module_default_verify` | 是 |
| `doctor()` | `() -> int` | 0=健檢通過 | 無(唯讀/離線) | `module_default_doctor` | 是 |
| `is_outdated()` | `() -> int` | 0=有新版可裝 | 無(只讀) | `module_default_*_is_outdated` | 是 |

archetype 預設實作對照見 §4.10(`module_default_*`)。`verify` 與 `doctor`
的契約與邊界見下方 §4.1.1。
...
### 4.1.1 verify 與 doctor 契約(ADR-0009)

`verify` 與 `doctor` 都回答「這個工具好不好」,但問的時機與問題不同。兩者
**都是唯讀、預設離線**,不改系統(不 heal Sidecar、不 apt update、不打
網路)。

#### `verify()` — 安裝後驗收(post-install acceptance)

- **回答的問題**:「`install()` / `upgrade()` 有真的裝完嗎?」
- **呼叫時機**:install / upgrade 鏈的**結尾自動**跑一次(最乾淨的時刻,
  什麼都還沒 drift),以及使用者手動 `setup_ubuntu verify <module>`。
- **預設實作**(`module_default_verify`):`is_installed` 成功後,若
  `TEST_VERIFY_CMD` 非空就跑它當 smoke test。
- **不可假設**:daemon 在跑、使用者在正確 group、config 仍完整、外網可達。

#### `doctor()` — 隨選健檢(on-demand runtime health)

- **回答的問題**:「我現在能不能真的用這個工具?」
- **呼叫時機**:只由使用者觸發 —
  - `setup_ubuntu doctor <module>`,或 `bash module/<name>.module.sh doctor`
    = **單一 module 的 runtime health**(「工具實際能不能動」)。
  - `setup_ubuntu doctor`(**不帶 module 參數**)= 另一個東西:engine
    dispatcher 產生的 **system state-vs-host DRIFT 報告**(state.json 記錄
    的安裝清單 vs 主機實況),不是逐 module 的 `doctor()`。見 §8。
- **預設實作**(`module_default_doctor`):`is_installed` **且**(若
  `TEST_VERIFY_CMD` 非空)`TEST_VERIFY_CMD`,任一失敗即回非 0。這刻意與
  `verify` 的預設共用同一個 acceptance probe,再加上 `is_installed`。
- **有真正 runtime surface 的 module 必須覆寫 `doctor()`** 加上真正的健檢
  (例:`docker` 檢查 daemon 在跑 + 使用者在 `docker` group;`git` 檢查
  `git --version` 成功、全域 config 可讀;有 config 的 module 檢查 config
  valid)。macro 之後重新宣告 `doctor()` 即覆寫(bash 後宣告者勝)。

#### 邊界一句話

verify = 在 install context 跑的安裝後驗收(`is_installed` +
`TEST_VERIFY_CMD`);doctor = 隨選健檢,**重用**那些檢查,**再加上**任何
module 專屬的 runtime 檢查。無 runtime surface 的 module(純 CLI、config-drop)
沿用預設即可;有 runtime surface 的必須覆寫。

### 4.2 Idempotency 要求

- `install()` / `upgrade()` 被重複呼叫**必須** exit 0
  - 已裝就 return 0(`install` 通常用 `module_skip_if_installed` 短路);`upgrade` 可重複拉 latest
...
1. **獨立模式**:`bash module/<name>.module.sh <phase> [options]` — 使用者直接執行單一檔案,module 自己 source 依賴 + 解析參數 + 呼叫對應 lifecycle 函式
...
| `module_default_doctor` | `is_installed` + `TEST_VERIFY_CMD?` | 任何 archetype 都用;runtime health = `is_installed` 且(若 `TEST_VERIFY_CMD` 非空)`TEST_VERIFY_CMD`,任一失敗回非 0(§4.1.1);有 runtime surface 的 module 覆寫 |

#### Archetype macros(一行綁定全部 8 個可預設的 lifecycle)

ADR-0027:macro 發出全部可預設的 lifecycle 函式(不再只有舊版的 6~7 個),
外加 `module_provided_version` Sidecar hook。`detect` 與 `is_recommended`
維持 module 自訂。

| Macro | 綁定函式 |
|---|---|
| `module_use_apt_archetype` | `is_installed / install / upgrade / remove / purge / verify / doctor / is_outdated` 全部接到 `module_default_apt_*` + `module_default_verify` + `module_default_doctor` |
...
| `module_emit_post_install` | runner 於 session_end 呼叫 | 發 `action_required` 結構化事件(`attributes.kind=post_install`,message 為 i18n-resolved 字串;空則 return 0)。人讀輸出由 engine 於結尾「Action required」聚合區衍生(PRD §7.7.2,2026-06-06) |
| `module_emit_reboot_required` | 同上 | 若 `REBOOT_REQUIRED=true` 發 `action_required` 事件(`attributes.kind=reboot`),同上聚合 |
...
```
Usage: bash module/<name>.module.sh <phase> [options]

Phases (lifecycle):
  install            run install()
  upgrade            run upgrade()        (exit 2 only for archetype-D modules
                                           that leave it unimplemented)
  remove             run remove()
  purge              run purge()
  verify             run verify()
  doctor             run doctor()         (exit 2 only for archetype-D modules
                                           that leave it unimplemented)
  detect             run detect()         (read-only)
  is-installed       run is_installed()   (read-only)
  is-recommended     run is_recommended() (read-only)
  is-outdated        run is_outdated()    (read-only; exit 2 only for
                                           archetype-D modules that leave it
                                           unimplemented)
...
```bash
#!/usr/bin/env bash
# module/neovim.module.sh — Neovim + nvimdots personal config
...
`install()` 若違反此規則,unit test 會抓出(`test/unit/module/<name>_spec.bats` 內含 path assertion)。

---

## 7. 測試契約

每個 module 必須附帶單元測試 `test/unit/module/<name>_spec.bats`,**最少涵蓋**:
...
v0.1 階段可手動跑 `script/lint-modules.sh`(將於 Phase 2 建立)。
...
## 11. 其他 template 類型的 OUTWARD INTERFACE(fast-expand 契約)

本 repo 除 module 外還有四類可被「fast-expand」產生的檔案:**tool**、
**hook**、**lib**、**test**。本節定義每一類的 **對外介面(outward
interface)** — 這是 fast-expand 產生器與各自 bootstrap **必須滿足** 的
權威契約(authoritative contract)。module 的契約見 §1–§10;本節是其他四類
的對應規範。

> **前瞻標記(forward-looking):** `lib/tool_bootstrap.sh` 與
> `lib/hook_bootstrap.sh` **尚未實作**。以下把它們記為
> template-first 計畫要落地的 **DEFINED TARGET(已定義的目標介面)**,不是
> 「已出貨」的現況;fast-expand 產生器落地時必須以此為準。`lib/<name>.sh`
> 與 bats test 的介面則是已出貨的現況。

### 11.1 Tool template — `tool/<name>.sh`(source `lib/tool_bootstrap.sh`)

單檔、可獨立執行的宿主端小工具(非 module — 不管 install/upgrade/remove/
purge 生命週期、不寫 state)。

- **呼叫方式:** `bash tool/<name>.sh [--dry-run] [--help]`
- **`--help` / `-h`:** 印 usage 到 stdout,`exit 0`
- **未知參數:** 印 usage 到 **stderr**,`exit 2`
- **`--dry-run`:** **不得**改動檔案系統(no filesystem mutation);只 log
  「將會做什麼」
- **退出碼:** 成功 `exit 0`,否則非 0
- **strict 模式:** `set -euo pipefail` + `shopt -s inherit_errexit`
  (always-act 語意,ADR-0007)
- **不得裝宿主套件:** tool **不做** host package install;需要裝套件就改寫成
  一個 module
- **冪等 + 有守衛的編輯:** 用 `grep`-guarded 編輯(先查再改),**永不**盲目
  `mv` / `cp` 覆寫既有檔案
- **日誌:** 一律走 logger 的 `log_*`(不自印 raw echo 當日誌)

### 11.2 Hook template — `.claude/hook/<name>.sh`(source `lib/hook_bootstrap.sh`)

agentic CLI(Claude Code 等)的 PreToolUse / PostToolUse / Stop 鉤子。

- **輸入:** 從 **stdin** 讀 hook JSON
- **`exit 0`:** allow / pass(放行)
- **`exit 2`:** block,並把訊息印到 **stderr**
- **strict 模式:** `set -uo pipefail`(exit-code-contract 腳本,ADR-0007;
  **非** `-euo`)
- **決定性(deterministic):** 除了「放行 / 阻擋」這個決定本身,**不得**有其他
  副作用

### 11.3 Lib template — `lib/<name>.sh`

只被 `source` 的共用函式庫。

- **只可被 source:** guard 拒絕被直接執行(`bash lib/<name>.sh` 應該報錯 /
  no-op,不做事)
- **命名:** public 函式命名 `<filestem>_<name>`(如 `logger_info`);private
  函式前綴底線 `_<name>`
- **無 source-time 副作用:** 被 source 當下不得跑任何動作(只宣告函式 +
  常數)
- **不設頂層 strict 模式:** library **不** `set -e`(strict 模式由最終
  執行的 entrypoint 決定,library 設了會污染 sourcer)

### 11.4 Test template — bats

- **只斷言對外行為(external behavior),永不戳私有內部**(不呼叫 `_`-前綴
  私有函式、不 assert 內部變數)
- **module spec 至少涵蓋**(呼應 §7):
  - metadata sanity(NAME / DESCRIPTION / CATEGORY / SUPPORTED_UBUNTU)
  - 乾淨容器內 `is_installed` 為 false
  - dry-run 為 no-op(`--partial "DRY-RUN"`)
  - idempotency(lifecycle 跑兩次都 exit 0)
  - standalone CLI:無參數 → usage + `exit 2`、`--help`、`--version`、未知 phase
  - **每一個 lifecycle 函式,含 `doctor`**
- **tool spec 至少涵蓋:** `--help` → usage + `exit 0`;未知參數 → `exit 2`;
  `--dry-run` 不改動檔案系統
- **hook spec 至少涵蓋:** block 路徑(`exit 2` + 訊息)與 allow 路徑
  (`exit 0`)
