...
9. 作為清理使用者,我想要 remove / purge 時用 `--with-orphans` 一併清掉不再
   被其他 module 依賴的孤兒 dep,以便環境保持乾淨。(`--with-orphans` 0.1.0
   尚未實作,目前退 exit 2 —— 見 Out of Scope 的 Deferred 清單。)
...
18. 作為無 sudo 使用者,我想要工具自動偵測並 fallback 到 user-home 安裝,
    以便在受限環境下也能用。(user-home 安裝目標 ADR-0017 於 0.1.0 尚未實作,
    目前僅支援 sudo 安裝 —— 見 Out of Scope 的 Deferred 清單。)
...
31. 作為 optional 使用者,我想要用 `install --tag=<group>` 一次裝整組工具
    (如 cli-essentials),以便不必逐一點名。(`install --tag=*` 批次路徑
    ADR-0014 於 0.1.0 尚未接線,install 目前拒絕 `--tag=*`(退 exit 2)——
    見 Out of Scope 的 Deferred 清單。)
...
- **optional**:預設不勾選、使用者主動選擇。以 `TAGS[0]` 再分次群組
  ...
  CLI 的 `--tag=<group>` 整群安裝設計已定案(ADR-0014)但 0.1.0 尚未接線
  (install 目前拒絕 `--tag=*`,見 Out of Scope 的 Deferred 清單)。
...
最後對「未設 enabled」者呼叫 `is_recommended()`。`--force` 設計上可跨過這些
過濾強裝(ADR-0012),但**0.1.0 尚未實作** —— `--force` 目前退 exit 2(見
Out of Scope 的 Deferred 清單)。使用者也可用 `--profile` 或 config 覆寫偵測
到的平台。
...
`--state-dir`、`--install-target=auto|sudo|user-home`、`--profile`。其中
`--install-target=user-home`(ADR-0017)於 0.1.0 尚未實作,目前僅 sudo 安裝
(見 Out of Scope 的 Deferred 清單)。
...
rollback,只 warn 提示手動 pin 版本)。**設計上** upgrade 與 install 共用同一條
install -> verify pipeline(ADR-0015)、verify 失敗即 upgrade 失敗並觸發回滾;
但**0.1.0 尚未實作此綁定** —— verify 為獨立步驟、不觸發自動回滾或 state 移除
(見 Out of Scope 的 Deferred 清單)。
...
- **State** 是機器寫入的執行期事實(XDG_STATE_HOME 下),不由使用者手改。
  ...
  **0.1.0 實際**:install 成功即寫 state;verify 是**獨立**步驟、永不移除
  state。ADR-0015 設計的「verify 失敗自動 purge 收尾且不寫 state」尚未實作
  (見 Out of Scope 的 Deferred 清單)。
...
- **Log** 的結構化 JSONL schema 對齊 OTel + W3C Trace Context(ADR-0006),
  主要消費者是 agent 做診斷。**0.1.0 實際**:schema 已就緒,但 production
  **未開任何 log 檔** —— 每 session 的 log file 輸出、`log_info`→JSONL 橋接與
  session 結尾保留清理(近 30 天且至多 100 檔)皆屬 observability 落檔,尚未
  實作(見 Out of Scope 的 Deferred 清單)。
...
依序偏好 pass > gnome-keyring > encrypted-file(絕不寫明文;encrypted-file 以
openssl AES-256-CBC + PBKDF2 加密;可用 `INIT_UBUNTU_SECRETS_BACKEND` 環境
變數覆寫;選擇邏輯見 ADR-0016)。把 secrets 抽離主流程是刻意的:避免在批次
安裝時誤洩敏感資料。ADR-0016 設計的 `resolved_backend` session-pinning 與
age 佈局尚未實作(見 Out of Scope 的 Deferred 清單)。
...
### 設計已定案、但 0.1.0 尚未實作(Deferred / design-accepted, not yet built)

以下項目的設計已在對應 ADR 收斂並被接受,但 **0.1.0 尚未實作**;它們既非
「明確不做」(見上「非目標」),也非「路線圖上排定的未來版本」(見下「未來
工作」),而是**已定案設計、待建成**。本規格其餘各節在提到它們時會標記
「尚未實作」並指回此清單。列出「已定案設計」與「0.1.0 實際行為」兩欄,避免
把未建成的東西當成 as-built:

- **`--force` 軟/硬過濾語意(ADR-0012)**:設計上 `--force` 應跨過
  `SUPPORTED_PLATFORMS` 硬過濾與 enabled 軟過濾強裝。**0.1.0 實際**:`--force`
  與 `--with-orphans` 目前皆 HARD-ERROR、退 exit 2(尚未實作,不再是舊版的
  no-op 忽略)。
- **verify 失敗自動 purge 回滾(ADR-0015)**:設計上 install 成功但 verify
  失敗應自動跑 purge 收尾、不寫 state(install 與 verify 綁成單一 pipeline)。
  **0.1.0 實際**:install 成功即寫 state;verify 是**獨立**步驟,自成一格、
  永不移除 state。upgrade 亦同 —— 無 verify 失敗觸發的自動回滾。
- **secrets `resolved_backend` session-pinning + age 佈局(ADR-0016)**:設計上
  應把選定後端 pin 進 session、並偏好 age 加密佈局。**0.1.0 實際**:後端為
  encrypted-file(openssl,AES-256-CBC + PBKDF2),優先序 pass > gnome-keyring
  > encrypted-file,可用環境變數 `INIT_UBUNTU_SECRETS_BACKEND` 覆寫;
  session-pinning 與 age 佈局延後。
- **user-home 安裝目標(ADR-0017)**:設計上無 sudo 時應 fallback 到 user-home
  安裝。**0.1.0 實際**:未實作 —— `--install-target=user-home`、
  `SUPPORTS_USER_HOME`、exit 4、state 的 `user_home_root` 皆為預留設計,0.1.0
  只支援 sudo 安裝。
- **`list --json` 面向 agent 的 wrapper schema(ADR-0019)**:設計上 `--json`
  應輸出穩定的 agent-facing schema。**0.1.0 實際**:輸出為精簡的 TUI feed;
  完整 agent schema 待 TUI 決策定案後再補。
- **TUI 不收自由文字輸入(ADR-0025)**:設計上 TUI 全程只勾選、不收 free-form
  text。**0.1.0 實際**:未實作 —— TUI 仍透過 `tui_render_input` 委派文字輸入。
- **`sync --include-user-local-modules`(ADR-0013)**:設計上此 flag 應把
  user-local module 一併納入同步 payload(附「同步未受信任的私有 module」的
  信任警告)。**0.1.0 實際**:此 flag 目前退 exit 2,延後至實作完成再開放。
- **`install --tag=<group>` 批次路徑(ADR-0014)**:設計上 `install --tag=config`
  等應一次安裝整組。**0.1.0 實際**:未接線 —— install 目前拒絕 `--tag=*`
  (退 exit 2)。
- **結構化 log 落檔 + `log_info`→JSONL bridge(ADR-0006)**:OTel / W3C Trace
  Context 對齊的 JSONL schema 已就緒,但 **production 未開任何 log 檔**;每
  session 的 log file 輸出與 `log_info`→JSONL 橋接(observability 落檔)延後。
...
>
> **注意**:下列表格為求完整仍列出若干「設計已定案、0.1.0 尚未實作」的 flag
> 與 schema 欄位(如 `--force`、`--with-orphans`、`install --tag=*`、
> `--install-target=user-home`、`sync --include-user-local-modules`、
> `list --json` 的 agent schema、state 的 `user_home_root`);這些以行內
> 「(尚未實作)」標記,權威清單見 Out of Scope 的 Deferred 一節。

### A. CLI subcommand 表

| Subcommand | Args | 主要 Flags | 行為 | 對標 apt |
|---|---|---|---|---|
| `install` | `<module>...` | `-y`、`--dry-run`、`--no-deps`、`--base`、`--recommended`、`--category=<n>`、`--tag=<t>`、`--install-target=auto\|sudo\|user-home`、`--force` | 安裝指定 module(自動帶 dep);非 `-y` 時先列 plan 問 `Proceed? [Y/n]`。**尚未實作**:`--force`、`--with-orphans` 退 exit 2;`--tag=*` 被拒(exit 2);`--install-target=user-home` 只支援 sudo(見 Deferred) | `apt install` |
| `remove` | `<module>...` | `-y`、`--dry-run`、`--with-orphans` | 移除 module(保留 config);`--with-orphans` **尚未實作**(退 exit 2) | `apt remove` |
| `purge` | `<module>...` | `-y`、`--dry-run`、`--with-orphans` | 完整移除 module + config;`--with-orphans` **尚未實作**(退 exit 2) | `apt purge` |
...
| `list` | — | `--category=<n>`、`--installed`、`--upgradable`、`--available`、`--tag=<t>`、`--json` | 列出 module;`--json` 目前輸出精簡的 TUI feed,面向 agent 的完整 schema(ADR-0019)**尚未實作**(待 TUI 決策) | `apt list` |
...
| `sync` | `<user@host>` | `--modules=<list>`、`--include-user-local-modules`、`--pull`、`--apply` | SSH 跨機同步;預設 dry-run(ADR-0013);`--include-user-local-modules` **尚未實作**(退 exit 2) | — |
...
| `--install-target=auto\|sudo\|user-home` | 強制安裝目標(預設 auto);`user-home`(ADR-0017)**尚未實作**,0.1.0 僅 sudo 安裝 |
...
> --json`,動作 fork `setup_ubuntu <subcommand>` 子程序;TUI 自身不 source
> engine lib、不寫 state。Rich 層(fzf)以兩欄式導航器呈現(左欄目前層級、
> 右欄即時預覽),Fallback 層(whiptail)以對話框呈現,兩層共用同一資料層與
> 同一條安裝路徑(ADR-0024)。**尚未實作**:`list --json` 的 agent-facing
> schema(ADR-0019)目前只是精簡 TUI feed;ADR-0025 的「TUI 全程不收 free-form
> text」尚未落實 —— TUI 仍透過 `tui_render_input` 委派文字輸入(見 Deferred)。
...
下例 neovim 的 `install_target_resolved: "user-home"` 與 `user_home_root`
是**預留設計**:user-home 安裝目標(ADR-0017)於 0.1.0 尚未實作,實際只會
出現 `sudo`(見 Out of Scope 的 Deferred 清單)。
