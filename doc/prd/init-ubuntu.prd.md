...
| Module 檔名 | 來源 | 說明 | 依賴 |
|---|---|---|---|
| `git.module.sh` | (新建) | git 版本控制(archetype-A,單一 apt 套件) | — |
| `vim.module.sh` | (新建) | vim 編輯器(archetype-A,單一 apt 套件) | — |
| `curl.module.sh` | (新建) | curl HTTP 客戶端(archetype-A,單一 apt 套件) | — |
| `wget.module.sh` | (新建) | wget 下載工具(archetype-A,單一 apt 套件) | — |
| `jq.module.sh` | (新建) | jq JSON 處理器(archetype-A,單一 apt 套件) | — |
| `build-essential.module.sh` | (新建) | build-essential 編譯工具鏈(archetype-A,單一 apt 套件) | — |
| `htop.module.sh` | (新建) | htop 行程監控(archetype-A,單一 apt 套件) | — |
| `unzip.module.sh` | (新建) | unzip 解壓工具(archetype-A,單一 apt 套件) | — |
| `shell.module.sh` | `module/setup_shell.sh` | bash 設定 + 基本 alias | git curl |

> **base 由 bundle 拆為 per-tool module(ADR-0026,supersedes ADR-0011 的 bundle 機制)**:原 `apt-essentials.module.sh` 已拆為上列 8 個獨立 base module,各自為 archetype-A 單一 apt 套件、`CATEGORY=base`、`DEPENDS_ON=()`,可獨立安裝 / 移除。
> **`ca-certificates` 與 `software-properties-common` 不是 user-facing module** — 它們是 transitive apt 依賴:`ca-certificates` 由 `curl` / `wget` / `git` 自動拉入;`software-properties-common` 由任何加 PPA 的 module 自行安裝。
...
|---|---|---|---|---|
| `nvidia-driver.module.sh` | `module/setup_nvidia_driver.sh` | NVIDIA 顯卡驅動(`RISK_LEVEL=high`,含失敗回復機制,見 §11.3) | git curl | 偵測到 NVIDIA GPU **且** 非 VM/container/WSL/jetson |
| `docker.module.sh` | `module/setup_docker.sh` | Docker Engine + Compose plugin | curl | 非 container 內 |
| `fish.module.sh` | `module/config/fish/` 為基礎 | Fish shell + 個人 config | curl shell | 永遠 |
| `neovim.module.sh` | `module/setup_neovim.sh`(拆分) | Neovim + 推薦勾選 nvimdots(default Enter 即裝) | curl, git-config, fzf, lazygit, ripgrep, fdfind, fnm | desktop / server / SBC 皆 yes(個人主力編輯器) |
| `font.module.sh` | `module/setup_font.sh` | Nerd Font(Cascadia / FiraCode) | curl unzip | 有桌面環境 |
| `tmux.module.sh` | `module/config/tmux/` | tmux + 個人 config | git curl | 永遠(日用必裝) |
...
| Module 檔名 | 來源 | 說明 | 依賴 |
|---|---|---|---|
| `lazygit.module.sh` | `module/submodules/lazygit.sh` | Git TUI | curl git |
| `lazydocker.module.sh` | `module/submodules/lazydocker.sh` | Docker TUI | docker |
| `fzf.module.sh` | `module/submodules/fzf.sh` | Fuzzy finder | curl |
...
| `notion.module.sh` | (新建,Q50 / #35) | Notion 桌面版(github-release archetype 吃 notion-electron `.deb`;取代 small-tools snap 路徑;`SUPPORTED_PLATFORMS=("desktop")`) | curl | notes |
| `jetson-stats.module.sh` | (新建,Q51 / #37) | jetson-stats(`jtop` 監控 TUI;pip 安裝;`SUPPORTED_PLATFORMS=("jetson-orin")`) | git curl | hardware |
...
> **`DEPENDS_ON` 規則(Q39,2026-06-06;ADR-0026 更新)**:元素**只允許 module 名**(registry 內解析得到的 `NAME`),不得填非 module 的 apt pkg 名或平台條件(平台限制走 `SUPPORTED_PLATFORMS`)。注意:`git` / `curl` / `vim` / `wget` / `jq` / `build-essential` / `htop` / `unzip` 現在**本身就是 base module**(ADR-0026 拆自舊 `apt-essentials` bundle),故可合法填入 `DEPENDS_ON`;規則不變的是仍不得填**非 module 的** apt 套件名(例:`ca-certificates` 不是 module,而是 transitive 依賴)。`doctor --validate-modules` 驗證每個 dep 都能解析,解析不到 = metadata 不合法(exit 2)。
...
| `installed.<name>.local.frozen_pkgs` | string[] | 安裝當下 freeze 的 pkg 集(經 compat filter)| 僅適用「install 解析為一組套件」的 module(ADR-0011 機制);舊 `apt-essentials` bundle 已不存在(ADR-0026 §3:恰為單一 apt 套件的 base module 無需 freeze) |
| `installed.<name>.local.frozen_platform` | string | freeze 當下偵測到的 form factor(診斷用) | 同上;僅 multi-pkg module 適用 |
...
| # | Question | **決定** |
|---|---|---|
| Q1 | `apt-essentials.module.sh` 該裝哪些套件? | **Universal devel pkg list,全平台同一份**(ADR-0011 修正版,2026-05-20):任何 devel 平台都裝 `git` / `vim` / `curl` / `wget` / `ca-certificates` / `build-essential` / `htop` / `unzip` / `jq` / `software-properties-common`,僅以「相容性 + 功能重複」filter 排除。**(superseded by ADR-0026:apt-essentials bundle 已拆為 8 個 per-tool base module,`ca-certificates` / `software-properties-common` 改為 transitive apt 依賴)** |
...
| Q39 | `DEPENDS_ON=("git")` 懸空(`git` 非 module)? | **dep 一律是 module 名**;`git-config` / `lazygit` / `fzf` 三處改 `apt-essentials`;§9.1 補規則,`doctor --validate-modules` lint。**(superseded by ADR-0026:apt-essentials 不再是 module,deps 改填具體 tool module;`git` / `curl` 等本身已是 base module,故 `DEPENDS_ON=("git")` 現為合法)** |
...
| Q49 | anydesk 依賴欄寫平台條件? | **dep 改 `apt-essentials`**;條件移 `SUPPORTED_PLATFORMS=("desktop")`(Q39 規則一併防堵)。**(superseded by ADR-0026:anydesk dep 改為 `curl`)** |
...
|---|---|---|---|---|
| `apt-essentials`(superseded by ADR-0026:已拆為 per-tool base module) | + GUI 套件 | 純 CLI | Jetson SDK 子集 | 純 CLI |
