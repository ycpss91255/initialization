...
| `config load` | `[<module>]` | `-y` | 批次套用 modules/config/*(對映 module 的 config 部分,不裝主程式) | — |
...
| AC-23 | `bash modules/docker.module.sh install --dry-run` 跑完,Sidecar 寫入正確(Standalone 模式)但 `state.json` 完全沒變(ADR-0001) |
...
| AC-25 | 全 10 個 lifecycle 函式對每個 module 都能跑(`bash modules/<m>.module.sh <phase>` 都 exit 0 或預期 Query-no 的 exit 1,絕無「not implemented」exit 2) |
...
| Q6 | `modules/config/` 內的 config 檔該怎麼套用? | 每個有對應的 `<name>-config.module.sh` 含 install / remove;**另加 subcommand `config load`** 做批次套用 |
