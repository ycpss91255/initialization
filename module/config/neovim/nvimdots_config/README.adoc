...
| 複製&貼上當前行 | `N` | kbd:[<C-p>]
| 貼上暫存器內容 | `N` | kbd:[<A-p>]
| 貼上上一行相同位置的字元 | `N` | kbd:[<C-y>]

| Markdown 預覽 (使用Web瀏覽器) | `N` | kbd:[<F12>]

| 儲存當前工作階段 (要在專案中) | `N` | kbd:[<Leader>ss]
| 載入當前工作階段 (要在專案中) | `N` | kbd:[<Leader>sl]
| 刪除當前工作階段 (要在專案中) | `N` | kbd:[<Leader>sd]

| 計算遊標/選擇下的表達式(僅支援 C, Cpp, rust ,go, Python) | `N` | kbd:[K]
|===
...
[cols="7,^2,^3", option="header"]
|===
| 開啟 `lazygit` | `N` | kbd:[<Leader>gg]
| 開啟暫存列表 | `N` | kbd:[<Leader>gG]
| 開啟 diff 檢視 | `N` | kbd:[<Leader>gd]
| 關閉 diff 檢視 | `N` | kbd:[<Leader>gD]
| 當前變更/選擇行暫存 | `N`/`V` | kbd:[<Leader>gs]
| 清除當前變更/選擇行不在暫存區的內容 | `N`/`V` | kbd:[<Leader>gr]
| 檔案/選擇行取消暫存 | `N` | kbd:[<Leader>gu]
| 清除當前檔案不在暫存區的內容 | `N` | kbd:[<Leader>gR]
| 預覽變更 | `N` | kbd:[<Leader>gp]
| 找兇手 | `N` | kbd:[<Leader>gb]
|===
...
| AsciiDoc & Markdown 預覽 | `N` | kbd:[<F12>]
|===
