#!/usr/bin/env bash
# module/git.module.sh — git: distributed version control  [archetype: apt]
#
...
# `git` package in main; the binary is `git`.
#
# Standalone usage:
#   bash module/git.module.sh install [--dry-run]
#   bash module/git.module.sh upgrade / remove / purge / verify
#   bash module/git.module.sh detect / is-installed / is-recommended / is-outdated
#   bash module/git.module.sh info / status        (read-only metadata views)
#
# Engine usage (resolves DEPENDS_ON, batches with state.json):
#   setup_ubuntu install git
...
declare -gA DESCRIPTION=(
    [en]="git — distributed version control system (apt git package)"
    [zh-TW]="git — 分散式版本控制系統(apt git 套件)"
)
...
TEST_VERIFY_CMD="command -v git && git --version"
...
install() {
    ...
    module_sidecar_write "${NAME}" "$(_git_pkg_version)"
}
...
doctor() {
    if ! is_installed; then
        log_warn "[${NAME}] doctor: git is not installed"
        return 1
    fi
    local _bin
    _bin="$(command -v git 2>/dev/null)" || {
        log_warn "[${NAME}] doctor: git binary not found on PATH"
        return 1
    }
...
_git_pkg_version() {
    local _ver=""
    _ver="$(dpkg-query -W -f='${Version}' git 2>/dev/null)" || _ver=""
