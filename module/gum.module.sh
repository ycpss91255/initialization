#!/usr/bin/env bash
# module/gum.module.sh — gum charmbracelet TUI toolkit  [archetype: github-release]
#
# gum is the preferred modern TUI backend for setup_ubuntu_tui.sh (issue #171;
# gum > whiptail, dialog dropped). It is a single static Go binary shipped per
# release as gum_<ver>_Linux_<arch>.tar.gz (multi-arch: x86_64 / arm64 / armv7
# → covers x86_64 desktops, rpi4/5, Jetson). Installs to ~/.local/bin with no
# sudo (AC-13 spirit). The asset name is version- and arch-dependent, so
# install/upgrade resolve the latest tag + arch first, then super-call the
# archetype default fetch (doc/guide/archetype-cookbook.md §B). Sidecar
# lifecycle per ADR-0001 / module-spec §4.7.4: written on install/upgrade
# success, deleted on remove/purge.
#
# Standalone usage:
#   bash module/gum.module.sh install [--dry-run]
#   bash module/gum.module.sh upgrade / remove / purge / verify
#   bash module/gum.module.sh detect / is-installed / is-recommended / is-outdated
#   bash module/gum.module.sh info / status        (read-only metadata views)
#
# Engine usage (resolves DEPENDS_ON, batches with state.json):
#   setup_ubuntu install gum

# ── Dual-mode header ────────────────────────────────────────────────────────
MODULE_STANDALONE="true"
[[ "${BASH_SOURCE[0]:-}" != "${0:-}" ]] && MODULE_STANDALONE="false"
if [[ "${MODULE_STANDALONE}" == "true" ]]; then
    # shellcheck source=../lib/module_bootstrap.sh
    source "${LIB_DIR:-$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")/../lib" && pwd -P)}/module_bootstrap.sh"
    module_bootstrap
fi
# Static-analysis hint (never executed: the guard is always false; wrapped in
# kcov-exclude so the dead line is not counted against coverage). module_bootstrap
# sources the lib helpers at runtime, but shellcheck cannot trace that 2-level
# dynamic source — this guarded line lets `shellcheck -x` follow module_helper.sh
# so it sees the metadata + archetype vars below are used externally (avoids SC2034).
# kcov-exclude-start
# shellcheck source=../lib/module_helper.sh
[[ -n "${__module_lint_hint:-}" ]] && source "${LIB_DIR}/module_helper.sh"
# kcov-exclude-end

# ── Metadata (doc/module-spec.md §3) ────────────────────────────────────────
NAME="gum"
VERSION_PROVIDED="latest"
CATEGORY="optional"
TAGS=("cli-essentials" "tui")
HOMEPAGE="https://github.com/charmbracelet/gum"
declare -gA DESCRIPTION=(
    [en]="gum — a tool for glamorous shell scripts (preferred modern TUI backend)"
    [zh-TW]="gum — 為 shell 腳本打造的互動式介面工具(偏好的現代 TUI 後端)"
)
declare -gA POST_INSTALL_MESSAGE=(
    [en]="Ensure ~/.local/bin is on PATH so setup_ubuntu_tui.sh can detect gum."
    [zh-TW]="請確認 ~/.local/bin 已加入 PATH,setup_ubuntu_tui.sh 才能偵測到 gum。"
)
declare -gA WARN_MESSAGE=()
SUPPORTED_UBUNTU=("22.04" "24.04" "26.04")
SUPPORTED_PLATFORMS=("desktop" "server" "wsl")
DEPENDS_ON=("curl")
CONFLICTS_WITH=()
SUPPORTS_USER_HOME=true
RISK_LEVEL="low"
REBOOT_REQUIRED=false
INSTALL_TARGET_DEFAULT="user-home"
TEST_VERIFY_CMD="command -v gum && gum --version"

# ── Archetype B — GitHub release (multi-arch, user-home, no sudo) ────────────
GITHUB_REPO="charmbracelet/gum"
# Asset name is version- and arch-dependent (gum_<ver>_Linux_<arch>.tar.gz);
# resolved at run time by _gum_resolve_asset_pattern. Placeholder keeps the
# archetype data contract complete for `info` / dry-run output.
GITHUB_ASSET_PATTERN="gum_<latest>_Linux_<arch>.tar.gz"
INSTALL_DIR="${HOME}/.local/share/init_ubuntu/gum"
BIN_NAME="gum"
# Tarball nests everything under gum_<ver>_Linux_<arch>/ ; strip that one
# leading component so the binary lands at INSTALL_DIR/gum.
BIN_PATH_IN_TAR="gum"
BIN_LINK="${HOME}/.local/bin/gum"
STRIP_COMPONENTS=1
USE_SUDO=false
CONFIG_PATHS=()
module_use_github_release_archetype

# Override install/upgrade: resolve the versioned + arch'd asset name first,
# then super-call the archetype fetch; write the Sidecar on success (ADR-0001).
install() {
    module_dryrun_guard install \
        "fetch ${GITHUB_REPO} latest -> ${INSTALL_DIR}, symlink ${BIN_LINK}" \
        && return 0
    module_skip_if_installed && return 0
    _gum_resolve_asset_pattern || return $?
    mkdir -p "${BIN_LINK%/*}"
    _module_github_release_fetch_and_install || return $?
    module_sidecar_write "${NAME}" "${_GUM_TARGET_VERSION:-unknown}"
}

upgrade() {
    module_dryrun_guard upgrade "force re-download ${GITHUB_REPO} latest" \
        && return 0
    _gum_resolve_asset_pattern || return $?
    mkdir -p "${BIN_LINK%/*}"
    _module_github_release_fetch_and_install || return $?
    module_sidecar_write "${NAME}" "${_GUM_TARGET_VERSION:-unknown}"
}

remove() {
    module_dryrun_guard remove \
        "rm ${INSTALL_DIR} + ${BIN_LINK} + Sidecar" \
        && return 0
    module_default_github_release_remove || return $?
    module_sidecar_remove "${NAME}"
}

purge() {
    module_dryrun_guard purge \
        "rm ${INSTALL_DIR} + ${BIN_LINK} + Sidecar + CONFIG_PATHS" \
        && return 0
    module_default_github_release_purge || return $?
    module_sidecar_remove "${NAME}"
}

# detect — gum ships a Linux tarball for x86_64 / arm64 / armv7 only.
detect() {
    _gum_arch >/dev/null 2>&1
}

is_recommended() {
    ! is_installed
}

# is_outdated — compare Sidecar (or binary-reported) version against the
# latest GitHub release tag (doc/guide/archetype-cookbook.md, Archetype B).
is_outdated() {
    is_installed || return 1
    local _local="" _remote=""
    _local="$(_gum_installed_version)" || _local=""
    if declare -F get_github_pkg_latest_version >/dev/null 2>&1; then
        get_github_pkg_latest_version _remote "${GITHUB_REPO}" 2>/dev/null \
            || _remote=""
    fi
    [[ -n "${_remote}" && "${_local}" != "${_remote}" ]]
}

# doctor — metadata contract self-check + Sidecar invariant
# (module-spec §4.7.4: is_installed ⟷ Sidecar exists).
doctor() {
    local _ok=0
    if ! _gum_metadata_selfcheck; then
        log_warn "[${NAME}] doctor: metadata contract check failed"
        _ok=1
    fi
    local _sidecar
    _sidecar="$(module_sidecar_path "${NAME}")"
    if is_installed; then
        if [[ ! -f "${_sidecar}" ]]; then
            log_warn "[${NAME}] doctor: installed but Sidecar missing (ADR-0001 drift; re-run install/upgrade to heal)"
            _ok=1
        fi
    else
        if [[ -e "${_sidecar}" ]]; then
            log_warn "[${NAME}] doctor: Sidecar present but ${BIN_NAME} not installed (ADR-0001 drift; rm ${_sidecar} or reinstall)"
            _ok=1
        fi
    fi
    return "${_ok}"
}

# ── Private helpers ─────────────────────────────────────────────────────────

# Map `uname -m` to gum's release asset arch suffix.
# x86_64 → x86_64, aarch64/arm64 → arm64, armv7l → armv7.
_gum_arch() {
    case "$(uname -m)" in
        x86_64)        printf 'x86_64' ;;
        aarch64|arm64) printf 'arm64' ;;
        armv7l)        printf 'armv7' ;;
        *)             return 1 ;;
    esac
}

# Resolve the latest release tag + arch and materialise the concrete asset
# name into GITHUB_ASSET_PATTERN for the archetype fetch.
_gum_resolve_asset_pattern() {
    local _arch=""
    _arch="$(_gum_arch)" || {
        log_error "[${NAME}] unsupported architecture: $(uname -m)"
        return 1
    }
    local _ver=""
    if declare -F get_github_pkg_latest_version >/dev/null 2>&1; then
        get_github_pkg_latest_version _ver "${GITHUB_REPO}" || _ver=""
    fi
    if [[ -z "${_ver}" ]]; then
        log_error "[${NAME}] could not resolve latest ${GITHUB_REPO} version"
        return 1
    fi
    GITHUB_ASSET_PATTERN="gum_${_ver}_Linux_${_arch}.tar.gz"
    _GUM_TARGET_VERSION="${_ver}"
}

# Installed version: Sidecar first (fast, offline, module_sidecar_* shared
# helpers), fall back to parsing `gum --version` (pre-Sidecar installs).
_gum_installed_version() {
    if module_sidecar_get_version "${NAME}" 2>/dev/null; then
        return 0
    fi
    local _bin="${BIN_LINK:-${HOME}/.local/bin/${BIN_NAME}}"
    [[ -x "${_bin}" ]] || _bin="${BIN_NAME}"
    # `gum version <ver>` / `gum version v<ver>` → bare version.
    "${_bin}" --version 2>/dev/null \
        | sed -n 's/.*[[:space:]]v\{0,1\}\([0-9][0-9.]*\).*/\1/p' \
        | head -n1 \
        || true
}

# Engine-contract assertions (also exercised by `doctor`): every metadata
# field the engine consumes post-source must be declared and well-formed.
_gum_metadata_selfcheck() {
    [[ -n "${DESCRIPTION[en]:-}" ]] || return 1
    [[ "${#POST_INSTALL_MESSAGE[@]}" -ge 0 ]] || return 1
    [[ "${#WARN_MESSAGE[@]}" -ge 0 ]] || return 1
    [[ "${SUPPORTS_USER_HOME}" == "true" || "${SUPPORTS_USER_HOME}" == "false" ]] || return 1
    [[ "${INSTALL_TARGET_DEFAULT}" == "sudo" || "${INSTALL_TARGET_DEFAULT}" == "user-home" || "${INSTALL_TARGET_DEFAULT}" == "auto" ]] || return 1
    return 0
}

# ── Standalone footer ───────────────────────────────────────────────────────
if [[ "${MODULE_STANDALONE:-false}" == "true" ]]; then
    module_standalone_main "$@"
fi
