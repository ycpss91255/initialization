#!/usr/bin/env bash
# module/htop.module.sh — htop — interactive process viewer (apt htop package)  [archetype: apt]
#
...
# `htop` package; the binary is `htop`.
#
# Standalone usage:
#   bash module/htop.module.sh install [--dry-run]
#   bash module/htop.module.sh upgrade / remove / purge / verify
#   bash module/htop.module.sh detect / is-installed / is-recommended / is-outdated
#   bash module/htop.module.sh info / status        (read-only metadata views)
#
# Engine usage (resolves DEPENDS_ON, batches with state.json):
#   setup_ubuntu install htop
...
declare -gA DESCRIPTION=(
    [en]="htop — interactive process viewer (apt htop package)"
    [zh-TW]="htop — 互動式行程監看工具(apt htop 套件)"
)
...
TEST_VERIFY_CMD="command -v htop && htop --version | head -n1"
...
install() {
    ...
    module_sidecar_write "${NAME}" "$(_htop_pkg_version)"
}
...
doctor() {
    if ! is_installed; then
        log_warn "[${NAME}] doctor: htop is not installed"
        return 1
    fi
    local _bin
    _bin="$(command -v htop 2>/dev/null)" || {
        log_warn "[${NAME}] doctor: htop binary not found on PATH"
        return 1
    }
...
_htop_pkg_version() {
    local _ver=""
    _ver="$(dpkg-query -W -f='${Version}' htop 2>/dev/null)" || _ver=""
