...
install() {
    ...
    module_sidecar_write "${NAME}" "$(_jq_pkg_version)"
}
...
doctor() {
    if ! is_installed; then
        log_warn "[${NAME}] doctor: jq is not installed"
        return 1
    fi
    local _bin
    _bin="$(command -v jq 2>/dev/null)" || {
        log_warn "[${NAME}] doctor: jq binary not found on PATH"
        return 1
    }
...
_jq_pkg_version() {
    local _ver=""
    _ver="$(dpkg-query -W -f='${Version}' jq 2>/dev/null)" || _ver=""
