...
if ! have_sudo_access; then
    if [[ "${MAIN_FILE}" == "true" ]]; then
        log_fatal "No sudo access. Cannot continue install 'lazygit'."
    else
        log_warn "Skip install 'lazygit' due to no sudo access."
        return 1
    fi
fi
...
function install_lazygit() {
    ...
    if check_pkg_status --exec -- "${_bin_file}" ;then
        _install_version="$("${_bin_file}" --version | sed -n 's/.*, version=\([^,]*\),.*/\1/p')"
    ...
    if [[ "${_no_download}" == "false" ]]; then
        ...
        _download_file="${_pkg_name}_${_latest_version}_Linux_x86_64.tar.gz"
        _repo_url="https://github.com/${_github_repo}/releases/latests/download/${_download_file}"
...
install_lazygit "jesseduffield/lazygit" "lazygit"
