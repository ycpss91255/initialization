#!/usr/bin/env bash
# module/vim.module.sh — vim — Vi IMproved terminal text editor (apt vim package)  [archetype: apt]
#
...
# `vim` package; the binary is `vim`.
#
# Standalone usage:
#   bash module/vim.module.sh install [--dry-run]
#   bash module/vim.module.sh upgrade / remove / purge / verify
#   bash module/vim.module.sh detect / is-installed / is-recommended / is-outdated
#   bash module/vim.module.sh info / status        (read-only metadata views)
#
# Engine usage (resolves DEPENDS_ON, batches with state.json):
#   setup_ubuntu install vim
...
declare -gA DESCRIPTION=(
    [en]="vim — Vi IMproved terminal text editor (apt vim package)"
    [zh-TW]="vim — Vi IMproved 終端機文字編輯器(apt vim 套件)"
)
...
TEST_VERIFY_CMD="command -v vim && vim --version | head -n1"
...
install() {
    ...
    module_sidecar_write "${NAME}" "$(_vim_pkg_version)"
}
...
doctor() {
    if ! is_installed; then
        log_warn "[${NAME}] doctor: vim is not installed"
        return 1
    fi
    local _bin
    _bin="$(command -v vim 2>/dev/null)" || {
        log_warn "[${NAME}] doctor: vim binary not found on PATH"
        return 1
    }
...
_vim_pkg_version() {
    local _ver=""
    _ver="$(dpkg-query -W -f='${Version}' vim 2>/dev/null)" || _ver=""
