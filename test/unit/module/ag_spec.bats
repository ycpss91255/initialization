#!/usr/bin/env bats
# test/unit/module/ag_spec.bats — module/ag.module.sh
#
...
_load_module() {
    ...
    # shellcheck source=../../../module/ag.module.sh
    source "${MODULE_DIR}/ag.module.sh"
}

_standalone_module() {
    bash "${MODULE_DIR}/ag.module.sh" "$@"
}
...
@test "ag module file parses (bash -n)" {
    run bash -n "${MODULE_DIR}/ag.module.sh"
    assert_success
}

@test "ag module sources cleanly in engine mode (MODULE_STANDALONE=false)" {
    _load_module
    [[ "${MODULE_STANDALONE}" == "false" ]]
}

@test "ag module defines all 10 lifecycle functions" {
    _load_module
    local _fn
...
@test "ag module declares NAME=ag" {
    _load_module
    [[ "${NAME}" == "ag" ]]
}

@test "ag module CATEGORY=optional" {
    _load_module
    [[ "${CATEGORY}" == "optional" ]]
}

@test "ag module TAGS contains cli" {
    _load_module
    [[ " ${TAGS[*]} " == *" cli "* ]]
}

@test "ag module DEPENDS_ON is empty" {
    _load_module
    [[ "${#DEPENDS_ON[@]}" -eq 0 ]]
}

@test "ag DESCRIPTION is associative with en + zh-TW entries" {
    _load_module
...
@test "ag module_get_description returns language-specific text + en fallback" {
    _load_module
    [[ -n "$(module_get_description en)" ]]
    [[ "$(module_get_description zh-TW)" == *"grep 替代"* ]]
    [[ "$(module_get_description xx)" == "$(module_get_description en)" ]]
}

@test "ag SUPPORTED_UBUNTU includes 22.04 24.04 26.04" {
    _load_module
...
@test "ag module RISK_LEVEL=low and VERSION_PROVIDED=apt-managed" {
    _load_module
    [[ "${RISK_LEVEL}" == "low" ]]
    [[ "${VERSION_PROVIDED}" == "apt-managed" ]]
}

@test "ag archetype data installs the silversearcher-ag apt package" {
    _load_module
...
@test "is_installed returns zero when dpkg reports the package as ii" {
    _load_module
    MOCK_DPKG_OUTPUT='ii  silversearcher-ag  2.2.0  amd64  pkg'
    _mock_dpkg
    run is_installed
    assert_success
}
...
@test "dry-run install writes nothing under the state dir" {
    ...
    INIT_UBUNTU_DRY_RUN=true run install
    assert_success
    [[ ! -e "${INIT_UBUNTU_STATE_DIR}/versions/ag" ]]
...
@test "install writes the sidecar with the dpkg-reported version" {
    _load_module
    _mock_apt_defaults
    MOCK_PKG_VERSION="2.2.0"
    _mock_dpkg_query
    module_standalone_main install
    [[ -f "${INIT_UBUNTU_STATE_DIR}/versions/ag" ]]
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/ag")" == "2.2.0" ]]
}
...
@test "remove deletes the sidecar" {
    ...
    printf '1.0.0\n' > "${INIT_UBUNTU_STATE_DIR}/versions/ag"
    _mock_apt_defaults
...
@test "doctor passes when the ag probe binary is available" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
    _fake_bin ag 'printf "ag version 2.2.0\n"'
    PATH="${INIT_UBUNTU_TEST_SCRATCH}/bin:${PATH}" run doctor
    assert_success
}

@test "doctor fails when the ag probe binary is missing even though installed" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
...
@test "is_outdated returns zero when apt reports the package upgradable" {
    _load_module
    MOCK_APT_UPGRADABLE='silversearcher-ag/noble-updates 2.2.0 amd64 [upgradable from: 2.1.0]'
    _mock_apt_list
    run is_outdated
    assert_success
}
