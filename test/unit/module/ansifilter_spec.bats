#!/usr/bin/env bats
# test/unit/module/ansifilter_spec.bats — module/ansifilter.module.sh
#
...
_load_module() {
    ...
    # shellcheck source=../../../module/ansifilter.module.sh
    source "${MODULE_DIR}/ansifilter.module.sh"
}

_standalone_module() {
    bash "${MODULE_DIR}/ansifilter.module.sh" "$@"
}
...
@test "ansifilter module file parses (bash -n)" {
    run bash -n "${MODULE_DIR}/ansifilter.module.sh"
    assert_success
}

@test "ansifilter module sources cleanly in engine mode (MODULE_STANDALONE=false)" {
    _load_module
    [[ "${MODULE_STANDALONE}" == "false" ]]
}

@test "ansifilter module defines all 10 lifecycle functions" {
    _load_module
    local _fn
...
@test "ansifilter module declares NAME=ansifilter" {
    _load_module
    [[ "${NAME}" == "ansifilter" ]]
}

@test "ansifilter module CATEGORY=optional" {
    _load_module
    [[ "${CATEGORY}" == "optional" ]]
}

@test "ansifilter module TAGS contains cli" {
    _load_module
    [[ " ${TAGS[*]} " == *" cli "* ]]
}

@test "ansifilter module DEPENDS_ON is empty" {
    _load_module
    [[ "${#DEPENDS_ON[@]}" -eq 0 ]]
}

@test "ansifilter DESCRIPTION is associative with en + zh-TW entries" {
    _load_module
...
@test "ansifilter module_get_description returns language-specific text + en fallback" {
    _load_module
    [[ -n "$(module_get_description en)" ]]
    [[ "$(module_get_description zh-TW)" == *"跳脫碼"* ]]
    [[ "$(module_get_description xx)" == "$(module_get_description en)" ]]
}

@test "ansifilter SUPPORTED_UBUNTU includes 22.04 24.04 26.04" {
    _load_module
...
@test "ansifilter module RISK_LEVEL=low and VERSION_PROVIDED=apt-managed" {
    _load_module
    [[ "${RISK_LEVEL}" == "low" ]]
    [[ "${VERSION_PROVIDED}" == "apt-managed" ]]
}

@test "ansifilter archetype data installs the ansifilter apt package" {
    _load_module
    [[ " ${APT_PKGS[*]} " == *" ansifilter "* ]]
    [[ "${#APT_PKGS[@]}" -eq 1 ]]
    [[ -z "${APT_PPA}" ]]
...
@test "is_installed returns zero when dpkg reports the package as ii" {
    _load_module
    MOCK_DPKG_OUTPUT='ii  ansifilter  2.18  amd64  pkg'
    _mock_dpkg
    run is_installed
    assert_success
}
...
@test "dry-run install writes nothing under the state dir" {
    ...
    INIT_UBUNTU_DRY_RUN=true run install
    assert_success
    [[ ! -e "${INIT_UBUNTU_STATE_DIR}/versions/ansifilter" ]]
...
@test "install writes the sidecar with the dpkg-reported version" {
    _load_module
    _mock_apt_defaults
    MOCK_PKG_VERSION="2.18"
    _mock_dpkg_query
    module_standalone_main install
    [[ -f "${INIT_UBUNTU_STATE_DIR}/versions/ansifilter" ]]
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/ansifilter")" == "2.18" ]]
}
...
@test "remove deletes the sidecar" {
    ...
    printf '1.0.0\n' > "${INIT_UBUNTU_STATE_DIR}/versions/ansifilter"
    _mock_apt_defaults
...
@test "doctor passes when the ansifilter probe binary is available" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
    _fake_bin ansifilter 'printf "ansifilter version 2.18\n"'
    PATH="${INIT_UBUNTU_TEST_SCRATCH}/bin:${PATH}" run doctor
    assert_success
}

@test "doctor fails when the ansifilter probe binary is missing even though installed" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
...
@test "is_outdated returns zero when apt reports the package upgradable" {
    _load_module
    MOCK_APT_UPGRADABLE='ansifilter/noble-updates 2.18 amd64 [upgradable from: 2.17]'
    _mock_apt_list
    run is_outdated
    assert_success
}
...
@test "standalone: --version prints NAME + VERSION_PROVIDED" {
    ...
    assert_success
    assert_output --partial "ansifilter"
}
...
@test "standalone: info prints metadata" {
    ...
    assert_success
    assert_output --partial "name:        ansifilter"
