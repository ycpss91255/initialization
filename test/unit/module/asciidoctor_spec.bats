#!/usr/bin/env bats
# test/unit/module/asciidoctor_spec.bats — module/asciidoctor.module.sh
#
# apt archetype-A module. Mirrors the tree archetype-A spec: smoke / metadata /
...
_load_module() {
    ...
    # shellcheck source=../../../module/asciidoctor.module.sh
    source "${MODULE_DIR}/asciidoctor.module.sh"
}

_standalone_module() {
    bash "${MODULE_DIR}/asciidoctor.module.sh" "$@"
}
...
@test "asciidoctor module file parses (bash -n)" {
    run bash -n "${MODULE_DIR}/asciidoctor.module.sh"
    assert_success
}

@test "asciidoctor module sources cleanly in engine mode (MODULE_STANDALONE=false)" {
    _load_module
    [[ "${MODULE_STANDALONE}" == "false" ]]
}

@test "asciidoctor module defines all 10 lifecycle functions" {
    _load_module
    local _fn
...
@test "asciidoctor module declares NAME=asciidoctor" {
    _load_module
    [[ "${NAME}" == "asciidoctor" ]]
}

@test "asciidoctor module CATEGORY=optional" {
    _load_module
    [[ "${CATEGORY}" == "optional" ]]
}

@test "asciidoctor module TAGS contains cli" {
    _load_module
    [[ " ${TAGS[*]} " == *" cli "* ]]
}

@test "asciidoctor module DEPENDS_ON is empty" {
    _load_module
    [[ "${#DEPENDS_ON[@]}" -eq 0 ]]
}

@test "asciidoctor DESCRIPTION is associative with en + zh-TW entries" {
    _load_module
...
@test "asciidoctor module_get_description returns language-specific text + en fallback" {
    _load_module
    [[ -n "$(module_get_description en)" ]]
    [[ "$(module_get_description zh-TW)" == *"文件處理器"* ]]
    [[ "$(module_get_description xx)" == "$(module_get_description en)" ]]
}

@test "asciidoctor SUPPORTED_UBUNTU includes 22.04 24.04 26.04" {
    _load_module
...
@test "asciidoctor module RISK_LEVEL=low and VERSION_PROVIDED=apt-managed" {
    _load_module
    [[ "${RISK_LEVEL}" == "low" ]]
    [[ "${VERSION_PROVIDED}" == "apt-managed" ]]
}

@test "asciidoctor archetype data installs the asciidoctor apt package" {
    _load_module
    [[ " ${APT_PKGS[*]} " == *" asciidoctor "* ]]
    [[ "${#APT_PKGS[@]}" -eq 1 ]]
    [[ -z "${APT_PPA}" ]]
}
...
@test "is_installed returns zero when dpkg reports the package as ii" {
    _load_module
    MOCK_DPKG_OUTPUT='ii  asciidoctor  2.0.18  all  pkg'
    _mock_dpkg
    run is_installed
    assert_success
}
...
@test "dry-run install writes nothing under the state dir" {
    ...
    INIT_UBUNTU_DRY_RUN=true run install
    assert_success
    [[ ! -e "${INIT_UBUNTU_STATE_DIR}/versions/asciidoctor" ]]
...
@test "install writes the sidecar with the dpkg-reported version" {
    _load_module
    _mock_apt_defaults
    MOCK_PKG_VERSION="2.0.18"
    _mock_dpkg_query
    module_standalone_main install
    [[ -f "${INIT_UBUNTU_STATE_DIR}/versions/asciidoctor" ]]
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/asciidoctor")" == "2.0.18" ]]
}
...
@test "remove deletes the sidecar" {
    ...
    printf '1.0.0\n' > "${INIT_UBUNTU_STATE_DIR}/versions/asciidoctor"
    _mock_apt_defaults
...
@test "doctor passes when the asciidoctor probe binary is available" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
    _fake_bin asciidoctor 'printf "Asciidoctor 2.0.18\n"'
    PATH="${INIT_UBUNTU_TEST_SCRATCH}/bin:${PATH}" run doctor
    assert_success
}

@test "doctor fails when the asciidoctor probe binary is missing even though installed" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
...
@test "is_outdated returns zero when apt reports the package upgradable" {
    _load_module
    MOCK_APT_UPGRADABLE='asciidoctor/noble-updates 2.0.18 all [upgradable from: 2.0.17]'
    _mock_apt_list
    run is_outdated
    assert_success
}
...
@test "standalone: info prints metadata" {
    ...
    assert_success
    assert_output --partial "name:        asciidoctor"
