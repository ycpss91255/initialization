#!/usr/bin/env bats
# test/unit/module/bmon_spec.bats — module/bmon.module.sh
#
...
_load_module() {
    ...
    # shellcheck source=../../../module/bmon.module.sh
    source "${MODULE_DIR}/bmon.module.sh"
}

_standalone_module() {
    bash "${MODULE_DIR}/bmon.module.sh" "$@"
}
...
@test "bmon module file parses (bash -n)" {
    run bash -n "${MODULE_DIR}/bmon.module.sh"
    assert_success
}

@test "bmon module sources cleanly in engine mode (MODULE_STANDALONE=false)" {
    _load_module
    [[ "${MODULE_STANDALONE}" == "false" ]]
}

@test "bmon module defines all 10 lifecycle functions (ADR-0002 / #305)" {
    _load_module
    local _fn
...
@test "bmon module declares NAME=bmon" {
    _load_module
    [[ "${NAME}" == "bmon" ]]
}

@test "bmon module CATEGORY=optional" {
    _load_module
    [[ "${CATEGORY}" == "optional" ]]
}

@test "bmon module TAGS contains monitoring" {
    _load_module
    [[ " ${TAGS[*]} " == *" monitoring "* ]]
}

@test "bmon module DEPENDS_ON is empty (standalone tool)" {
    _load_module
    [[ "${#DEPENDS_ON[@]}" -eq 0 ]]
}

@test "bmon DESCRIPTION is associative with en + zh-TW entries" {
    _load_module
...
@test "bmon module_get_description returns language-specific text + en fallback" {
    _load_module
...
@test "bmon SUPPORTED_UBUNTU includes 22.04 24.04 26.04" {
    _load_module
...
@test "bmon SUPPORTED_PLATFORMS covers all five form factors" {
    _load_module
    local _p
...
@test "bmon module RISK_LEVEL=low and VERSION_PROVIDED=apt-managed" {
    _load_module
    [[ "${RISK_LEVEL}" == "low" ]]
    [[ "${VERSION_PROVIDED}" == "apt-managed" ]]
}

@test "bmon archetype data installs the bmon apt package" {
    _load_module
    [[ " ${APT_PKGS[*]} " == *" bmon "* ]]
    [[ "${#APT_PKGS[@]}" -eq 1 ]]
    [[ -z "${APT_PPA}" ]]
...
@test "is_installed returns nonzero when dpkg does not report bmon" {
    _load_module
    MOCK_DPKG_RC=1
    _mock_dpkg
    run is_installed
...
@test "is_installed returns zero when dpkg reports bmon as ii" {
    _load_module
    MOCK_DPKG_OUTPUT='ii  bmon  99.9.9  amd64  pkg'
    _mock_dpkg
    run is_installed
    assert_success
}
...
@test "dry-run install writes nothing under the state dir" {
    ...
    INIT_UBUNTU_DRY_RUN=true run install
    assert_success
    [[ ! -e "${INIT_UBUNTU_STATE_DIR}/versions/bmon" ]]
...
@test "install writes the sidecar with the dpkg-reported version" {
    ...
    [[ -f "${INIT_UBUNTU_STATE_DIR}/versions/bmon" ]]
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/bmon")" == "1.2.3" ]]
}
...
@test "install sidecar falls back to apt-managed when dpkg-query is empty" {
    ...
    _mock_dpkg_query
    module_standalone_main install
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/bmon")" == "apt-managed" ]]
}
...
@test "upgrade refreshes the sidecar version" {
    ...
    printf '1.0.0\n' > "${INIT_UBUNTU_STATE_DIR}/versions/bmon"
    _mock_apt_defaults
    MOCK_PKG_VERSION="1.2.3"
    _mock_dpkg_query
...
@test "doctor passes when installed and the bmon binary runs" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
    _stub_bin bmon
...
@test "is_outdated returns zero when apt reports bmon upgradable" {
    _load_module
    MOCK_APT_UPGRADABLE='bmon/noble-updates 1.2.3 amd64 [upgradable from: 1.2.2]'
    _mock_apt_list
    run is_outdated
    assert_success
}

@test "is_outdated returns nonzero when bmon is not in the upgradable list" {
    _load_module
...
@test "standalone: info prints metadata" {
    ...
    assert_success
    assert_output --partial "name:        bmon"
