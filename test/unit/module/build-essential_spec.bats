#!/usr/bin/env bats
# test/unit/module/build-essential_spec.bats — module/build-essential.module.sh
#
# Base apt module split out of apt-essentials (ADR-0026). build-essential is a
# metapackage with no own binary, so verify/doctor probe `gcc` as the toolchain
# proxy. Otherwise mirrors the ripgrep archetype-A spec.
...
_load_module() {
    ...
    # shellcheck source=../../../module/build-essential.module.sh
    source "${MODULE_DIR}/build-essential.module.sh"
}

_standalone_module() {
    bash "${MODULE_DIR}/build-essential.module.sh" "$@"
}
...
@test "build-essential module file parses (bash -n)" {
    run bash -n "${MODULE_DIR}/build-essential.module.sh"
    assert_success
}

@test "build-essential module sources cleanly in engine mode" {
    _load_module
    [[ "${MODULE_STANDALONE}" == "false" ]]
}

@test "build-essential module defines all 10 lifecycle functions" {
    _load_module
    local _fn
...
@test "build-essential module declares NAME=build-essential" {
    _load_module
    [[ "${NAME}" == "build-essential" ]]
}

@test "build-essential module CATEGORY=base" {
    _load_module
    [[ "${CATEGORY}" == "base" ]]
}

@test "build-essential module TAGS contains build" {
    _load_module
    [[ " ${TAGS[*]} " == *" build "* ]]
}

@test "build-essential module DEPENDS_ON is empty (base tool)" {
    _load_module
    [[ "${#DEPENDS_ON[@]}" -eq 0 ]]
}

@test "build-essential DESCRIPTION is associative with en + zh-TW entries" {
    _load_module
...
@test "build-essential module_get_description returns text + en fallback" {
    _load_module
    [[ -n "$(module_get_description en)" ]]
    [[ "$(module_get_description zh-TW)" == *"編譯"* ]]
    [[ "$(module_get_description xx)" == "$(module_get_description en)" ]]
}

@test "build-essential SUPPORTED_UBUNTU includes 22.04 24.04 26.04" {
    _load_module
...
@test "build-essential SUPPORTED_PLATFORMS covers all five form factors" {
    _load_module
    local _p
...
@test "build-essential RISK_LEVEL=low and VERSION_PROVIDED=apt-managed" {
    _load_module
    [[ "${RISK_LEVEL}" == "low" ]]
    [[ "${VERSION_PROVIDED}" == "apt-managed" ]]
}

@test "build-essential archetype data installs the build-essential apt package" {
    _load_module
...
@test "is_installed returns nonzero when dpkg does not report build-essential" {
    _load_module
    MOCK_DPKG_RC=1
    _mock_dpkg
    run is_installed
...
@test "is_installed returns zero when dpkg reports build-essential as ii" {
    _load_module
    MOCK_DPKG_OUTPUT='ii  build-essential  12.9  amd64  metapackage'
    _mock_dpkg
    run is_installed
    assert_success
}
...
# ── verify / doctor / is_outdated (gcc proxy) ────────────────────────────────
...
@test "doctor passes when gcc answers --version (toolchain proxy)" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
    mkdir -p "${INIT_UBUNTU_TEST_SCRATCH}/bin"
    printf '#!/usr/bin/env bash\nprintf "gcc 13.2.0\\n"\n' \
        > "${INIT_UBUNTU_TEST_SCRATCH}/bin/gcc"
    chmod +x "${INIT_UBUNTU_TEST_SCRATCH}/bin/gcc"
...
@test "is_outdated returns zero when apt reports build-essential upgradable" {
    _load_module
    MOCK_APT_UPGRADABLE='build-essential/noble-updates 12.10 amd64 [upgradable from: 12.9]'
    _mock_apt_list
    run is_outdated
    assert_success
}

@test "is_outdated returns nonzero when build-essential is not upgradable" {
    _load_module
...
@test "standalone: info prints metadata" {
    ...
    assert_success
    assert_output --partial "name:        build-essential"
