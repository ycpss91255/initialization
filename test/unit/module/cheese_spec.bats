#!/usr/bin/env bats
# test/unit/module/cheese_spec.bats — module/cheese.module.sh
#
...
_load_module() {
    ...
    # shellcheck source=../../../module/cheese.module.sh
    source "${MODULE_DIR}/cheese.module.sh"
}

_standalone_module() {
    bash "${MODULE_DIR}/cheese.module.sh" "$@"
}
...
@test "cheese module file parses (bash -n)" {
    run bash -n "${MODULE_DIR}/cheese.module.sh"
    assert_success
}

@test "cheese module sources cleanly in engine mode (MODULE_STANDALONE=false)" {
    _load_module
    [[ "${MODULE_STANDALONE}" == "false" ]]
}

@test "cheese module defines all 10 lifecycle functions (ADR-0002 / #305)" {
    _load_module
    local _fn
...
@test "cheese module declares NAME=cheese" {
    _load_module
    [[ "${NAME}" == "cheese" ]]
}

@test "cheese module CATEGORY=optional" {
    _load_module
    [[ "${CATEGORY}" == "optional" ]]
}

@test "cheese module TAGS contains media" {
    _load_module
    [[ " ${TAGS[*]} " == *" media "* ]]
}

@test "cheese module DEPENDS_ON is empty (standalone tool)" {
    _load_module
    [[ "${#DEPENDS_ON[@]}" -eq 0 ]]
}

@test "cheese DESCRIPTION is associative with en + zh-TW entries" {
    _load_module
...
@test "cheese module_get_description returns language-specific text + en fallback" {
    _load_module
    [[ -n "$(module_get_description en)" ]]
    [[ "$(module_get_description zh-TW)" == *"攝影機"* ]]
    [[ "$(module_get_description xx)" == "$(module_get_description en)" ]]
}

@test "cheese SUPPORTED_UBUNTU includes 22.04 24.04 26.04" {
    _load_module
...
@test "cheese SUPPORTED_PLATFORMS is desktop-only" {
    _load_module
    [[ "${#SUPPORTED_PLATFORMS[@]}" -eq 1 ]]
    [[ "${SUPPORTED_PLATFORMS[0]}" == "desktop" ]]
}

@test "cheese module RISK_LEVEL=low and VERSION_PROVIDED=apt-managed" {
    _load_module
    [[ "${RISK_LEVEL}" == "low" ]]
    [[ "${VERSION_PROVIDED}" == "apt-managed" ]]
}

@test "cheese archetype data installs the cheese apt package" {
    _load_module
    [[ " ${APT_PKGS[*]} " == *" cheese "* ]]
    [[ "${#APT_PKGS[@]}" -eq 1 ]]
    [[ -z "${APT_PPA}" ]]
...
@test "cheese TEST_VERIFY_CMD probes the cheese binary" {
    _load_module
    [[ "${TEST_VERIFY_CMD}" == *"cheese"* ]]
}

# ── is_installed ─────────────────────────────────────────────────────────────

@test "is_installed returns nonzero when dpkg does not report cheese" {
    _load_module
    MOCK_DPKG_RC=1
    _mock_dpkg
    run is_installed
...
@test "is_installed returns zero when dpkg reports cheese as ii" {
    _load_module
    MOCK_DPKG_OUTPUT='ii  cheese  99.9.9  amd64  pkg'
    _mock_dpkg
    run is_installed
    assert_success
}
...
@test "dry-run install writes nothing under the state dir" {
    ...
    INIT_UBUNTU_DRY_RUN=true run install
    assert_success
    [[ ! -e "${INIT_UBUNTU_STATE_DIR}/versions/cheese" ]]
...
@test "install writes the sidecar with the dpkg-reported version" {
    ...
    [[ -f "${INIT_UBUNTU_STATE_DIR}/versions/cheese" ]]
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/cheese")" == "1.2.3" ]]
}
...
@test "install sidecar falls back to apt-managed when dpkg-query is empty" {
    ...
    _mock_dpkg_query
    module_standalone_main install
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/cheese")" == "apt-managed" ]]
}
...
@test "upgrade refreshes the sidecar version" {
    ...
    printf '1.0.0\n' > "${INIT_UBUNTU_STATE_DIR}/versions/cheese"
    _mock_apt_defaults
    MOCK_PKG_VERSION="1.2.3"
    _mock_dpkg_query
...
@test "doctor fails when installed but the cheese binary is not runnable" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
...
@test "doctor passes when installed and the cheese binary runs" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
    _stub_bin cheese
...
@test "is_outdated returns zero when apt reports cheese upgradable" {
    _load_module
    MOCK_APT_UPGRADABLE='cheese/noble-updates 7.94 amd64 [upgradable from: 7.93]'
    _mock_apt_list
    run is_outdated
    assert_success
}

@test "is_outdated returns nonzero when cheese is not in the upgradable list" {
    _load_module
...
@test "standalone: info prints metadata" {
    ...
    assert_success
    assert_output --partial "name:        cheese"
