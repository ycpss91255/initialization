#!/usr/bin/env bats
# test/unit/module/cmatrix_spec.bats — module/cmatrix.module.sh
#
# apt archetype-A module (novelty). Mirrors the tree archetype-A spec: smoke /
# metadata / lifecycle dry-run / no-side-fx / idempotency / standalone CLI /
# sidecar lifecycle (ADR-0001) + a REAL doctor() probe.
...
_load_module() {
    ...
    # shellcheck source=../../../module/cmatrix.module.sh
    source "${MODULE_DIR}/cmatrix.module.sh"
}

_standalone_module() {
    bash "${MODULE_DIR}/cmatrix.module.sh" "$@"
}
...
@test "cmatrix module file parses (bash -n)" {
    run bash -n "${MODULE_DIR}/cmatrix.module.sh"
    assert_success
}

@test "cmatrix module sources cleanly in engine mode (MODULE_STANDALONE=false)" {
    _load_module
    [[ "${MODULE_STANDALONE}" == "false" ]]
}

@test "cmatrix module defines all 10 lifecycle functions" {
    _load_module
    local _fn
...
@test "cmatrix module declares NAME=cmatrix" {
    _load_module
    [[ "${NAME}" == "cmatrix" ]]
}

@test "cmatrix module CATEGORY=optional" {
    _load_module
    [[ "${CATEGORY}" == "optional" ]]
}

@test "cmatrix module TAGS contains novelty" {
    _load_module
    [[ " ${TAGS[*]} " == *" novelty "* ]]
}

@test "cmatrix module DEPENDS_ON is empty" {
    _load_module
    [[ "${#DEPENDS_ON[@]}" -eq 0 ]]
}

@test "cmatrix DESCRIPTION is associative with en + zh-TW entries" {
    _load_module
...
@test "cmatrix module_get_description returns language-specific text + en fallback" {
    _load_module
    [[ -n "$(module_get_description en)" ]]
    [[ "$(module_get_description zh-TW)" == *"數字雨"* ]]
    [[ "$(module_get_description xx)" == "$(module_get_description en)" ]]
}

@test "cmatrix SUPPORTED_UBUNTU includes 22.04 24.04 26.04" {
    _load_module
...
@test "cmatrix module RISK_LEVEL=low and VERSION_PROVIDED=apt-managed" {
    _load_module
    [[ "${RISK_LEVEL}" == "low" ]]
    [[ "${VERSION_PROVIDED}" == "apt-managed" ]]
}

@test "cmatrix archetype data installs the cmatrix apt package" {
    _load_module
    [[ " ${APT_PKGS[*]} " == *" cmatrix "* ]]
    [[ "${#APT_PKGS[@]}" -eq 1 ]]
    [[ -z "${APT_PPA}" ]]
...
@test "is_installed returns nonzero when dpkg does not report cmatrix" {
    _load_module
    MOCK_DPKG_RC=1
    _mock_dpkg
    run is_installed
...
@test "is_installed returns zero when dpkg reports cmatrix as ii" {
    _load_module
    MOCK_DPKG_OUTPUT='ii  cmatrix  2.0-5  all  pkg'
    _mock_dpkg
    run is_installed
    assert_success
}
...
@test "dry-run install writes nothing under the state dir" {
    ...
    INIT_UBUNTU_DRY_RUN=true run install
    assert_success
    [[ ! -e "${INIT_UBUNTU_STATE_DIR}/versions/cmatrix" ]]
...
@test "install writes the sidecar with the dpkg-reported version" {
    _load_module
    _mock_apt_defaults
    MOCK_PKG_VERSION="2.0-5"
    _mock_dpkg_query
    module_standalone_main install
    [[ -f "${INIT_UBUNTU_STATE_DIR}/versions/cmatrix" ]]
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/cmatrix")" == "2.0-5" ]]
}
...
@test "remove deletes the sidecar" {
    ...
    printf '1.0.0\n' > "${INIT_UBUNTU_STATE_DIR}/versions/cmatrix"
    _mock_apt_defaults
...
@test "doctor passes when the cmatrix probe binary is available" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
    _fake_bin cmatrix 'printf "cmatrix digital rain\n"'
    PATH="${INIT_UBUNTU_TEST_SCRATCH}/bin:${PATH}" run doctor
    assert_success
}

@test "doctor fails when the cmatrix probe binary is missing even though installed" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
...
@test "is_outdated returns zero when apt reports cmatrix upgradable" {
    _load_module
    MOCK_APT_UPGRADABLE='cmatrix/noble-updates 2.0-5 all [upgradable from: 2.0-4]'
    _mock_apt_list
    run is_outdated
    assert_success
}

@test "is_outdated returns nonzero when cmatrix is not in the upgradable list" {
    _load_module
...
@test "standalone: info prints metadata" {
    ...
    assert_success
    assert_output --partial "name:        cmatrix"
