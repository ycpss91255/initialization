#!/usr/bin/env bats
# test/unit/module/cowsay_spec.bats — module/cowsay.module.sh
#
...
_load_module() {
    ...
    # shellcheck source=../../../module/cowsay.module.sh
    source "${MODULE_DIR}/cowsay.module.sh"
}

_standalone_module() {
    bash "${MODULE_DIR}/cowsay.module.sh" "$@"
}
...
@test "cowsay module file parses (bash -n)" {
    run bash -n "${MODULE_DIR}/cowsay.module.sh"
    assert_success
}

@test "cowsay module sources cleanly in engine mode (MODULE_STANDALONE=false)" {
    _load_module
    [[ "${MODULE_STANDALONE}" == "false" ]]
}

@test "cowsay module defines all 10 lifecycle functions" {
    _load_module
    local _fn
...
@test "cowsay module declares NAME=cowsay" {
    _load_module
    [[ "${NAME}" == "cowsay" ]]
}

@test "cowsay module CATEGORY=optional" {
    _load_module
    [[ "${CATEGORY}" == "optional" ]]
}

@test "cowsay module TAGS contains novelty" {
    _load_module
    [[ " ${TAGS[*]} " == *" novelty "* ]]
}

@test "cowsay module DEPENDS_ON is empty" {
    _load_module
    [[ "${#DEPENDS_ON[@]}" -eq 0 ]]
}

@test "cowsay DESCRIPTION is associative with en + zh-TW entries" {
    _load_module
...
@test "cowsay module_get_description returns language-specific text + en fallback" {
    _load_module
    [[ -n "$(module_get_description en)" ]]
    [[ "$(module_get_description zh-TW)" == *"母牛"* ]]
    [[ "$(module_get_description xx)" == "$(module_get_description en)" ]]
}

@test "cowsay SUPPORTED_UBUNTU includes 22.04 24.04 26.04" {
    _load_module
...
@test "cowsay module RISK_LEVEL=low and VERSION_PROVIDED=apt-managed" {
    _load_module
    [[ "${RISK_LEVEL}" == "low" ]]
    [[ "${VERSION_PROVIDED}" == "apt-managed" ]]
}

@test "cowsay archetype data installs the cowsay apt package" {
    _load_module
    [[ " ${APT_PKGS[*]} " == *" cowsay "* ]]
    [[ "${#APT_PKGS[@]}" -eq 1 ]]
    [[ -z "${APT_PPA}" ]]
...
@test "is_installed returns nonzero when dpkg does not report cowsay" {
    _load_module
    MOCK_DPKG_RC=1
    _mock_dpkg
    run is_installed
...
@test "is_installed returns zero when dpkg reports cowsay as ii" {
    _load_module
    MOCK_DPKG_OUTPUT='ii  cowsay  3.03+dfsg2  all  pkg'
    _mock_dpkg
    run is_installed
    assert_success
}
...
@test "dry-run install writes nothing under the state dir" {
    ...
    INIT_UBUNTU_DRY_RUN=true run install
    assert_success
    [[ ! -e "${INIT_UBUNTU_STATE_DIR}/versions/cowsay" ]]
...
@test "install writes the sidecar with the dpkg-reported version" {
    _load_module
    _mock_apt_defaults
    MOCK_PKG_VERSION="3.03+dfsg2"
    _mock_dpkg_query
    module_standalone_main install
    [[ -f "${INIT_UBUNTU_STATE_DIR}/versions/cowsay" ]]
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/cowsay")" == "3.03+dfsg2" ]]
}
...
@test "remove deletes the sidecar" {
    ...
    printf '1.0.0\n' > "${INIT_UBUNTU_STATE_DIR}/versions/cowsay"
    _mock_apt_defaults
...
@test "is_outdated returns zero when apt reports cowsay upgradable" {
    _load_module
    MOCK_APT_UPGRADABLE='cowsay/noble-updates 3.03+dfsg2 all [upgradable from: 3.03+dfsg1]'
    _mock_apt_list
    run is_outdated
    assert_success
}

@test "is_outdated returns nonzero when cowsay is not in the upgradable list" {
    _load_module
...
@test "standalone: info prints metadata" {
    ...
    assert_success
    assert_output --partial "name:        cowsay"
