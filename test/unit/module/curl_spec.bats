#!/usr/bin/env bats
# test/unit/module/curl_spec.bats — module/curl.module.sh
#
...
_load_module() {
    ...
    # shellcheck source=../../../module/curl.module.sh
    source "${MODULE_DIR}/curl.module.sh"
}

_standalone_module() {
    bash "${MODULE_DIR}/curl.module.sh" "$@"
}
...
@test "curl module file parses (bash -n)" {
    run bash -n "${MODULE_DIR}/curl.module.sh"
    assert_success
}

@test "curl module sources cleanly in engine mode (MODULE_STANDALONE=false)" {
    _load_module
    [[ "${MODULE_STANDALONE}" == "false" ]]
}

@test "curl module defines all 10 lifecycle functions" {
    _load_module
    local _fn
...
@test "curl module declares NAME=curl" {
    _load_module
    [[ "${NAME}" == "curl" ]]
}

@test "curl module CATEGORY=base" {
    _load_module
    [[ "${CATEGORY}" == "base" ]]
}

@test "curl module TAGS contains http" {
    _load_module
    [[ " ${TAGS[*]} " == *" http "* ]]
}

@test "curl module DEPENDS_ON is empty (base tool)" {
    _load_module
    [[ "${#DEPENDS_ON[@]}" -eq 0 ]]
}

@test "curl DESCRIPTION is associative with en + zh-TW entries" {
    _load_module
...
@test "curl module_get_description returns language-specific text + en fallback" {
    _load_module
    [[ -n "$(module_get_description en)" ]]
    [[ "$(module_get_description zh-TW)" == *"傳輸"* ]]
    [[ "$(module_get_description xx)" == "$(module_get_description en)" ]]
}

@test "curl SUPPORTED_UBUNTU includes 22.04 24.04 26.04" {
    _load_module
...
@test "curl SUPPORTED_PLATFORMS covers all five form factors" {
    _load_module
    local _p
...
@test "curl module RISK_LEVEL=low and VERSION_PROVIDED=apt-managed" {
    _load_module
    [[ "${RISK_LEVEL}" == "low" ]]
    [[ "${VERSION_PROVIDED}" == "apt-managed" ]]
}

@test "curl archetype data installs the curl apt package" {
    _load_module
    [[ " ${APT_PKGS[*]} " == *" curl "* ]]
    [[ "${#APT_PKGS[@]}" -eq 1 ]]
    [[ -z "${APT_PPA}" ]]
...
@test "is_installed returns nonzero when dpkg does not report curl" {
    _load_module
    MOCK_DPKG_RC=1
    _mock_dpkg
    run is_installed
...
@test "is_installed returns zero when dpkg reports curl as ii" {
    _load_module
    MOCK_DPKG_OUTPUT='ii  curl  99.9.9  amd64  pkg'
    _mock_dpkg
    run is_installed
    assert_success
}
...
@test "dry-run install writes nothing under the state dir" {
    ...
    INIT_UBUNTU_DRY_RUN=true run install
    assert_success
    [[ ! -e "${INIT_UBUNTU_STATE_DIR}/versions/curl" ]]
...
@test "install writes the sidecar with the dpkg-reported version" {
    ...
    [[ -f "${INIT_UBUNTU_STATE_DIR}/versions/curl" ]]
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/curl")" == "1.2.3" ]]
}
...
@test "install sidecar falls back to apt-managed when dpkg-query is empty" {
    ...
    _mock_dpkg_query
    module_standalone_main install
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/curl")" == "apt-managed" ]]
}
...
@test "upgrade refreshes the sidecar version" {
    ...
    printf '1.0.0\n' > "${INIT_UBUNTU_STATE_DIR}/versions/curl"
    _mock_apt_defaults
    MOCK_PKG_VERSION="1.2.3"
    _mock_dpkg_query
...
@test "doctor passes when the curl binary answers --version" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
    mkdir -p "${INIT_UBUNTU_TEST_SCRATCH}/bin"
    printf '#!/usr/bin/env bash\nprintf "curl 1.2.3\n"\n' \
        > "${INIT_UBUNTU_TEST_SCRATCH}/bin/curl"
    chmod +x "${INIT_UBUNTU_TEST_SCRATCH}/bin/curl"
    PATH="${INIT_UBUNTU_TEST_SCRATCH}/bin:${PATH}" run doctor
    assert_success
}

@test "is_outdated returns zero when apt reports curl upgradable" {
    _load_module
    MOCK_APT_UPGRADABLE='curl/noble-updates 1.2.3 amd64 [upgradable from: 1.2.2]'
    _mock_apt_list
    run is_outdated
    assert_success
}

@test "is_outdated returns nonzero when curl is not in the upgradable list" {
    _load_module
