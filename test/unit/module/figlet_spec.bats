#!/usr/bin/env bats
# test/unit/module/figlet_spec.bats — module/figlet.module.sh
#
...
_load_module() {
    ...
    # shellcheck source=../../../module/figlet.module.sh
    source "${MODULE_DIR}/figlet.module.sh"
}

_standalone_module() {
    bash "${MODULE_DIR}/figlet.module.sh" "$@"
}
...
@test "figlet module file parses (bash -n)" {
    run bash -n "${MODULE_DIR}/figlet.module.sh"
    assert_success
}

@test "figlet module sources cleanly in engine mode (MODULE_STANDALONE=false)" {
    _load_module
    [[ "${MODULE_STANDALONE}" == "false" ]]
}

@test "figlet module defines all 10 lifecycle functions" {
    _load_module
    local _fn
...
@test "figlet module declares NAME=figlet" {
    _load_module
    [[ "${NAME}" == "figlet" ]]
}

@test "figlet module CATEGORY=optional" {
    _load_module
    [[ "${CATEGORY}" == "optional" ]]
}

@test "figlet module TAGS contains novelty" {
    _load_module
    [[ " ${TAGS[*]} " == *" novelty "* ]]
}

@test "figlet module DEPENDS_ON is empty" {
    _load_module
    [[ "${#DEPENDS_ON[@]}" -eq 0 ]]
}

@test "figlet DESCRIPTION is associative with en + zh-TW entries" {
    _load_module
...
@test "figlet module_get_description returns language-specific text + en fallback" {
    _load_module
    [[ -n "$(module_get_description en)" ]]
    [[ "$(module_get_description zh-TW)" == *"藝術字"* ]]
    [[ "$(module_get_description xx)" == "$(module_get_description en)" ]]
}

@test "figlet SUPPORTED_UBUNTU includes 22.04 24.04 26.04" {
    _load_module
...
@test "figlet module RISK_LEVEL=low and VERSION_PROVIDED=apt-managed" {
    _load_module
    [[ "${RISK_LEVEL}" == "low" ]]
    [[ "${VERSION_PROVIDED}" == "apt-managed" ]]
}

@test "figlet archetype data installs the figlet apt package" {
    _load_module
    [[ " ${APT_PKGS[*]} " == *" figlet "* ]]
    [[ "${#APT_PKGS[@]}" -eq 1 ]]
    [[ -z "${APT_PPA}" ]]
...
@test "is_installed returns nonzero when dpkg does not report figlet" {
    _load_module
    MOCK_DPKG_RC=1
    _mock_dpkg
    run is_installed
...
@test "is_installed returns zero when dpkg reports figlet as ii" {
    _load_module
    MOCK_DPKG_OUTPUT='ii  figlet  2.2.5-3  all  pkg'
    _mock_dpkg
    run is_installed
    assert_success
}
...
@test "dry-run install writes nothing under the state dir" {
    ...
    INIT_UBUNTU_DRY_RUN=true run install
    assert_success
    [[ ! -e "${INIT_UBUNTU_STATE_DIR}/versions/figlet" ]]
...
@test "install writes the sidecar with the dpkg-reported version" {
    _load_module
    _mock_apt_defaults
    MOCK_PKG_VERSION="2.2.5-3"
    _mock_dpkg_query
    module_standalone_main install
    [[ -f "${INIT_UBUNTU_STATE_DIR}/versions/figlet" ]]
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/figlet")" == "2.2.5-3" ]]
}
...
@test "remove deletes the sidecar" {
    ...
    printf '1.0.0\n' > "${INIT_UBUNTU_STATE_DIR}/versions/figlet"
    _mock_apt_defaults
...
@test "doctor passes when the figlet probe binary is available" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
    _fake_bin figlet 'printf "figlet banner\n"'
    PATH="${INIT_UBUNTU_TEST_SCRATCH}/bin:${PATH}" run doctor
    assert_success
}

@test "doctor fails when the figlet probe binary is missing even though installed" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
...
@test "is_outdated returns zero when apt reports figlet upgradable" {
    _load_module
    MOCK_APT_UPGRADABLE='figlet/noble-updates 2.2.5-3 all [upgradable from: 2.2.5-2]'
    _mock_apt_list
    run is_outdated
    assert_success
}

@test "is_outdated returns nonzero when figlet is not in the upgradable list" {
    _load_module
...
@test "standalone: info prints metadata" {
    ...
    assert_success
    assert_output --partial "name:        figlet"
