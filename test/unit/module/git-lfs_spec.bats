#!/usr/bin/env bats
# test/unit/module/git-lfs_spec.bats — module/git-lfs.module.sh
#
# apt archetype-A module with an overridden install() that runs the one-time
# `git lfs install` post-step. Mirrors the htop archetype-A spec (smoke /
# metadata / lifecycle dry-run / no-side-fx / idempotency / standalone CLI /
# sidecar lifecycle, ADR-0001) + a REAL doctor() probe (`git lfs version`) and
# a post-install registration test.
...
setup() {
    ...
    # Point HOME at a scratch dir so any git config write never touches the
    # real container HOME.
...
_load_module() {
    ...
    # shellcheck source=../../../module/git-lfs.module.sh
    source "${MODULE_DIR}/git-lfs.module.sh"
}

_standalone_module() {
    bash "${MODULE_DIR}/git-lfs.module.sh" "$@"
}
...
# Neutralize the git-lfs registration post-step (keeps install() side-effect
# free while still exercising the archetype apt path + sidecar wiring).
_neutralize_post() {
    _git_lfs_register_filters() { printf 'register-filters-run\n'; }
}
...
@test "git-lfs module file parses (bash -n)" {
    run bash -n "${MODULE_DIR}/git-lfs.module.sh"
    assert_success
}

@test "git-lfs module sources cleanly in engine mode (MODULE_STANDALONE=false)" {
    _load_module
    [[ "${MODULE_STANDALONE}" == "false" ]]
}

@test "git-lfs module defines all 10 lifecycle functions" {
    _load_module
    local _fn
...
@test "git-lfs module declares NAME=git-lfs" {
    _load_module
    [[ "${NAME}" == "git-lfs" ]]
}

@test "git-lfs module CATEGORY=optional" {
    _load_module
    [[ "${CATEGORY}" == "optional" ]]
}

@test "git-lfs module TAGS contains git" {
    _load_module
    [[ " ${TAGS[*]} " == *" git "* ]]
}

@test "git-lfs module DEPENDS_ON contains git" {
    _load_module
    [[ " ${DEPENDS_ON[*]} " == *" git "* ]]
}

@test "git-lfs DESCRIPTION is associative with en + zh-TW entries" {
    _load_module
...
@test "git-lfs module_get_description returns language-specific text + en fallback" {
    _load_module
    [[ -n "$(module_get_description en)" ]]
    [[ "$(module_get_description zh-TW)" == *"大型檔案"* ]]
    [[ "$(module_get_description xx)" == "$(module_get_description en)" ]]
}

@test "git-lfs POST_INSTALL_MESSAGE has en + zh-TW entries" {
    _load_module
    [[ -n "$(module_get_post_install_message en)" ]]
    [[ -n "$(module_get_post_install_message zh-TW)" ]]
}

@test "git-lfs SUPPORTED_UBUNTU includes 22.04 24.04 26.04" {
    _load_module
...
@test "git-lfs module RISK_LEVEL=low and VERSION_PROVIDED=apt-managed" {
    _load_module
    [[ "${RISK_LEVEL}" == "low" ]]
    [[ "${VERSION_PROVIDED}" == "apt-managed" ]]
}

@test "git-lfs archetype data installs the git-lfs apt package" {
    _load_module
    [[ " ${APT_PKGS[*]} " == *" git-lfs "* ]]
    [[ "${#APT_PKGS[@]}" -eq 1 ]]
    [[ -z "${APT_PPA}" ]]
...
@test "is_installed returns nonzero when dpkg does not report git-lfs" {
    _load_module
    MOCK_DPKG_RC=1
    _mock_dpkg
    run is_installed
...
@test "is_installed returns zero when dpkg reports git-lfs as ii" {
    _load_module
    MOCK_DPKG_OUTPUT='ii  git-lfs  3.4.1  amd64  pkg'
    _mock_dpkg
    run is_installed
    assert_success
}
...
@test "install dry-run does not run the git lfs registration step" {
    _load_module
    # A fake git that records any invocation; the dry-run path must not call it.
    _fake_bin git "printf 'git-called\n' >> '${INIT_UBUNTU_TEST_SCRATCH}/git.log'"
    PATH="${INIT_UBUNTU_TEST_SCRATCH}/bin:${PATH}" INIT_UBUNTU_DRY_RUN=true run install
    assert_success
    [[ ! -e "${INIT_UBUNTU_TEST_SCRATCH}/git.log" ]]
}
...
# ── install post-step (git lfs install) ──────────────────────────────────────

@test "install runs 'git lfs install' as the apt post-step" {
    _load_module
    _mock_apt_defaults
    MOCK_PKG_VERSION="3.4.1"
    _mock_dpkg_query
    _fake_bin git "printf '%s\n' \"\$*\" >> '${INIT_UBUNTU_TEST_SCRATCH}/git.log'"
    PATH="${INIT_UBUNTU_TEST_SCRATCH}/bin:${PATH}" run install
    assert_success
    grep -q 'lfs install' "${INIT_UBUNTU_TEST_SCRATCH}/git.log"
}

@test "install still succeeds when git is not on PATH (warns, no abort)" {
    _load_module
    _mock_apt_defaults
    MOCK_PKG_VERSION="3.4.1"
    _mock_dpkg_query
    PATH="${INIT_UBUNTU_TEST_SCRATCH}/empty" run install
    assert_success
}
...
@test "install writes the sidecar with the dpkg-reported version" {
    ...
    [[ -f "${INIT_UBUNTU_STATE_DIR}/versions/git-lfs" ]]
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/git-lfs")" == "3.4.1" ]]
}
...
@test "remove deletes the sidecar" {
    ...
    printf '1.0.0\n' > "${INIT_UBUNTU_STATE_DIR}/versions/git-lfs"
    _mock_apt_defaults
...
@test "doctor passes when 'git lfs version' succeeds" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
    _fake_bin git 'exit 0'
    PATH="${INIT_UBUNTU_TEST_SCRATCH}/bin:${PATH}" run doctor
    assert_success
}

@test "doctor fails when git is missing even though installed" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
...
@test "is_outdated returns zero when apt reports git-lfs upgradable" {
    _load_module
    MOCK_APT_UPGRADABLE='git-lfs/noble-updates 3.4.2 amd64 [upgradable from: 3.4.1]'
    _mock_apt_list
    run is_outdated
    assert_success
}

@test "is_outdated returns nonzero when git-lfs is not in the upgradable list" {
    _load_module
...
@test "standalone: info prints metadata" {
    ...
    assert_success
    assert_output --partial "name:        git-lfs"
