#!/usr/bin/env bats
# test/unit/module/git_spec.bats — module/git.module.sh
#
...
_load_module() {
    ...
    # shellcheck source=../../../module/git.module.sh
    source "${MODULE_DIR}/git.module.sh"
}

_standalone_module() {
    bash "${MODULE_DIR}/git.module.sh" "$@"
}
...
@test "git module file parses (bash -n)" {
    run bash -n "${MODULE_DIR}/git.module.sh"
    assert_success
}

@test "git module sources cleanly in engine mode (MODULE_STANDALONE=false)" {
    _load_module
    [[ "${MODULE_STANDALONE}" == "false" ]]
}

@test "git module defines all 10 lifecycle functions" {
    _load_module
    local _fn
...
@test "git module declares NAME=git" {
    _load_module
    [[ "${NAME}" == "git" ]]
}

@test "git module CATEGORY=base" {
    _load_module
    [[ "${CATEGORY}" == "base" ]]
}

@test "git module TAGS contains vcs" {
    _load_module
    [[ " ${TAGS[*]} " == *" vcs "* ]]
}

@test "git module DEPENDS_ON is empty (base tool)" {
    _load_module
    [[ "${#DEPENDS_ON[@]}" -eq 0 ]]
}

@test "git DESCRIPTION is associative with en + zh-TW entries" {
    _load_module
...
@test "git module_get_description returns language-specific text + en fallback" {
    _load_module
    [[ -n "$(module_get_description en)" ]]
    [[ "$(module_get_description zh-TW)" == *"版本控制"* ]]
    [[ "$(module_get_description xx)" == "$(module_get_description en)" ]]
}

@test "git SUPPORTED_UBUNTU includes 22.04 24.04 26.04" {
    _load_module
...
@test "git SUPPORTED_PLATFORMS covers all five form factors" {
    _load_module
    local _p
...
@test "git module RISK_LEVEL=low and VERSION_PROVIDED=apt-managed" {
    _load_module
    [[ "${RISK_LEVEL}" == "low" ]]
    [[ "${VERSION_PROVIDED}" == "apt-managed" ]]
}

@test "git archetype data installs the git apt package" {
    _load_module
    [[ " ${APT_PKGS[*]} " == *" git "* ]]
    [[ "${#APT_PKGS[@]}" -eq 1 ]]
    [[ -z "${APT_PPA}" ]]
...
@test "is_installed returns nonzero when dpkg does not report git" {
    _load_module
    MOCK_DPKG_RC=1
    _mock_dpkg
    run is_installed
...
@test "is_installed returns zero when dpkg reports git as ii" {
    _load_module
    MOCK_DPKG_OUTPUT='ii  git  99.9.9  amd64  pkg'
    _mock_dpkg
    run is_installed
    assert_success
}
...
@test "dry-run install writes nothing under the state dir" {
    ...
    INIT_UBUNTU_DRY_RUN=true run install
    assert_success
    [[ ! -e "${INIT_UBUNTU_STATE_DIR}/versions/git" ]]
...
@test "install writes the sidecar with the dpkg-reported version" {
    ...
    _mock_dpkg_query
    install
    [[ -f "${INIT_UBUNTU_STATE_DIR}/versions/git" ]]
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/git")" == "1.2.3" ]]
}
...
@test "install sidecar falls back to apt-managed when dpkg-query is empty" {
    ...
    _mock_dpkg_query
    install
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/git")" == "apt-managed" ]]
}
...
@test "upgrade refreshes the sidecar version" {
    ...
    printf '1.0.0\n' > "${INIT_UBUNTU_STATE_DIR}/versions/git"
    _mock_apt_defaults
    MOCK_PKG_VERSION="1.2.3"
    _mock_dpkg_query
    upgrade
...
@test "doctor passes when the git binary answers --version" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
    mkdir -p "${INIT_UBUNTU_TEST_SCRATCH}/bin"
    printf '#!/usr/bin/env bash\nprintf "git 1.2.3\n"\n' \
        > "${INIT_UBUNTU_TEST_SCRATCH}/bin/git"
    chmod +x "${INIT_UBUNTU_TEST_SCRATCH}/bin/git"
    PATH="${INIT_UBUNTU_TEST_SCRATCH}/bin:${PATH}" run doctor
    assert_success
}

@test "is_outdated returns zero when apt reports git upgradable" {
    _load_module
    MOCK_APT_UPGRADABLE='git/noble-updates 1.2.3 amd64 [upgradable from: 1.2.2]'
    _mock_apt_list
    run is_outdated
    assert_success
}

@test "is_outdated returns nonzero when git is not in the upgradable list" {
    _load_module
...
@test "standalone: info prints metadata" {
    ...
    assert_success
    assert_output --partial "name:        git"
