#!/usr/bin/env bats
# test/unit/module/glow_spec.bats — module/glow.module.sh (issue #314)
#
# Coverage per PRD Q29: smoke / metadata / lifecycle presence / dry-run /
# no-side-fx / sidecar (ADR-0001) / idempotency / is_outdated (mocked,
# Q46 zero-network) / doctor / standalone CLI (AC-25). glow is the
# yazi markdown-preview dependency; this module makes it installable.
...
_load_module() {
    ...
    # shellcheck source=../../../module/glow.module.sh
    source "${MODULE_DIR}/glow.module.sh"
}

# Point all mutable paths into the per-test scratch dir so non-dry-run
# lifecycle runs never touch the real filesystem.
_sandbox_paths() {
    INSTALL_DIR="${INIT_UBUNTU_TEST_SCRATCH}/opt/glow"
    BIN_LINK="${INIT_UBUNTU_TEST_SCRATCH}/bin/glow"
    USE_SUDO=false
    CONFIG_PATHS=("${INIT_UBUNTU_TEST_SCRATCH}/home/.config/glow")
    mkdir -p "${INIT_UBUNTU_TEST_SCRATCH}/bin"
}

_sidecar_file() {
    printf '%s/versions/glow' "${INIT_UBUNTU_STATE_DIR}"
}
...
_mock_remote() {
    ...
    _module_github_release_fetch_and_install() {
        ...
        ln -sfn "${INSTALL_DIR}/glow" "${BIN_LINK}"
    }
}

# ── Smoke ────────────────────────────────────────────────────────────────────

@test "glow module file exists" {
    [[ -f "${MODULE_DIR}/glow.module.sh" ]]
}

@test "glow module parses (bash -n)" {
    bash -n "${MODULE_DIR}/glow.module.sh"
}
...
@test "NAME=glow matches the filename stem" {
    _load_module
    [[ "${NAME}" == "glow" ]]
}
...
@test "DEPENDS_ON is exactly curl (Q39)" {
    _load_module
    [[ "${#DEPENDS_ON[@]}" -eq 1 ]]
    [[ " ${DEPENDS_ON[*]} " == *" curl "* ]]
}
...
@test "archetype data: wrapped tarball -> STRIP_COMPONENTS=1 and BIN_PATH_IN_TAR=glow" {
    _load_module
    [[ "${STRIP_COMPONENTS}" == "1" ]]
...
@test "glow module defines all 10 lifecycle functions" {
    _load_module
    local _fn
...
@test "is_outdated returns 0 when a newer release exists" {
    ...
    _mock_remote "1.5.0"
    module_standalone_main install
    eval 'get_github_pkg_latest_version() { local -n _out="${1}"; _out="2.0.0"; }'
    run is_outdated
    assert_success
}
...
@test "registry discovers glow under --tag=cli-essentials" {
    # shellcheck source=../../../lib/logger.sh
    source "${LIB_DIR}/logger.sh"
...
_standalone() {
    ...
    HOME="${_home}" XDG_STATE_HOME="" INIT_UBUNTU_STATE_DIR="" \
        run bash "${MODULE_DIR}/glow.module.sh" "$@"
}
...
@test "standalone info prints metadata" {
    _standalone info
    assert_success
    assert_output --partial "name:        glow"
...
@test "standalone --dry-run install leaves a fresh HOME empty" {
    ...
    mkdir -p "${_home}"
    HOME="${_home}" XDG_STATE_HOME="" INIT_UBUNTU_STATE_DIR="" \
        run bash "${MODULE_DIR}/glow.module.sh" install --dry-run
    assert_success
