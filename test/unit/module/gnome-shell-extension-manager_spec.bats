#!/usr/bin/env bats
# test/unit/module/gnome-shell-extension-manager_spec.bats — module/gnome-shell-extension-manager.module.sh
#
# Desktop-only apt-archetype tool split into its own module (small-tools
# modularization program). Mirrors the nmap archetype-A spec: smoke / metadata
# / lifecycle dry-run / no-side-fx / idempotency / standalone CLI / sidecar
# lifecycle (ADR-0001) / desktop-only is_recommended gate / real doctor probe.
...
_load_module() {
    ...
    # shellcheck source=../../../module/gnome-shell-extension-manager.module.sh
    source "${MODULE_DIR}/gnome-shell-extension-manager.module.sh"
}

_standalone_module() {
    bash "${MODULE_DIR}/gnome-shell-extension-manager.module.sh" "$@"
}
...
_stub_bin() {
    ...
    printf '#!/usr/bin/env bash\nprintf "%s stub 0.0.0\n"\nexit 0\n' "${1}"         > "${INIT_UBUNTU_TEST_SCRATCH}/bin/${1}"
    chmod +x "${INIT_UBUNTU_TEST_SCRATCH}/bin/${1}"
}

# ── Smoke ────────────────────────────────────────────────────────────────────

@test "gnome-shell-extension-manager module file parses (bash -n)" {
    run bash -n "${MODULE_DIR}/gnome-shell-extension-manager.module.sh"
    assert_success
}

@test "gnome-shell-extension-manager module sources cleanly in engine mode (MODULE_STANDALONE=false)" {
    _load_module
    [[ "${MODULE_STANDALONE}" == "false" ]]
}

@test "gnome-shell-extension-manager module defines all 10 lifecycle functions (ADR-0002 / #305)" {
    _load_module
    local _fn
    for _fn in detect is_recommended is_installed install upgrade                remove purge verify is_outdated doctor; do
...
@test "gnome-shell-extension-manager module declares NAME=gnome-shell-extension-manager" {
    _load_module
    [[ "${NAME}" == "gnome-shell-extension-manager" ]]
}

@test "gnome-shell-extension-manager module CATEGORY=optional" {
    _load_module
    [[ "${CATEGORY}" == "optional" ]]
}

@test "gnome-shell-extension-manager module TAGS contains gnome" {
    _load_module
    [[ " ${TAGS[*]} " == *" gnome "* ]]
}

@test "gnome-shell-extension-manager module DEPENDS_ON is empty (standalone tool)" {
    _load_module
    [[ "${#DEPENDS_ON[@]}" -eq 0 ]]
}

@test "gnome-shell-extension-manager DESCRIPTION is associative with en + zh-TW entries" {
    _load_module
...
@test "gnome-shell-extension-manager module_get_description returns language-specific text + en fallback" {
    _load_module
    [[ -n "$(module_get_description en)" ]]
    [[ "$(module_get_description zh-TW)" == *"擴充功能"* ]]
    [[ "$(module_get_description xx)" == "$(module_get_description en)" ]]
}

@test "gnome-shell-extension-manager SUPPORTED_UBUNTU includes 22.04 24.04 26.04" {
    _load_module
...
@test "gnome-shell-extension-manager SUPPORTED_PLATFORMS is desktop-only" {
    _load_module
    [[ "${#SUPPORTED_PLATFORMS[@]}" -eq 1 ]]
    [[ "${SUPPORTED_PLATFORMS[0]}" == "desktop" ]]
}

@test "gnome-shell-extension-manager module RISK_LEVEL=low and VERSION_PROVIDED=apt-managed" {
    _load_module
    [[ "${RISK_LEVEL}" == "low" ]]
    [[ "${VERSION_PROVIDED}" == "apt-managed" ]]
}

@test "gnome-shell-extension-manager archetype data installs the gnome-shell-extension-manager apt package" {
    _load_module
...
@test "gnome-shell-extension-manager TEST_VERIFY_CMD probes the extension-manager binary" {
    _load_module
    [[ "${TEST_VERIFY_CMD}" == *"extension-manager"* ]]
}

# ── is_installed ─────────────────────────────────────────────────────────────

@test "is_installed returns nonzero when dpkg does not report gnome-shell-extension-manager" {
    _load_module
    MOCK_DPKG_RC=1
    _mock_dpkg
    run is_installed
...
@test "is_installed returns zero when dpkg reports gnome-shell-extension-manager as ii" {
    _load_module
    MOCK_DPKG_OUTPUT='ii  gnome-shell-extension-manager  99.9.9  amd64  pkg'
    _mock_dpkg
    run is_installed
    assert_success
}
...
@test "dry-run install writes nothing under the state dir" {
    ...
    INIT_UBUNTU_DRY_RUN=true run install
    assert_success
    [[ ! -e "${INIT_UBUNTU_STATE_DIR}/versions/gnome-shell-extension-manager" ]]
...
@test "standalone dry-run install creates no files in a scratch HOME" {
    ...
    mkdir -p "${_home}"
    HOME="${_home}" XDG_STATE_HOME="${_home}/.local/state"         run _standalone_module install --dry-run
    assert_success
    local _leftover
...
@test "install writes the sidecar with the dpkg-reported version" {
    ...
    [[ -f "${INIT_UBUNTU_STATE_DIR}/versions/gnome-shell-extension-manager" ]]
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/gnome-shell-extension-manager")" == "1.2.3" ]]
}
...
@test "install sidecar falls back to apt-managed when dpkg-query is empty" {
    ...
    _mock_dpkg_query
    module_standalone_main install
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/gnome-shell-extension-manager")" == "apt-managed" ]]
}
...
@test "upgrade refreshes the sidecar version" {
    ...
    printf '1.0.0\n' > "${INIT_UBUNTU_STATE_DIR}/versions/gnome-shell-extension-manager"
    _mock_apt_defaults
    MOCK_PKG_VERSION="1.2.3"
    _mock_dpkg_query
...
@test "doctor fails when installed but the extension-manager binary is not runnable" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
...
@test "doctor passes when installed and the extension-manager binary runs" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
    _stub_bin extension-manager
    PATH="${INIT_UBUNTU_TEST_SCRATCH}/bin:${PATH}" run doctor
    assert_success
}

@test "is_outdated returns zero when apt reports gnome-shell-extension-manager upgradable" {
    _load_module
    MOCK_APT_UPGRADABLE='gnome-shell-extension-manager/noble-updates 7.94 amd64 [upgradable from: 7.93]'
    _mock_apt_list
    run is_outdated
    assert_success
}

@test "is_outdated returns nonzero when gnome-shell-extension-manager is not in the upgradable list" {
    _load_module
...
# ── is_recommended (desktop-only gate, module-spec §4.3.1) ───────────────────
...
@test "standalone: --version prints NAME + VERSION_PROVIDED" {
    ...
    assert_success
    assert_output --partial "gnome-shell-extension-manager"
}
...
@test "standalone: info prints metadata" {
    ...
    assert_success
    assert_output --partial "name:        gnome-shell-extension-manager"
