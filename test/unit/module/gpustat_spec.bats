#!/usr/bin/env bats
# test/unit/module/gpustat_spec.bats — module/gpustat.module.sh
#
...
_load_module() {
    ...
    # shellcheck source=../../../module/gpustat.module.sh
    source "${MODULE_DIR}/gpustat.module.sh"
}

# _standalone_module runs the module as a self-contained CLI (the same entry
# users hit when they type `bash module/gpustat.module.sh ...`).
_standalone_module() {
    bash "${MODULE_DIR}/gpustat.module.sh" "$@"
}
...
# Fake gpustat binary answering --version.
_fake_gpustat_bin() {
    mkdir -p "${INIT_UBUNTU_TEST_SCRATCH}/bin"
    printf '#!/usr/bin/env bash\nprintf "gpustat 1.1.1\\n"\n' \
        > "${INIT_UBUNTU_TEST_SCRATCH}/bin/gpustat"
    chmod +x "${INIT_UBUNTU_TEST_SCRATCH}/bin/gpustat"
...
@test "gpustat module file parses (bash -n)" {
    run bash -n "${MODULE_DIR}/gpustat.module.sh"
    assert_success
}

@test "gpustat module sources cleanly in engine mode (MODULE_STANDALONE=false)" {
    _load_module
    [[ "${MODULE_STANDALONE}" == "false" ]]
}

@test "gpustat module defines all 10 lifecycle functions" {
    _load_module
    local _fn
...
@test "gpustat module declares NAME=gpustat" {
    _load_module
    [[ "${NAME}" == "gpustat" ]]
}

@test "gpustat module CATEGORY=optional" {
    _load_module
    [[ "${CATEGORY}" == "optional" ]]
}

@test "gpustat module TAGS contains gpu" {
    _load_module
    [[ " ${TAGS[*]} " == *" gpu "* ]]
}

@test "gpustat module DEPENDS_ON contains pipx" {
    _load_module
    [[ " ${DEPENDS_ON[*]} " == *" pipx "* ]]
}

@test "gpustat DESCRIPTION is associative with en + zh-TW entries" {
    _load_module
...
@test "gpustat module_get_description returns language-specific text" {
    _load_module
    [[ "$(module_get_description en)" == *"GPU"* ]]
    [[ -n "$(module_get_description zh-TW)" ]]
    # Unknown language falls back to en.
    [[ "$(module_get_description xx)" == "$(module_get_description en)" ]]
}

@test "gpustat module VERSION_PROVIDED=pipx-managed" {
    _load_module
    [[ "${VERSION_PROVIDED}" == "pipx-managed" ]]
}

@test "gpustat module INSTALL_TARGET_DEFAULT=user-home (pipx is user-level)" {
    _load_module
    [[ "${INSTALL_TARGET_DEFAULT}" == "user-home" ]]
}

@test "gpustat module SUPPORTS_USER_HOME=true" {
    _load_module
    [[ "${SUPPORTS_USER_HOME}" == "true" ]]
}

@test "gpustat module RISK_LEVEL=low" {
    _load_module
    [[ "${RISK_LEVEL}" == "low" ]]
}

@test "gpustat module SUPPORTED_UBUNTU covers 22.04 / 24.04 / 26.04" {
    _load_module
...
@test "gpustat HOMEPAGE points at the wookayin/gpustat project" {
    _load_module
    [[ "${HOMEPAGE}" == *"wookayin/gpustat"* ]]
}

@test "gpustat POST_INSTALL_MESSAGE mentions pipx / PATH" {
    _load_module
    [[ "$(module_get_post_install_message en)" == *"pipx"* ]]
    [[ -n "$(module_get_post_install_message zh-TW)" ]]
}

@test "TEST_VERIFY_CMD exercises the gpustat binary" {
    _load_module
    [[ "${TEST_VERIFY_CMD}" == *"gpustat"* ]]
}

# ── is_installed: pipx ownership ─────────────────────────────────────────────

@test "is_installed returns zero when pipx list shows gpustat" {
    _load_module
    MOCK_PIPX_LIST="gpustat 1.1.1"
    _mock_pipx
    run is_installed
    assert_success
}
...
@test "dry-run install writes nothing under the state dir" {
    ...
    INIT_UBUNTU_DRY_RUN=true run install
    assert_success
    [[ ! -e "${INIT_UBUNTU_STATE_DIR}/versions/gpustat" ]]
...
@test "install runs pipx install gpustat" {
    _load_module
    MOCK_PIPX_LIST=""
    _mock_pipx
    install
    grep -q '^install gpustat$' "${MOCK_PIPX_LOG}"
}
...
@test "install skips when already pipx-managed (idempotent fast path)" {
    ...
    run grep -q '^install gpustat$' "${MOCK_PIPX_LOG}"
    assert_failure
}
...
@test "upgrade uses pipx upgrade when already pipx-managed" {
    ...
    run upgrade
    assert_success
    grep -q '^upgrade gpustat$' "${MOCK_PIPX_LOG}"
}
...
@test "remove uses pipx uninstall" {
    ...
    run remove
    assert_success
    grep -q '^uninstall gpustat$' "${MOCK_PIPX_LOG}"
}

@test "remove on a not-installed gpustat is a no-op" {
    _load_module
    MOCK_PIPX_LIST=""
    _mock_pipx
    run remove
...
@test "install writes the sidecar with the pipx-reported version" {
    ...
    [[ -f "${INIT_UBUNTU_STATE_DIR}/versions/gpustat" ]]
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/gpustat")" == "1.1.1" ]]
}
...
@test "install sidecar falls back to pipx-managed when pipx list is empty" {
    ...
    _mock_pipx
    module_standalone_main install
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/gpustat")" == "pipx-managed" ]]
}
...
@test "remove deletes the sidecar" {
    ...
    printf '1.1.1\n' > "${INIT_UBUNTU_STATE_DIR}/versions/gpustat"
    MOCK_PIPX_LIST="gpustat 1.1.1"
    _mock_pipx
...
@test "doctor passes when gpustat answers --version" {
    _load_module
    MOCK_PIPX_LIST="gpustat 1.1.1"
    _mock_pipx
    _fake_gpustat_bin
...
@test "doctor fails when the gpustat binary is not on PATH" {
    _load_module
    MOCK_PIPX_LIST="gpustat 1.1.1"
    _mock_pipx
...
@test "standalone: info prints metadata" {
    ...
    assert_success
    assert_output --partial "name:        gpustat"
