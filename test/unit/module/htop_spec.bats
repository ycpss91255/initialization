#!/usr/bin/env bats
# test/unit/module/htop_spec.bats — module/htop.module.sh
#
...
_load_module() {
    ...
    # shellcheck source=../../../module/htop.module.sh
    source "${MODULE_DIR}/htop.module.sh"
}

_standalone_module() {
    bash "${MODULE_DIR}/htop.module.sh" "$@"
}
...
@test "htop module file parses (bash -n)" {
    run bash -n "${MODULE_DIR}/htop.module.sh"
    assert_success
}

@test "htop module sources cleanly in engine mode (MODULE_STANDALONE=false)" {
    _load_module
    [[ "${MODULE_STANDALONE}" == "false" ]]
}

@test "htop module defines all 10 lifecycle functions" {
    _load_module
    local _fn
...
@test "htop module declares NAME=htop" {
    _load_module
    [[ "${NAME}" == "htop" ]]
}

@test "htop module CATEGORY=base" {
    _load_module
    [[ "${CATEGORY}" == "base" ]]
}

@test "htop module TAGS contains monitor" {
    _load_module
    [[ " ${TAGS[*]} " == *" monitor "* ]]
}

@test "htop module DEPENDS_ON is empty (base tool)" {
    _load_module
    [[ "${#DEPENDS_ON[@]}" -eq 0 ]]
}

@test "htop DESCRIPTION is associative with en + zh-TW entries" {
    _load_module
...
@test "htop module_get_description returns language-specific text + en fallback" {
    _load_module
    [[ -n "$(module_get_description en)" ]]
    [[ "$(module_get_description zh-TW)" == *"監看"* ]]
    [[ "$(module_get_description xx)" == "$(module_get_description en)" ]]
}

@test "htop SUPPORTED_UBUNTU includes 22.04 24.04 26.04" {
    _load_module
...
@test "htop SUPPORTED_PLATFORMS covers all five form factors" {
    _load_module
    local _p
...
@test "htop module RISK_LEVEL=low and VERSION_PROVIDED=apt-managed" {
    _load_module
    [[ "${RISK_LEVEL}" == "low" ]]
    [[ "${VERSION_PROVIDED}" == "apt-managed" ]]
}

@test "htop archetype data installs the htop apt package" {
    _load_module
    [[ " ${APT_PKGS[*]} " == *" htop "* ]]
    [[ "${#APT_PKGS[@]}" -eq 1 ]]
    [[ -z "${APT_PPA}" ]]
...
@test "is_installed returns nonzero when dpkg does not report htop" {
    _load_module
    MOCK_DPKG_RC=1
    _mock_dpkg
    run is_installed
...
@test "is_installed returns zero when dpkg reports htop as ii" {
    _load_module
    MOCK_DPKG_OUTPUT='ii  htop  99.9.9  amd64  pkg'
    _mock_dpkg
    run is_installed
    assert_success
}
...
@test "dry-run install writes nothing under the state dir" {
    ...
    INIT_UBUNTU_DRY_RUN=true run install
    assert_success
    [[ ! -e "${INIT_UBUNTU_STATE_DIR}/versions/htop" ]]
...
@test "install writes the sidecar with the dpkg-reported version" {
    ...
    _mock_dpkg_query
    install
    [[ -f "${INIT_UBUNTU_STATE_DIR}/versions/htop" ]]
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/htop")" == "1.2.3" ]]
}
...
@test "install sidecar falls back to apt-managed when dpkg-query is empty" {
    ...
    _mock_dpkg_query
    install
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/htop")" == "apt-managed" ]]
}
...
@test "upgrade refreshes the sidecar version" {
    ...
    printf '1.0.0\n' > "${INIT_UBUNTU_STATE_DIR}/versions/htop"
    _mock_apt_defaults
    MOCK_PKG_VERSION="1.2.3"
    _mock_dpkg_query
    upgrade
...
@test "doctor passes when the htop binary answers --version" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
    mkdir -p "${INIT_UBUNTU_TEST_SCRATCH}/bin"
    printf '#!/usr/bin/env bash\nprintf "htop 1.2.3\n"\n' \
        > "${INIT_UBUNTU_TEST_SCRATCH}/bin/htop"
    chmod +x "${INIT_UBUNTU_TEST_SCRATCH}/bin/htop"
    PATH="${INIT_UBUNTU_TEST_SCRATCH}/bin:${PATH}" run doctor
    assert_success
}

@test "is_outdated returns zero when apt reports htop upgradable" {
    _load_module
    MOCK_APT_UPGRADABLE='htop/noble-updates 1.2.3 amd64 [upgradable from: 1.2.2]'
    _mock_apt_list
    run is_outdated
    assert_success
}

@test "is_outdated returns nonzero when htop is not in the upgradable list" {
    _load_module
...
@test "standalone: info prints metadata" {
    ...
    assert_success
    assert_output --partial "name:        htop"
