#!/usr/bin/env bats
# test/unit/module/ibus-rime_spec.bats — module/ibus-rime.module.sh
#
...
_load_module() {
    ...
    # shellcheck source=../../../module/ibus-rime.module.sh
    source "${MODULE_DIR}/ibus-rime.module.sh"
}

_standalone_module() {
    bash "${MODULE_DIR}/ibus-rime.module.sh" "$@"
}
...
@test "ibus-rime module file parses (bash -n)" {
    run bash -n "${MODULE_DIR}/ibus-rime.module.sh"
    assert_success
}

@test "ibus-rime module sources cleanly in engine mode (MODULE_STANDALONE=false)" {
    _load_module
    [[ "${MODULE_STANDALONE}" == "false" ]]
}

@test "ibus-rime module defines all 10 lifecycle functions (ADR-0002 / #305)" {
    _load_module
    local _fn
...
@test "ibus-rime module declares NAME=ibus-rime" {
    _load_module
    [[ "${NAME}" == "ibus-rime" ]]
}

@test "ibus-rime module CATEGORY=optional" {
    _load_module
    [[ "${CATEGORY}" == "optional" ]]
}

@test "ibus-rime module TAGS contains input-method" {
    _load_module
    [[ " ${TAGS[*]} " == *" input-method "* ]]
}

@test "ibus-rime module DEPENDS_ON is empty (standalone tool)" {
    _load_module
    [[ "${#DEPENDS_ON[@]}" -eq 0 ]]
}

@test "ibus-rime DESCRIPTION is associative with en + zh-TW entries" {
    _load_module
...
@test "ibus-rime module_get_description returns language-specific text + en fallback" {
    _load_module
    [[ -n "$(module_get_description en)" ]]
    [[ "$(module_get_description zh-TW)" == *"輸入法"* ]]
    [[ "$(module_get_description xx)" == "$(module_get_description en)" ]]
}

@test "ibus-rime SUPPORTED_UBUNTU includes 22.04 24.04 26.04" {
    _load_module
...
@test "ibus-rime SUPPORTED_PLATFORMS is desktop-only" {
    _load_module
    [[ "${#SUPPORTED_PLATFORMS[@]}" -eq 1 ]]
    [[ "${SUPPORTED_PLATFORMS[0]}" == "desktop" ]]
}

@test "ibus-rime module RISK_LEVEL=low and VERSION_PROVIDED=apt-managed" {
    _load_module
    [[ "${RISK_LEVEL}" == "low" ]]
    [[ "${VERSION_PROVIDED}" == "apt-managed" ]]
}

@test "ibus-rime archetype data installs the ibus-rime apt package" {
    _load_module
    [[ " ${APT_PKGS[*]} " == *" ibus-rime "* ]]
    [[ "${#APT_PKGS[@]}" -eq 1 ]]
    [[ -z "${APT_PPA}" ]]
...
@test "ibus-rime TEST_VERIFY_CMD probes the ibus binary" {
    _load_module
    [[ "${TEST_VERIFY_CMD}" == *"ibus"* ]]
}

# ── is_installed ─────────────────────────────────────────────────────────────

@test "is_installed returns nonzero when dpkg does not report ibus-rime" {
    _load_module
    MOCK_DPKG_RC=1
    _mock_dpkg
    run is_installed
...
@test "is_installed returns zero when dpkg reports ibus-rime as ii" {
    _load_module
    MOCK_DPKG_OUTPUT='ii  ibus-rime  99.9.9  amd64  pkg'
    _mock_dpkg
    run is_installed
    assert_success
}
...
@test "dry-run install writes nothing under the state dir" {
    ...
    INIT_UBUNTU_DRY_RUN=true run install
    assert_success
    [[ ! -e "${INIT_UBUNTU_STATE_DIR}/versions/ibus-rime" ]]
...
@test "install writes the sidecar with the dpkg-reported version" {
    ...
    [[ -f "${INIT_UBUNTU_STATE_DIR}/versions/ibus-rime" ]]
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/ibus-rime")" == "1.2.3" ]]
}
...
@test "install sidecar falls back to apt-managed when dpkg-query is empty" {
    ...
    _mock_dpkg_query
    module_standalone_main install
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/ibus-rime")" == "apt-managed" ]]
}
...
@test "upgrade refreshes the sidecar version" {
    ...
    printf '1.0.0\n' > "${INIT_UBUNTU_STATE_DIR}/versions/ibus-rime"
    _mock_apt_defaults
    MOCK_PKG_VERSION="1.2.3"
    _mock_dpkg_query
...
@test "doctor fails when installed but the ibus binary is not runnable" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
...
@test "doctor passes when installed and the ibus binary runs" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
    _stub_bin ibus
    RIME_DATA_DIR="${INIT_UBUNTU_TEST_SCRATCH}/rime-data"
    mkdir -p "${RIME_DATA_DIR}"
    PATH="${INIT_UBUNTU_TEST_SCRATCH}/bin:${PATH}" run doctor
    assert_success
}

@test "ibus-rime doctor fails when the Rime data dir is missing" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
    _stub_bin ibus
    RIME_DATA_DIR="${INIT_UBUNTU_TEST_SCRATCH}/nonexistent-rime-data"
    PATH="${INIT_UBUNTU_TEST_SCRATCH}/bin:${PATH}" run doctor
    assert_failure
}

@test "is_outdated returns zero when apt reports ibus-rime upgradable" {
    _load_module
    MOCK_APT_UPGRADABLE='ibus-rime/noble-updates 7.94 amd64 [upgradable from: 7.93]'
    _mock_apt_list
    run is_outdated
    assert_success
}

@test "is_outdated returns nonzero when ibus-rime is not in the upgradable list" {
    _load_module
...
@test "standalone: --version prints NAME + VERSION_PROVIDED" {
    ...
    assert_success
    assert_output --partial "ibus-rime"
}
...
@test "standalone: info prints metadata" {
    ...
    assert_success
    assert_output --partial "name:        ibus-rime"
