#!/usr/bin/env bats
# test/unit/module/ifstat_spec.bats — module/ifstat.module.sh
#
...
_load_module() {
    ...
    # shellcheck source=../../../module/ifstat.module.sh
    source "${MODULE_DIR}/ifstat.module.sh"
}

_standalone_module() {
    bash "${MODULE_DIR}/ifstat.module.sh" "$@"
}
...
@test "ifstat module file parses (bash -n)" {
    run bash -n "${MODULE_DIR}/ifstat.module.sh"
    assert_success
}

@test "ifstat module sources cleanly in engine mode (MODULE_STANDALONE=false)" {
    _load_module
    [[ "${MODULE_STANDALONE}" == "false" ]]
}

@test "ifstat module defines all 10 lifecycle functions (ADR-0002 / #305)" {
    _load_module
    local _fn
...
@test "ifstat module declares NAME=ifstat" {
    _load_module
    [[ "${NAME}" == "ifstat" ]]
}

@test "ifstat module CATEGORY=optional" {
    _load_module
    [[ "${CATEGORY}" == "optional" ]]
}

@test "ifstat module TAGS contains monitoring" {
    _load_module
    [[ " ${TAGS[*]} " == *" monitoring "* ]]
}

@test "ifstat module DEPENDS_ON is empty (standalone tool)" {
    _load_module
    [[ "${#DEPENDS_ON[@]}" -eq 0 ]]
}

@test "ifstat DESCRIPTION is associative with en + zh-TW entries" {
    _load_module
...
@test "ifstat module_get_description returns language-specific text + en fallback" {
    _load_module
    [[ -n "$(module_get_description en)" ]]
    [[ "$(module_get_description zh-TW)" == *"頻寬統計"* ]]
    [[ "$(module_get_description xx)" == "$(module_get_description en)" ]]
}

@test "ifstat SUPPORTED_UBUNTU includes 22.04 24.04 26.04" {
    _load_module
...
@test "ifstat SUPPORTED_PLATFORMS covers all five form factors" {
    _load_module
    local _p
...
@test "ifstat module RISK_LEVEL=low and VERSION_PROVIDED=apt-managed" {
    _load_module
    [[ "${RISK_LEVEL}" == "low" ]]
    [[ "${VERSION_PROVIDED}" == "apt-managed" ]]
}

@test "ifstat archetype data installs the ifstat apt package" {
    _load_module
    [[ " ${APT_PKGS[*]} " == *" ifstat "* ]]
    [[ "${#APT_PKGS[@]}" -eq 1 ]]
    [[ -z "${APT_PPA}" ]]
...
@test "is_installed returns nonzero when dpkg does not report ifstat" {
    _load_module
    MOCK_DPKG_RC=1
    _mock_dpkg
    run is_installed
...
@test "is_installed returns zero when dpkg reports ifstat as ii" {
    _load_module
    MOCK_DPKG_OUTPUT='ii  ifstat  99.9.9  amd64  pkg'
    _mock_dpkg
    run is_installed
    assert_success
}
...
@test "dry-run install writes nothing under the state dir" {
    ...
    INIT_UBUNTU_DRY_RUN=true run install
    assert_success
    [[ ! -e "${INIT_UBUNTU_STATE_DIR}/versions/ifstat" ]]
...
@test "install writes the sidecar with the dpkg-reported version" {
    ...
    [[ -f "${INIT_UBUNTU_STATE_DIR}/versions/ifstat" ]]
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/ifstat")" == "1.2.3" ]]
}
...
@test "install sidecar falls back to apt-managed when dpkg-query is empty" {
    ...
    _mock_dpkg_query
    module_standalone_main install
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/ifstat")" == "apt-managed" ]]
}
...
@test "upgrade refreshes the sidecar version" {
    ...
    printf '1.0.0\n' > "${INIT_UBUNTU_STATE_DIR}/versions/ifstat"
    _mock_apt_defaults
    MOCK_PKG_VERSION="1.2.3"
    _mock_dpkg_query
...
@test "doctor passes when installed and the ifstat binary is present" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
    _stub_bin ifstat
...
@test "is_outdated returns zero when apt reports ifstat upgradable" {
    _load_module
    MOCK_APT_UPGRADABLE='ifstat/noble-updates 1.2.3 amd64 [upgradable from: 1.2.2]'
    _mock_apt_list
    run is_outdated
    assert_success
}

@test "is_outdated returns nonzero when ifstat is not in the upgradable list" {
    _load_module
...
@test "standalone: info prints metadata" {
    ...
    assert_success
    assert_output --partial "name:        ifstat"
