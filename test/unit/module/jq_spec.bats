#!/usr/bin/env bats
# test/unit/module/jq_spec.bats — module/jq.module.sh
#
...
_load_module() {
    ...
    # shellcheck source=../../../module/jq.module.sh
    source "${MODULE_DIR}/jq.module.sh"
}

_standalone_module() {
    bash "${MODULE_DIR}/jq.module.sh" "$@"
}
...
@test "jq module file parses (bash -n)" {
    run bash -n "${MODULE_DIR}/jq.module.sh"
    assert_success
}

@test "jq module sources cleanly in engine mode (MODULE_STANDALONE=false)" {
    _load_module
    [[ "${MODULE_STANDALONE}" == "false" ]]
}

@test "jq module defines all 10 lifecycle functions" {
    _load_module
    local _fn
...
@test "jq module declares NAME=jq" {
    _load_module
    [[ "${NAME}" == "jq" ]]
}

@test "jq module CATEGORY=base" {
    _load_module
    [[ "${CATEGORY}" == "base" ]]
}

@test "jq module TAGS contains core" {
    _load_module
    [[ " ${TAGS[*]} " == *" core "* ]]
}

@test "jq module DEPENDS_ON is empty (base tool)" {
    _load_module
    [[ "${#DEPENDS_ON[@]}" -eq 0 ]]
}

@test "jq DESCRIPTION is associative with en + zh-TW entries" {
    _load_module
...
@test "jq module_get_description returns language-specific text + en fallback" {
    _load_module
    [[ -n "$(module_get_description en)" ]]
    [[ "$(module_get_description zh-TW)" == *"處理"* ]]
    [[ "$(module_get_description xx)" == "$(module_get_description en)" ]]
}

@test "jq SUPPORTED_UBUNTU includes 22.04 24.04 26.04" {
    _load_module
...
@test "jq SUPPORTED_PLATFORMS covers all five form factors" {
    _load_module
    local _p
...
@test "jq module RISK_LEVEL=low and VERSION_PROVIDED=apt-managed" {
    _load_module
    [[ "${RISK_LEVEL}" == "low" ]]
    [[ "${VERSION_PROVIDED}" == "apt-managed" ]]
}

@test "jq archetype data installs the jq apt package" {
    _load_module
    [[ " ${APT_PKGS[*]} " == *" jq "* ]]
    [[ "${#APT_PKGS[@]}" -eq 1 ]]
    [[ -z "${APT_PPA}" ]]
...
@test "is_installed returns nonzero when dpkg does not report jq" {
    _load_module
    MOCK_DPKG_RC=1
    _mock_dpkg
    run is_installed
...
@test "is_installed returns zero when dpkg reports jq as ii" {
    _load_module
    MOCK_DPKG_OUTPUT='ii  jq  99.9.9  amd64  pkg'
    _mock_dpkg
    run is_installed
    assert_success
}
...
@test "dry-run install writes nothing under the state dir" {
    ...
    INIT_UBUNTU_DRY_RUN=true run install
    assert_success
    [[ ! -e "${INIT_UBUNTU_STATE_DIR}/versions/jq" ]]
...
@test "install writes the sidecar with the dpkg-reported version" {
    ...
    _mock_dpkg_query
    install
    [[ -f "${INIT_UBUNTU_STATE_DIR}/versions/jq" ]]
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/jq")" == "1.2.3" ]]
}
...
@test "install sidecar falls back to apt-managed when dpkg-query is empty" {
    ...
    _mock_dpkg_query
    install
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/jq")" == "apt-managed" ]]
}
...
@test "upgrade refreshes the sidecar version" {
    ...
    printf '1.0.0\n' > "${INIT_UBUNTU_STATE_DIR}/versions/jq"
    _mock_apt_defaults
    MOCK_PKG_VERSION="1.2.3"
    _mock_dpkg_query
    upgrade
...
@test "doctor passes when the jq binary answers --version" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
    mkdir -p "${INIT_UBUNTU_TEST_SCRATCH}/bin"
    printf '#!/usr/bin/env bash\nprintf "jq 1.2.3\n"\n' \
        > "${INIT_UBUNTU_TEST_SCRATCH}/bin/jq"
    chmod +x "${INIT_UBUNTU_TEST_SCRATCH}/bin/jq"
    PATH="${INIT_UBUNTU_TEST_SCRATCH}/bin:${PATH}" run doctor
    assert_success
}

@test "is_outdated returns zero when apt reports jq upgradable" {
    _load_module
    MOCK_APT_UPGRADABLE='jq/noble-updates 1.2.3 amd64 [upgradable from: 1.2.2]'
    _mock_apt_list
    run is_outdated
    assert_success
}

@test "is_outdated returns nonzero when jq is not in the upgradable list" {
    _load_module
