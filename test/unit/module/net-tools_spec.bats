#!/usr/bin/env bats
# test/unit/module/net-tools_spec.bats — module/net-tools.module.sh
#
...
_load_module() {
    ...
    # shellcheck source=../../../module/net-tools.module.sh
    source "${MODULE_DIR}/net-tools.module.sh"
}

_standalone_module() {
    bash "${MODULE_DIR}/net-tools.module.sh" "$@"
}
...
@test "net-tools module file parses (bash -n)" {
    run bash -n "${MODULE_DIR}/net-tools.module.sh"
    assert_success
}

@test "net-tools module sources cleanly in engine mode (MODULE_STANDALONE=false)" {
    _load_module
    [[ "${MODULE_STANDALONE}" == "false" ]]
}

@test "net-tools module defines all 10 lifecycle functions" {
    _load_module
    local _fn
...
@test "net-tools module declares NAME=net-tools" {
    _load_module
    [[ "${NAME}" == "net-tools" ]]
}

@test "net-tools module CATEGORY=optional" {
    _load_module
    [[ "${CATEGORY}" == "optional" ]]
}

@test "net-tools module TAGS contains network" {
    _load_module
    [[ " ${TAGS[*]} " == *" network "* ]]
}

@test "net-tools module DEPENDS_ON is empty" {
    _load_module
    [[ "${#DEPENDS_ON[@]}" -eq 0 ]]
}

@test "net-tools DESCRIPTION is associative with en + zh-TW entries" {
    _load_module
...
@test "net-tools module_get_description returns language-specific text + en fallback" {
    _load_module
    [[ -n "$(module_get_description en)" ]]
    [[ "$(module_get_description zh-TW)" == *"網路工具"* ]]
    [[ "$(module_get_description xx)" == "$(module_get_description en)" ]]
}

@test "net-tools SUPPORTED_UBUNTU includes 22.04 24.04 26.04" {
    _load_module
...
@test "net-tools module RISK_LEVEL=low and VERSION_PROVIDED=apt-managed" {
    _load_module
    [[ "${RISK_LEVEL}" == "low" ]]
    [[ "${VERSION_PROVIDED}" == "apt-managed" ]]
}

@test "net-tools archetype data installs the net-tools apt package" {
    _load_module
    [[ " ${APT_PKGS[*]} " == *" net-tools "* ]]
    [[ "${#APT_PKGS[@]}" -eq 1 ]]
    [[ -z "${APT_PPA}" ]]
...
@test "is_installed returns nonzero when dpkg does not report net-tools" {
    _load_module
    MOCK_DPKG_RC=1
    _mock_dpkg
    run is_installed
...
@test "is_installed returns zero when dpkg reports net-tools as ii" {
    _load_module
    MOCK_DPKG_OUTPUT='ii  net-tools  2.10-0.1  amd64  pkg'
    _mock_dpkg
    run is_installed
    assert_success
}
...
@test "dry-run install writes nothing under the state dir" {
    ...
    INIT_UBUNTU_DRY_RUN=true run install
    assert_success
    [[ ! -e "${INIT_UBUNTU_STATE_DIR}/versions/net-tools" ]]
...
@test "install writes the sidecar with the dpkg-reported version" {
    _load_module
    _mock_apt_defaults
    MOCK_PKG_VERSION="2.10-0.1"
    _mock_dpkg_query
    module_standalone_main install
    [[ -f "${INIT_UBUNTU_STATE_DIR}/versions/net-tools" ]]
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/net-tools")" == "2.10-0.1" ]]
}
...
@test "remove deletes the sidecar" {
    ...
    printf '1.0.0\n' > "${INIT_UBUNTU_STATE_DIR}/versions/net-tools"
    _mock_apt_defaults
...
@test "doctor passes when the ifconfig probe binary is available" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
    _fake_bin ifconfig 'exit 0'
    PATH="${INIT_UBUNTU_TEST_SCRATCH}/bin:${PATH}" run doctor
    assert_success
}

@test "doctor fails when the ifconfig probe binary is missing even though installed" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
...
@test "is_outdated returns zero when apt reports net-tools upgradable" {
    _load_module
    MOCK_APT_UPGRADABLE='net-tools/noble-updates 2.10-0.1 amd64 [upgradable from: 0.0.0]'
    _mock_apt_list
    run is_outdated
    assert_success
}

@test "is_outdated returns nonzero when net-tools is not in the upgradable list" {
    _load_module
...
@test "standalone: --version prints NAME + VERSION_PROVIDED" {
    ...
    assert_success
    assert_output --partial "net-tools"
}
...
@test "standalone: info prints metadata" {
    ...
    assert_success
    assert_output --partial "name:        net-tools"
