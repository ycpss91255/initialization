#!/usr/bin/env bats
# test/unit/module/nmap_spec.bats — module/nmap.module.sh
#
...
# (ADR-0001) / real doctor probe (nmap --version).
...
_load_module() {
    ...
    # shellcheck source=../../../module/nmap.module.sh
    source "${MODULE_DIR}/nmap.module.sh"
}

_standalone_module() {
    bash "${MODULE_DIR}/nmap.module.sh" "$@"
}
...
# (satisfies both `<name> --version` and `command -v <name>` doctor probes).
_stub_bin() {
...
@test "nmap module file parses (bash -n)" {
    run bash -n "${MODULE_DIR}/nmap.module.sh"
    assert_success
}

@test "nmap module sources cleanly in engine mode (MODULE_STANDALONE=false)" {
    _load_module
    [[ "${MODULE_STANDALONE}" == "false" ]]
}

@test "nmap module defines all 10 lifecycle functions (ADR-0002 / #305)" {
    _load_module
    local _fn
...
@test "nmap module declares NAME=nmap" {
    _load_module
    [[ "${NAME}" == "nmap" ]]
}

@test "nmap module CATEGORY=optional" {
    _load_module
    [[ "${CATEGORY}" == "optional" ]]
}

@test "nmap module TAGS contains monitoring" {
    _load_module
    [[ " ${TAGS[*]} " == *" monitoring "* ]]
}

@test "nmap module DEPENDS_ON is empty (standalone tool)" {
    _load_module
    [[ "${#DEPENDS_ON[@]}" -eq 0 ]]
}

@test "nmap DESCRIPTION is associative with en + zh-TW entries" {
    _load_module
...
@test "nmap module_get_description returns language-specific text + en fallback" {
    _load_module
    [[ -n "$(module_get_description en)" ]]
    [[ "$(module_get_description zh-TW)" == *"網路探測"* ]]
    [[ "$(module_get_description xx)" == "$(module_get_description en)" ]]
}

@test "nmap SUPPORTED_UBUNTU includes 22.04 24.04 26.04" {
    _load_module
...
@test "nmap SUPPORTED_PLATFORMS covers all five form factors" {
    _load_module
    local _p
...
@test "nmap module RISK_LEVEL=low and VERSION_PROVIDED=apt-managed" {
    _load_module
    [[ "${RISK_LEVEL}" == "low" ]]
    [[ "${VERSION_PROVIDED}" == "apt-managed" ]]
}

@test "nmap archetype data installs the nmap apt package" {
    _load_module
    [[ " ${APT_PKGS[*]} " == *" nmap "* ]]
    [[ "${#APT_PKGS[@]}" -eq 1 ]]
    [[ -z "${APT_PPA}" ]]
...
@test "is_installed returns nonzero when dpkg does not report nmap" {
    _load_module
    MOCK_DPKG_RC=1
    _mock_dpkg
    run is_installed
...
@test "is_installed returns zero when dpkg reports nmap as ii" {
    _load_module
    MOCK_DPKG_OUTPUT='ii  nmap  99.9.9  amd64  pkg'
    _mock_dpkg
    run is_installed
    assert_success
}
...
@test "dry-run install writes nothing under the state dir" {
    ...
    INIT_UBUNTU_DRY_RUN=true run install
    assert_success
    [[ ! -e "${INIT_UBUNTU_STATE_DIR}/versions/nmap" ]]
...
@test "install writes the sidecar with the dpkg-reported version" {
    ...
    [[ -f "${INIT_UBUNTU_STATE_DIR}/versions/nmap" ]]
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/nmap")" == "1.2.3" ]]
}
...
@test "install sidecar falls back to apt-managed when dpkg-query is empty" {
    ...
    _mock_dpkg_query
    module_standalone_main install
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/nmap")" == "apt-managed" ]]
}
...
@test "upgrade refreshes the sidecar version" {
    ...
    printf '1.0.0\n' > "${INIT_UBUNTU_STATE_DIR}/versions/nmap"
    _mock_apt_defaults
    MOCK_PKG_VERSION="1.2.3"
    _mock_dpkg_query
...
@test "doctor passes when installed and the nmap binary runs" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
    _stub_bin nmap
...
@test "is_outdated returns zero when apt reports nmap upgradable" {
    _load_module
    MOCK_APT_UPGRADABLE='nmap/noble-updates 7.94 amd64 [upgradable from: 7.93]'
    _mock_apt_list
    run is_outdated
    assert_success
}

@test "is_outdated returns nonzero when nmap is not in the upgradable list" {
    _load_module
...
@test "standalone: info prints metadata" {
    ...
    assert_success
    assert_output --partial "name:        nmap"
