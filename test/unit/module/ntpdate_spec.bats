#!/usr/bin/env bats
# test/unit/module/ntpdate_spec.bats — module/ntpdate.module.sh
#
# Legacy one-shot NTP sync tool, archetype-A apt module. Provisions the
# `ntpdate` package for the dual_system_time_sync one-off tool (ADR-0029).
# Mirrors the htop archetype-A spec: smoke / metadata / lifecycle dry-run /
# no-side-fx / idempotency / standalone CLI / sidecar lifecycle (ADR-0001),
# plus the module-specific real doctor() and always-off is_recommended().
...
_load_module() {
    ...
    # shellcheck source=../../../module/ntpdate.module.sh
    source "${MODULE_DIR}/ntpdate.module.sh"
}

_standalone_module() {
    bash "${MODULE_DIR}/ntpdate.module.sh" "$@"
}
...
@test "ntpdate module file parses (bash -n)" {
    run bash -n "${MODULE_DIR}/ntpdate.module.sh"
    assert_success
}

@test "ntpdate module sources cleanly in engine mode (MODULE_STANDALONE=false)" {
    _load_module
    [[ "${MODULE_STANDALONE}" == "false" ]]
}

@test "ntpdate module defines all 10 lifecycle functions" {
    _load_module
    local _fn
...
@test "ntpdate module declares NAME=ntpdate" {
    _load_module
    [[ "${NAME}" == "ntpdate" ]]
}

@test "ntpdate module CATEGORY=optional" {
    _load_module
    [[ "${CATEGORY}" == "optional" ]]
}

@test "ntpdate module TAGS contains time" {
    _load_module
    [[ " ${TAGS[*]} " == *" time "* ]]
}

@test "ntpdate module DEPENDS_ON is empty" {
    _load_module
    [[ "${#DEPENDS_ON[@]}" -eq 0 ]]
}

@test "ntpdate DESCRIPTION is associative with en + zh-TW entries" {
    _load_module
...
@test "ntpdate module_get_description returns language-specific text + en fallback" {
    _load_module
    [[ -n "$(module_get_description en)" ]]
    [[ "$(module_get_description zh-TW)" == *"對時"* ]]
    [[ "$(module_get_description xx)" == "$(module_get_description en)" ]]
}

@test "ntpdate SUPPORTED_UBUNTU includes 22.04 24.04 26.04" {
    _load_module
...
@test "ntpdate SUPPORTED_PLATFORMS covers desktop server wsl" {
    _load_module
    local _p
    for _p in desktop server wsl; do
...
@test "ntpdate module RISK_LEVEL=low and VERSION_PROVIDED=apt-managed" {
    _load_module
    [[ "${RISK_LEVEL}" == "low" ]]
    [[ "${VERSION_PROVIDED}" == "apt-managed" ]]
}

@test "ntpdate archetype data installs the ntpdate apt package" {
    _load_module
    [[ " ${APT_PKGS[*]} " == *" ntpdate "* ]]
    [[ "${#APT_PKGS[@]}" -eq 1 ]]
    [[ -z "${APT_PPA}" ]]
...
@test "is_installed returns nonzero when dpkg does not report ntpdate" {
    _load_module
    MOCK_DPKG_RC=1
    _mock_dpkg
    run is_installed
...
@test "is_installed returns zero when dpkg reports ntpdate as ii" {
    _load_module
    MOCK_DPKG_OUTPUT='ii  ntpdate  99.9.9  amd64  pkg'
    _mock_dpkg
    run is_installed
    assert_success
}
...
@test "dry-run install writes nothing under the state dir" {
    ...
    INIT_UBUNTU_DRY_RUN=true run install
    assert_success
    [[ ! -e "${INIT_UBUNTU_STATE_DIR}/versions/ntpdate" ]]
...
@test "install writes the sidecar with the dpkg-reported version" {
    _load_module
    _mock_apt_defaults
    MOCK_PKG_VERSION="1:4.2.8p15"
    _mock_dpkg_query
    module_standalone_main install
    [[ -f "${INIT_UBUNTU_STATE_DIR}/versions/ntpdate" ]]
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/ntpdate")" == "1:4.2.8p15" ]]
}
...
@test "install sidecar falls back to apt-managed when dpkg-query is empty" {
    ...
    _mock_dpkg_query
    module_standalone_main install
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/ntpdate")" == "apt-managed" ]]
}
...
@test "upgrade refreshes the sidecar version" {
    ...
    printf '1:4.2.8p10\n' > "${INIT_UBUNTU_STATE_DIR}/versions/ntpdate"
    _mock_apt_defaults
    MOCK_PKG_VERSION="1:4.2.8p15"
    _mock_dpkg_query
...
@test "doctor fails when installed but the ntpdate binary is not on PATH" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
    # Empty PATH so `command -v ntpdate` cannot resolve.
    PATH="${INIT_UBUNTU_TEST_SCRATCH}/empty-bin" run doctor
    assert_failure
}

@test "doctor passes when installed and the ntpdate binary is on PATH" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
    mkdir -p "${INIT_UBUNTU_TEST_SCRATCH}/bin"
    printf '#!/usr/bin/env bash\nprintf "ntpdate\\n"\n' \
        > "${INIT_UBUNTU_TEST_SCRATCH}/bin/ntpdate"
    chmod +x "${INIT_UBUNTU_TEST_SCRATCH}/bin/ntpdate"
    PATH="${INIT_UBUNTU_TEST_SCRATCH}/bin:${PATH}" run doctor
    assert_success
}

@test "is_outdated returns zero when apt reports ntpdate upgradable" {
    _load_module
    MOCK_APT_UPGRADABLE='ntpdate/noble-updates 1:4.2.8p15 amd64 [upgradable from: 1:4.2.8p10]'
    _mock_apt_list
    run is_outdated
    assert_success
}

@test "is_outdated returns nonzero when ntpdate is not in the upgradable list" {
    _load_module
...
# ── is_recommended (never part of Quick Setup — legacy special-purpose tool) ──

@test "is_recommended is nonzero when not installed (never auto-selected)" {
    _load_module
    MOCK_IS_INSTALLED_RC=1
    _mock_is_installed
    run is_recommended
...
@test "standalone: info prints metadata" {
    ...
    assert_success
    assert_output --partial "name:        ntpdate"
