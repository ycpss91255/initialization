#!/usr/bin/env bats
# test/unit/module/openssh_spec.bats — module/openssh.module.sh
#
# apt archetype-A module (multi-package: openssh-client + openssh-server).
# Mirrors the tree/figlet archetype-A spec: smoke / metadata / lifecycle
# dry-run / no-side-fx / idempotency / standalone CLI / sidecar lifecycle
# (ADR-0001) + a REAL doctor() probe (`ssh -V`).
...
_load_module() {
    ...
    # shellcheck source=../../../module/openssh.module.sh
    source "${MODULE_DIR}/openssh.module.sh"
}

_standalone_module() {
    bash "${MODULE_DIR}/openssh.module.sh" "$@"
}
...
@test "openssh module file parses (bash -n)" {
    run bash -n "${MODULE_DIR}/openssh.module.sh"
    assert_success
}

@test "openssh module sources cleanly in engine mode (MODULE_STANDALONE=false)" {
    _load_module
    [[ "${MODULE_STANDALONE}" == "false" ]]
}

@test "openssh module defines all 10 lifecycle functions" {
    _load_module
    local _fn
...
@test "openssh module declares NAME=openssh" {
    _load_module
    [[ "${NAME}" == "openssh" ]]
}

@test "openssh module CATEGORY=optional" {
    _load_module
    [[ "${CATEGORY}" == "optional" ]]
}

@test "openssh module TAGS contains network" {
    _load_module
    [[ " ${TAGS[*]} " == *" network "* ]]
}

@test "openssh module DEPENDS_ON is empty" {
    _load_module
    [[ "${#DEPENDS_ON[@]}" -eq 0 ]]
}

@test "openssh DESCRIPTION is associative with en + zh-TW entries" {
    _load_module
...
@test "openssh DESCRIPTION mentions ssh/scp/sshd (the provided binaries)" {
    _load_module
    [[ "${DESCRIPTION[en]}" == *"ssh"* ]]
    [[ "${DESCRIPTION[en]}" == *"scp"* ]]
    [[ "${DESCRIPTION[en]}" == *"sshd"* ]]
...
@test "openssh module_get_description returns language-specific text + en fallback" {
    _load_module
    [[ -n "$(module_get_description en)" ]]
    [[ "$(module_get_description zh-TW)" == *"伺服器"* ]]
    [[ "$(module_get_description xx)" == "$(module_get_description en)" ]]
}

@test "openssh SUPPORTED_UBUNTU includes 22.04 24.04 26.04" {
    _load_module
...
@test "openssh module RISK_LEVEL=low and VERSION_PROVIDED=apt-managed" {
    _load_module
    [[ "${RISK_LEVEL}" == "low" ]]
    [[ "${VERSION_PROVIDED}" == "apt-managed" ]]
}

@test "openssh archetype installs both openssh-client and openssh-server" {
    _load_module
...
@test "is_installed returns zero when dpkg reports the packages as ii" {
    _load_module
    MOCK_DPKG_OUTPUT='ii  openssh-client  1:9.6p1  amd64  pkg'
    _mock_dpkg
    run is_installed
    assert_success
}
...
@test "dry-run install writes nothing under the state dir" {
    ...
    INIT_UBUNTU_DRY_RUN=true run install
    assert_success
    [[ ! -e "${INIT_UBUNTU_STATE_DIR}/versions/openssh" ]]
...
@test "install writes the sidecar with the dpkg-reported version" {
    _load_module
    _mock_apt_defaults
    MOCK_PKG_VERSION="1:9.6p1"
    _mock_dpkg_query
    module_standalone_main install
    [[ -f "${INIT_UBUNTU_STATE_DIR}/versions/openssh" ]]
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/openssh")" == "1:9.6p1" ]]
}
...
@test "remove deletes the sidecar" {
    ...
    printf '1.0.0\n' > "${INIT_UBUNTU_STATE_DIR}/versions/openssh"
    _mock_apt_defaults
...
@test "doctor passes when the ssh probe binary is available" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
    _fake_bin ssh 'exit 0'
    PATH="${INIT_UBUNTU_TEST_SCRATCH}/bin:${PATH}" run doctor
    assert_success
}

@test "doctor fails when the ssh probe binary is missing even though installed" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
...
@test "is_outdated returns zero when apt reports an openssh package upgradable" {
    _load_module
    MOCK_APT_UPGRADABLE='openssh-client/noble-updates 1:9.6p1 amd64 [upgradable from: 1:9.5p1]'
    _mock_apt_list
    run is_outdated
    assert_success
}

@test "is_outdated returns nonzero when no openssh package is upgradable" {
    _load_module
...
@test "standalone: info prints metadata" {
    ...
    assert_success
    assert_output --partial "name:        openssh"
