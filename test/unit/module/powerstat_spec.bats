#!/usr/bin/env bats
# test/unit/module/powerstat_spec.bats — module/powerstat.module.sh
#
# Monitoring tool split into its own module (small-tools modularization
# program). Mirrors the btop archetype-A spec: smoke / metadata / lifecycle
...
_load_module() {
    ...
    # shellcheck source=../../../module/powerstat.module.sh
    source "${MODULE_DIR}/powerstat.module.sh"
}

_standalone_module() {
    bash "${MODULE_DIR}/powerstat.module.sh" "$@"
}
...
@test "powerstat module file parses (bash -n)" {
    run bash -n "${MODULE_DIR}/powerstat.module.sh"
    assert_success
}

@test "powerstat module sources cleanly in engine mode (MODULE_STANDALONE=false)" {
    _load_module
    [[ "${MODULE_STANDALONE}" == "false" ]]
}

@test "powerstat module defines all 10 lifecycle functions (ADR-0002 / #305)" {
    _load_module
    local _fn
...
@test "powerstat module declares NAME=powerstat" {
    _load_module
    [[ "${NAME}" == "powerstat" ]]
}

@test "powerstat module CATEGORY=optional" {
    _load_module
    [[ "${CATEGORY}" == "optional" ]]
}

@test "powerstat module TAGS contains monitoring" {
    _load_module
    [[ " ${TAGS[*]} " == *" monitoring "* ]]
}

@test "powerstat module DEPENDS_ON is empty (standalone tool)" {
    _load_module
    [[ "${#DEPENDS_ON[@]}" -eq 0 ]]
}

@test "powerstat DESCRIPTION is associative with en + zh-TW entries" {
    _load_module
...
@test "powerstat module_get_description returns language-specific text + en fallback" {
    _load_module
    [[ -n "$(module_get_description en)" ]]
    [[ "$(module_get_description zh-TW)" == *"耗電量"* ]]
    [[ "$(module_get_description xx)" == "$(module_get_description en)" ]]
}

@test "powerstat SUPPORTED_UBUNTU includes 22.04 24.04 26.04" {
    _load_module
...
@test "powerstat SUPPORTED_PLATFORMS covers all five form factors" {
    _load_module
    local _p
...
@test "powerstat module RISK_LEVEL=low and VERSION_PROVIDED=apt-managed" {
    _load_module
    [[ "${RISK_LEVEL}" == "low" ]]
    [[ "${VERSION_PROVIDED}" == "apt-managed" ]]
}

@test "powerstat archetype data installs the powerstat apt package" {
    _load_module
    [[ " ${APT_PKGS[*]} " == *" powerstat "* ]]
    [[ "${#APT_PKGS[@]}" -eq 1 ]]
    [[ -z "${APT_PPA}" ]]
...
@test "is_installed returns nonzero when dpkg does not report powerstat" {
    _load_module
    MOCK_DPKG_RC=1
    _mock_dpkg
    run is_installed
...
@test "is_installed returns zero when dpkg reports powerstat as ii" {
    _load_module
    MOCK_DPKG_OUTPUT='ii  powerstat  99.9.9  amd64  pkg'
    _mock_dpkg
    run is_installed
    assert_success
}
...
@test "dry-run install writes nothing under the state dir" {
    ...
    INIT_UBUNTU_DRY_RUN=true run install
    assert_success
    [[ ! -e "${INIT_UBUNTU_STATE_DIR}/versions/powerstat" ]]
...
@test "install writes the sidecar with the dpkg-reported version" {
    ...
    [[ -f "${INIT_UBUNTU_STATE_DIR}/versions/powerstat" ]]
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/powerstat")" == "1.2.3" ]]
}
...
@test "install sidecar falls back to apt-managed when dpkg-query is empty" {
    ...
    _mock_dpkg_query
    module_standalone_main install
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/powerstat")" == "apt-managed" ]]
}
...
@test "upgrade refreshes the sidecar version" {
    ...
    printf '1.0.0\n' > "${INIT_UBUNTU_STATE_DIR}/versions/powerstat"
    _mock_apt_defaults
    MOCK_PKG_VERSION="1.2.3"
    _mock_dpkg_query
...
@test "doctor fails when installed but the binary is not on PATH" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
...
@test "doctor passes when installed and the powerstat binary is present" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
    _stub_bin powerstat
...
@test "is_outdated returns zero when apt reports powerstat upgradable" {
    _load_module
    MOCK_APT_UPGRADABLE='powerstat/noble-updates 1.2.3 amd64 [upgradable from: 1.2.2]'
    _mock_apt_list
    run is_outdated
    assert_success
}

@test "is_outdated returns nonzero when powerstat is not in the upgradable list" {
    _load_module
...
@test "standalone: --version prints NAME + VERSION_PROVIDED" {
    ...
    assert_success
    assert_output --partial "powerstat"
}
...
@test "standalone: info prints metadata" {
    ...
    assert_success
    assert_output --partial "name:        powerstat"
