#!/usr/bin/env bats
# test/unit/module/powertop_spec.bats — module/powertop.module.sh
#
...
_load_module() {
    ...
    # shellcheck source=../../../module/powertop.module.sh
    source "${MODULE_DIR}/powertop.module.sh"
}

_standalone_module() {
    bash "${MODULE_DIR}/powertop.module.sh" "$@"
}
...
@test "powertop module file parses (bash -n)" {
    run bash -n "${MODULE_DIR}/powertop.module.sh"
    assert_success
}

@test "powertop module sources cleanly in engine mode (MODULE_STANDALONE=false)" {
    _load_module
    [[ "${MODULE_STANDALONE}" == "false" ]]
}

@test "powertop module defines all 10 lifecycle functions (ADR-0002 / #305)" {
    _load_module
    local _fn
...
@test "powertop module declares NAME=powertop" {
    _load_module
    [[ "${NAME}" == "powertop" ]]
}

@test "powertop module CATEGORY=optional" {
    _load_module
    [[ "${CATEGORY}" == "optional" ]]
}

@test "powertop module TAGS contains monitoring" {
    _load_module
    [[ " ${TAGS[*]} " == *" monitoring "* ]]
}

@test "powertop module DEPENDS_ON is empty (standalone tool)" {
    _load_module
    [[ "${#DEPENDS_ON[@]}" -eq 0 ]]
}

@test "powertop DESCRIPTION is associative with en + zh-TW entries" {
    _load_module
...
@test "powertop module_get_description returns language-specific text + en fallback" {
    _load_module
    [[ -n "$(module_get_description en)" ]]
    [[ "$(module_get_description zh-TW)" == *"耗電"* ]]
    [[ "$(module_get_description xx)" == "$(module_get_description en)" ]]
}

@test "powertop SUPPORTED_UBUNTU includes 22.04 24.04 26.04" {
    _load_module
...
@test "powertop SUPPORTED_PLATFORMS covers all five form factors" {
    _load_module
    local _p
...
@test "powertop module RISK_LEVEL=low and VERSION_PROVIDED=apt-managed" {
    _load_module
    [[ "${RISK_LEVEL}" == "low" ]]
    [[ "${VERSION_PROVIDED}" == "apt-managed" ]]
}

@test "powertop archetype data installs the powertop apt package" {
    _load_module
    [[ " ${APT_PKGS[*]} " == *" powertop "* ]]
    [[ "${#APT_PKGS[@]}" -eq 1 ]]
    [[ -z "${APT_PPA}" ]]
...
@test "is_installed returns nonzero when dpkg does not report powertop" {
    _load_module
    MOCK_DPKG_RC=1
    _mock_dpkg
    run is_installed
...
@test "is_installed returns zero when dpkg reports powertop as ii" {
    _load_module
    MOCK_DPKG_OUTPUT='ii  powertop  99.9.9  amd64  pkg'
    _mock_dpkg
    run is_installed
    assert_success
}
...
@test "dry-run install writes nothing under the state dir" {
    ...
    INIT_UBUNTU_DRY_RUN=true run install
    assert_success
    [[ ! -e "${INIT_UBUNTU_STATE_DIR}/versions/powertop" ]]
...
@test "install writes the sidecar with the dpkg-reported version" {
    ...
    [[ -f "${INIT_UBUNTU_STATE_DIR}/versions/powertop" ]]
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/powertop")" == "1.2.3" ]]
}
...
@test "install sidecar falls back to apt-managed when dpkg-query is empty" {
    ...
    _mock_dpkg_query
    module_standalone_main install
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/powertop")" == "apt-managed" ]]
}
...
@test "upgrade refreshes the sidecar version" {
    ...
    printf '1.0.0\n' > "${INIT_UBUNTU_STATE_DIR}/versions/powertop"
    _mock_apt_defaults
    MOCK_PKG_VERSION="1.2.3"
    _mock_dpkg_query
...
@test "doctor passes when installed and the powertop binary runs" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
    _stub_bin powertop
...
@test "is_outdated returns zero when apt reports powertop upgradable" {
    _load_module
    MOCK_APT_UPGRADABLE='powertop/noble-updates 1.2.3 amd64 [upgradable from: 1.2.2]'
    _mock_apt_list
    run is_outdated
    assert_success
}

@test "is_outdated returns nonzero when powertop is not in the upgradable list" {
    _load_module
...
@test "standalone: --version prints NAME + VERSION_PROVIDED" {
    ...
    assert_success
    assert_output --partial "powertop"
}
...
@test "standalone: info prints metadata" {
    ...
    assert_success
    assert_output --partial "name:        powertop"
