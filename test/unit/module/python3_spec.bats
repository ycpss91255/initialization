#!/usr/bin/env bats
# test/unit/module/python3_spec.bats — module/python3.module.sh
#
# Foundation apt module for the small-tools modularization program: the Python 3
# runtime + packaging toolchain (pip, dev headers, setuptools). Archetype A;
# doctor() is overridden to probe `python3 --version` for real health. Mirrors
# the build-essential archetype-A spec.
...
_load_module() {
    ...
    # shellcheck source=../../../module/python3.module.sh
    source "${MODULE_DIR}/python3.module.sh"
}

_standalone_module() {
    bash "${MODULE_DIR}/python3.module.sh" "$@"
}
...
# Put a fake python3 answering --version on PATH (exit code = $1, default 0).
_fake_python3() {
...
@test "python3 module file parses (bash -n)" {
    run bash -n "${MODULE_DIR}/python3.module.sh"
    assert_success
}

@test "python3 module sources cleanly in engine mode" {
    _load_module
    [[ "${MODULE_STANDALONE}" == "false" ]]
}

@test "python3 module defines all 10 lifecycle functions" {
    _load_module
    local _fn
...
@test "python3 module declares NAME=python3" {
    _load_module
    [[ "${NAME}" == "python3" ]]
}

@test "python3 module CATEGORY=base" {
    _load_module
    [[ "${CATEGORY}" == "base" ]]
}

@test "python3 module TAGS contains python" {
    _load_module
    [[ " ${TAGS[*]} " == *" python "* ]]
}

@test "python3 module DEPENDS_ON is empty (foundation runtime)" {
    _load_module
    [[ "${#DEPENDS_ON[@]}" -eq 0 ]]
}

@test "python3 DESCRIPTION is associative with en + zh-TW entries" {
    _load_module
...
@test "python3 module_get_description returns text + en fallback" {
    _load_module
    [[ "$(module_get_description en)" == *"Python"* ]]
    [[ -n "$(module_get_description zh-TW)" ]]
    [[ "$(module_get_description xx)" == "$(module_get_description en)" ]]
}

@test "python3 SUPPORTED_UBUNTU includes 22.04 24.04 26.04" {
    _load_module
...
@test "python3 SUPPORTED_PLATFORMS covers all five form factors" {
    _load_module
    local _p
...
@test "python3 RISK_LEVEL=low and VERSION_PROVIDED=apt-managed" {
    _load_module
    [[ "${RISK_LEVEL}" == "low" ]]
    [[ "${VERSION_PROVIDED}" == "apt-managed" ]]
}

@test "python3 archetype data installs the toolchain apt packages" {
    _load_module
    [[ " ${APT_PKGS[*]} " == *" python3 "* ]]
    [[ " ${APT_PKGS[*]} " == *" python3-pip "* ]]
    [[ " ${APT_PKGS[*]} " == *" python3-dev "* ]]
    [[ " ${APT_PKGS[*]} " == *" python3-setuptools "* ]]
    [[ -z "${APT_PPA}" ]]
}

@test "python3 TEST_VERIFY_CMD exercises the python3 interpreter" {
    _load_module
    [[ "${TEST_VERIFY_CMD}" == *"python3"* ]]
}

# ── is_installed ─────────────────────────────────────────────────────────────

@test "is_installed returns nonzero when dpkg does not report the packages" {
    _load_module
    MOCK_DPKG_RC=1
    _mock_dpkg
    run is_installed
...
@test "is_installed returns zero when dpkg reports every package as ii" {
    _load_module
    MOCK_DPKG_OUTPUT='ii  python3  3.12.3-0ubuntu1  amd64  interactive interpreter'
    _mock_dpkg
    run is_installed
    assert_success
}
...
@test "dry-run install writes nothing under the state dir" {
    ...
    INIT_UBUNTU_DRY_RUN=true run install
    assert_success
    [[ ! -e "${INIT_UBUNTU_STATE_DIR}/versions/python3" ]]
...
@test "install writes the sidecar with the dpkg-reported version" {
    _load_module
    _mock_apt_defaults
    MOCK_PKG_VERSION="3.12.3-0ubuntu1"
    _mock_dpkg_query
    module_standalone_main install
    [[ -f "${INIT_UBUNTU_STATE_DIR}/versions/python3" ]]
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/python3")" == "3.12.3-0ubuntu1" ]]
}
...
@test "install sidecar falls back to apt-managed when dpkg-query is empty" {
    ...
    _mock_dpkg_query
    module_standalone_main install
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/python3")" == "apt-managed" ]]
}
...
@test "remove deletes the sidecar" {
    ...
    printf '3.12.3\n' > "${INIT_UBUNTU_STATE_DIR}/versions/python3"
    _mock_apt_defaults
...
# ── verify / doctor (real python3 --version probe) ───────────────────────────
...
@test "doctor passes when python3 answers --version" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
    _fake_python3 0
...
@test "doctor fails when python3 --version errors out" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
    _fake_python3 1
    PATH="${INIT_UBUNTU_TEST_SCRATCH}/bin:${PATH}" run doctor
    assert_failure
    assert_output --partial "python3 --version"
}
...
@test "standalone: info prints metadata" {
    ...
    assert_success
    assert_output --partial "name:        python3"
