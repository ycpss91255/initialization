#!/usr/bin/env bats
# test/unit/module/ripgrep_spec.bats — module/ripgrep.module.sh
#
...
_load_module() {
    ...
    # shellcheck source=../../../module/ripgrep.module.sh
    source "${MODULE_DIR}/ripgrep.module.sh"
}

# _standalone_module runs the module as a self-contained CLI (the same
# entry users hit when they type `bash module/ripgrep.module.sh ...`).
_standalone_module() {
    bash "${MODULE_DIR}/ripgrep.module.sh" "$@"
}
...
#   MOCK_DPKG_OUTPUT (line printed, e.g. "ii  ripgrep ...") / MOCK_DPKG_RC.
_mock_dpkg() {
    dpkg() {
...
@test "ripgrep module file parses (bash -n)" {
    run bash -n "${MODULE_DIR}/ripgrep.module.sh"
    assert_success
}

@test "ripgrep module sources cleanly in engine mode (MODULE_STANDALONE=false)" {
    _load_module
    [[ "${MODULE_STANDALONE}" == "false" ]]
}

@test "ripgrep module defines all 10 lifecycle functions" {
    _load_module
    local _fn
...
@test "ripgrep module declares NAME=ripgrep" {
    _load_module
    [[ "${NAME}" == "ripgrep" ]]
}

@test "ripgrep module CATEGORY=optional" {
    _load_module
    [[ "${CATEGORY}" == "optional" ]]
}

@test "ripgrep module TAGS contains cli-essentials" {
    _load_module
    [[ " ${TAGS[*]} " == *" cli-essentials "* ]]
}

@test "ripgrep module DEPENDS_ON is empty (issue #55 / Q39)" {
    _load_module
    [[ "${#DEPENDS_ON[@]}" -eq 0 ]]
}

@test "ripgrep DESCRIPTION is associative with en + zh-TW entries" {
    _load_module
...
@test "ripgrep module_get_description returns language-specific text" {
    _load_module
    [[ "$(module_get_description en)" == *"grep"* ]]
    [[ -n "$(module_get_description zh-TW)" ]]
    # Unknown language falls back to en.
    [[ "$(module_get_description xx)" == "$(module_get_description en)" ]]
}

@test "ripgrep SUPPORTED_UBUNTU includes 22.04 24.04 26.04" {
    _load_module
...
@test "ripgrep module RISK_LEVEL=low" {
    _load_module
    [[ "${RISK_LEVEL}" == "low" ]]
}

@test "ripgrep module VERSION_PROVIDED=apt-managed" {
    _load_module
    [[ "${VERSION_PROVIDED}" == "apt-managed" ]]
}

@test "ripgrep HOMEPAGE points at upstream BurntSushi/ripgrep" {
    _load_module
    [[ "${HOMEPAGE}" == *"BurntSushi/ripgrep"* ]]
}

@test "ripgrep archetype data installs the ripgrep apt package" {
    _load_module
    [[ " ${APT_PKGS[*]} " == *" ripgrep "* ]]
}

@test "ripgrep POST_INSTALL_MESSAGE explains the rg binary name" {
    _load_module
    [[ "$(module_get_post_install_message en)" == *"'rg'"* ]]
    [[ -n "$(module_get_post_install_message zh-TW)" ]]
}

# ── is_installed: relies on dpkg ─────────────────────────────────────────────

@test "is_installed returns nonzero when dpkg does not report ripgrep" {
    _load_module
    MOCK_DPKG_RC=1
    _mock_dpkg
    run is_installed
...
@test "is_installed returns zero when dpkg reports ripgrep as ii" {
    _load_module
    MOCK_DPKG_OUTPUT='ii  ripgrep  99.9.9  amd64  fast grep alternative'
    _mock_dpkg
    run is_installed
    assert_success
}
...
@test "dry-run install writes nothing under the state dir" {
    ...
    INIT_UBUNTU_DRY_RUN=true run install
    assert_success
    [[ ! -e "${INIT_UBUNTU_STATE_DIR}/versions/ripgrep" ]]
...
@test "dry-run remove leaves an existing sidecar in place" {
    ...
    printf '1.0.0\n' > "${INIT_UBUNTU_STATE_DIR}/versions/ripgrep"
    INIT_UBUNTU_DRY_RUN=true run remove
    assert_success
    [[ -f "${INIT_UBUNTU_STATE_DIR}/versions/ripgrep" ]]
}
...
@test "install writes the sidecar with the dpkg-reported version" {
    _load_module
    _mock_apt_defaults
    MOCK_PKG_VERSION="14.1.0-1"
    _mock_dpkg_query
    module_standalone_main install
    [[ -f "${INIT_UBUNTU_STATE_DIR}/versions/ripgrep" ]]
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/ripgrep")" == "14.1.0-1" ]]
}
...
@test "install sidecar falls back to apt-managed when dpkg-query is empty" {
    ...
    _mock_dpkg_query
    module_standalone_main install
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/ripgrep")" == "apt-managed" ]]
}
...
@test "doctor passes when the rg binary answers --version" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
    mkdir -p "${INIT_UBUNTU_TEST_SCRATCH}/bin"
    printf '#!/usr/bin/env bash\nprintf "ripgrep 14.1.0\\n"\n' \
        > "${INIT_UBUNTU_TEST_SCRATCH}/bin/rg"
    chmod +x "${INIT_UBUNTU_TEST_SCRATCH}/bin/rg"
    PATH="${INIT_UBUNTU_TEST_SCRATCH}/bin:${PATH}" run doctor
    assert_success
}

@test "is_outdated returns zero when apt reports ripgrep upgradable" {
    _load_module
    MOCK_APT_UPGRADABLE='ripgrep/noble-updates 14.1.0-1 amd64 [upgradable from: 14.0.3-1]'
    _mock_apt_list
    run is_outdated
    assert_success
}

@test "is_outdated returns nonzero when ripgrep is not in the upgradable list" {
    _load_module
...
@test "standalone: info prints metadata" {
    ...
    assert_success
    assert_output --partial "name:        ripgrep"
