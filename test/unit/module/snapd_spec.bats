#!/usr/bin/env bats
# test/unit/module/snapd_spec.bats — module/snapd.module.sh
#
...
# sidecar lifecycle (ADR-0001) + a REAL doctor() probe (`snap`).
...
_load_module() {
    ...
    # shellcheck source=../../../module/snapd.module.sh
    source "${MODULE_DIR}/snapd.module.sh"
}

_standalone_module() {
    bash "${MODULE_DIR}/snapd.module.sh" "$@"
}
...
@test "snapd module file parses (bash -n)" {
    run bash -n "${MODULE_DIR}/snapd.module.sh"
    assert_success
}

@test "snapd module sources cleanly in engine mode (MODULE_STANDALONE=false)" {
    _load_module
    [[ "${MODULE_STANDALONE}" == "false" ]]
}

@test "snapd module defines all 10 lifecycle functions" {
    _load_module
    local _fn
...
@test "snapd module declares NAME=snapd" {
    _load_module
    [[ "${NAME}" == "snapd" ]]
}

@test "snapd module CATEGORY=optional" {
    _load_module
    [[ "${CATEGORY}" == "optional" ]]
}

@test "snapd module TAGS contains packaging" {
    _load_module
    [[ " ${TAGS[*]} " == *" packaging "* ]]
}

@test "snapd module DEPENDS_ON is empty" {
    _load_module
    [[ "${#DEPENDS_ON[@]}" -eq 0 ]]
}

@test "snapd DESCRIPTION is associative with en + zh-TW entries" {
    _load_module
...
@test "snapd module_get_description returns language-specific text + en fallback" {
    _load_module
    [[ -n "$(module_get_description en)" ]]
    [[ "$(module_get_description zh-TW)" == *"精靈"* ]]
    [[ "$(module_get_description xx)" == "$(module_get_description en)" ]]
}

@test "snapd SUPPORTED_UBUNTU includes 22.04 24.04 26.04" {
    _load_module
...
@test "snapd module RISK_LEVEL=low and VERSION_PROVIDED=apt-managed" {
    _load_module
    [[ "${RISK_LEVEL}" == "low" ]]
    [[ "${VERSION_PROVIDED}" == "apt-managed" ]]
}

@test "snapd archetype data installs the snapd apt package" {
    _load_module
    [[ " ${APT_PKGS[*]} " == *" snapd "* ]]
    [[ "${#APT_PKGS[@]}" -eq 1 ]]
    [[ -z "${APT_PPA}" ]]
...
@test "is_installed returns nonzero when dpkg does not report snapd" {
    _load_module
    MOCK_DPKG_RC=1
    _mock_dpkg
    run is_installed
...
@test "is_installed returns zero when dpkg reports snapd as ii" {
    _load_module
    MOCK_DPKG_OUTPUT='ii  snapd  2.63  amd64  pkg'
    _mock_dpkg
    run is_installed
    assert_success
}
...
@test "dry-run install writes nothing under the state dir" {
    ...
    INIT_UBUNTU_DRY_RUN=true run install
    assert_success
    [[ ! -e "${INIT_UBUNTU_STATE_DIR}/versions/snapd" ]]
...
@test "install writes the sidecar with the dpkg-reported version" {
    _load_module
    _mock_apt_defaults
    MOCK_PKG_VERSION="2.63"
    _mock_dpkg_query
    module_standalone_main install
    [[ -f "${INIT_UBUNTU_STATE_DIR}/versions/snapd" ]]
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/snapd")" == "2.63" ]]
}
...
@test "remove deletes the sidecar" {
    ...
    printf '1.0.0\n' > "${INIT_UBUNTU_STATE_DIR}/versions/snapd"
    _mock_apt_defaults
...
@test "doctor passes when the snap probe binary is available" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
    _fake_bin snap 'exit 0'
    PATH="${INIT_UBUNTU_TEST_SCRATCH}/bin:${PATH}" run doctor
    assert_success
}

@test "doctor fails when the snap probe binary is missing even though installed" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
...
@test "is_outdated returns zero when apt reports snapd upgradable" {
    _load_module
    MOCK_APT_UPGRADABLE='snapd/noble-updates 2.63 amd64 [upgradable from: 2.62]'
    _mock_apt_list
    run is_outdated
    assert_success
}

@test "is_outdated returns nonzero when snapd is not in the upgradable list" {
    _load_module
...
@test "standalone: info prints metadata" {
    ...
    assert_success
    assert_output --partial "name:        snapd"
