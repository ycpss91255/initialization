#!/usr/bin/env bats
# test/unit/module/sshfs_spec.bats — module/sshfs.module.sh
#
...
_load_module() {
    ...
    # shellcheck source=../../../module/sshfs.module.sh
    source "${MODULE_DIR}/sshfs.module.sh"
}

_standalone_module() {
    bash "${MODULE_DIR}/sshfs.module.sh" "$@"
}
...
@test "sshfs module file parses (bash -n)" {
    run bash -n "${MODULE_DIR}/sshfs.module.sh"
    assert_success
}

@test "sshfs module sources cleanly in engine mode (MODULE_STANDALONE=false)" {
    _load_module
    [[ "${MODULE_STANDALONE}" == "false" ]]
}

@test "sshfs module defines all 10 lifecycle functions" {
    _load_module
    local _fn
...
@test "sshfs module declares NAME=sshfs" {
    _load_module
    [[ "${NAME}" == "sshfs" ]]
}

@test "sshfs module CATEGORY=optional" {
    _load_module
    [[ "${CATEGORY}" == "optional" ]]
}

@test "sshfs module TAGS contains network" {
    _load_module
    [[ " ${TAGS[*]} " == *" network "* ]]
}

@test "sshfs module DEPENDS_ON is empty" {
    _load_module
    [[ "${#DEPENDS_ON[@]}" -eq 0 ]]
}

@test "sshfs DESCRIPTION is associative with en + zh-TW entries" {
    _load_module
...
@test "sshfs module_get_description returns language-specific text + en fallback" {
    _load_module
    [[ -n "$(module_get_description en)" ]]
    [[ "$(module_get_description zh-TW)" == *"掛載遠端"* ]]
    [[ "$(module_get_description xx)" == "$(module_get_description en)" ]]
}

@test "sshfs SUPPORTED_UBUNTU includes 22.04 24.04 26.04" {
    _load_module
...
@test "sshfs module RISK_LEVEL=low and VERSION_PROVIDED=apt-managed" {
    _load_module
    [[ "${RISK_LEVEL}" == "low" ]]
    [[ "${VERSION_PROVIDED}" == "apt-managed" ]]
}

@test "sshfs archetype data installs the sshfs apt package" {
    _load_module
    [[ " ${APT_PKGS[*]} " == *" sshfs "* ]]
    [[ "${#APT_PKGS[@]}" -eq 1 ]]
    [[ -z "${APT_PPA}" ]]
...
@test "is_installed returns nonzero when dpkg does not report sshfs" {
    _load_module
    MOCK_DPKG_RC=1
    _mock_dpkg
    run is_installed
...
@test "is_installed returns zero when dpkg reports sshfs as ii" {
    _load_module
    MOCK_DPKG_OUTPUT='ii  sshfs  3.7.3-1  amd64  pkg'
    _mock_dpkg
    run is_installed
    assert_success
}
...
@test "dry-run install writes nothing under the state dir" {
    ...
    INIT_UBUNTU_DRY_RUN=true run install
    assert_success
    [[ ! -e "${INIT_UBUNTU_STATE_DIR}/versions/sshfs" ]]
...
@test "install writes the sidecar with the dpkg-reported version" {
    _load_module
    _mock_apt_defaults
    MOCK_PKG_VERSION="3.7.3-1"
    _mock_dpkg_query
    module_standalone_main install
    [[ -f "${INIT_UBUNTU_STATE_DIR}/versions/sshfs" ]]
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/sshfs")" == "3.7.3-1" ]]
}
...
@test "remove deletes the sidecar" {
    ...
    printf '1.0.0\n' > "${INIT_UBUNTU_STATE_DIR}/versions/sshfs"
    _mock_apt_defaults
...
@test "doctor passes when the sshfs probe binary is available" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
    _fake_bin sshfs 'exit 0'
    PATH="${INIT_UBUNTU_TEST_SCRATCH}/bin:${PATH}" run doctor
    assert_success
}

@test "doctor fails when the sshfs probe binary is missing even though installed" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
...
@test "is_outdated returns zero when apt reports sshfs upgradable" {
    _load_module
    MOCK_APT_UPGRADABLE='sshfs/noble-updates 3.7.3-1 amd64 [upgradable from: 0.0.0]'
    _mock_apt_list
    run is_outdated
    assert_success
}

@test "is_outdated returns nonzero when sshfs is not in the upgradable list" {
    _load_module
...
@test "standalone: info prints metadata" {
    ...
    assert_success
    assert_output --partial "name:        sshfs"
