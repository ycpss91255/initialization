#!/usr/bin/env bats
# test/unit/module/tealdeer_spec.bats — module/tealdeer.module.sh
#
# apt archetype-A module with an install() override that seeds the tldr page
# cache best-effort (issue #263). Mirrors the tree archetype-A spec: smoke /
# metadata / lifecycle dry-run / no-side-fx / idempotency / standalone CLI /
# sidecar lifecycle (ADR-0001) + a REAL doctor() probe, plus tealdeer-specific
# coverage of the best-effort cache seed and POST_INSTALL_MESSAGE.
...
_load_module() {
    ...
    # shellcheck source=../../../module/tealdeer.module.sh
    source "${MODULE_DIR}/tealdeer.module.sh"
}

_standalone_module() {
    bash "${MODULE_DIR}/tealdeer.module.sh" "$@"
}
...
@test "tealdeer module file parses (bash -n)" {
    run bash -n "${MODULE_DIR}/tealdeer.module.sh"
    assert_success
}

@test "tealdeer module sources cleanly in engine mode (MODULE_STANDALONE=false)" {
    _load_module
    [[ "${MODULE_STANDALONE}" == "false" ]]
}

@test "tealdeer module defines all 10 lifecycle functions" {
    _load_module
    local _fn
...
@test "tealdeer module declares NAME=tealdeer" {
    _load_module
    [[ "${NAME}" == "tealdeer" ]]
}

@test "tealdeer module CATEGORY=optional" {
    _load_module
    [[ "${CATEGORY}" == "optional" ]]
}

@test "tealdeer module TAGS contains cli" {
    _load_module
    [[ " ${TAGS[*]} " == *" cli "* ]]
}

@test "tealdeer module DEPENDS_ON is empty" {
    _load_module
    [[ "${#DEPENDS_ON[@]}" -eq 0 ]]
}

@test "tealdeer DESCRIPTION is associative with en + zh-TW entries" {
    _load_module
...
@test "tealdeer module_get_description returns language-specific text + en fallback" {
    _load_module
    [[ -n "$(module_get_description en)" ]]
    [[ "$(module_get_description zh-TW)" == *"客戶端"* ]]
    [[ "$(module_get_description xx)" == "$(module_get_description en)" ]]
}

@test "tealdeer POST_INSTALL_MESSAGE mentions the tldr --update cache seed" {
    _load_module
    local _decl; _decl="$(declare -p POST_INSTALL_MESSAGE 2>/dev/null)"
    [[ "${_decl}" == 'declare -'*A* ]]
    [[ -n "${POST_INSTALL_MESSAGE[en]}" ]]
    [[ -n "${POST_INSTALL_MESSAGE[zh-TW]}" ]]
    [[ "${POST_INSTALL_MESSAGE[en]}" == *"tldr --update"* ]]
}

@test "tealdeer SUPPORTED_UBUNTU includes 22.04 24.04 26.04" {
    _load_module
...
@test "tealdeer module RISK_LEVEL=low and VERSION_PROVIDED=apt-managed" {
    _load_module
    [[ "${RISK_LEVEL}" == "low" ]]
    [[ "${VERSION_PROVIDED}" == "apt-managed" ]]
}

@test "tealdeer archetype data installs the tealdeer apt package" {
    _load_module
    [[ " ${APT_PKGS[*]} " == *" tealdeer "* ]]
    [[ "${#APT_PKGS[@]}" -eq 1 ]]
    [[ -z "${APT_PPA}" ]]
...
@test "is_installed returns nonzero when dpkg does not report the package" {
    _load_module
    MOCK_DPKG_RC=1
    _mock_dpkg
    run is_installed
...
@test "is_installed returns zero when dpkg reports the package as ii" {
    _load_module
    MOCK_DPKG_OUTPUT='ii  tealdeer  1.6.1  amd64  pkg'
    _mock_dpkg
    run is_installed
    assert_success
}
...
@test "dry-run install writes nothing under the state dir" {
    ...
    INIT_UBUNTU_DRY_RUN=true run install
    assert_success
    [[ ! -e "${INIT_UBUNTU_STATE_DIR}/versions/tealdeer" ]]
...
# ── install() override: best-effort cache seed (issue #263) ──────────────────

@test "install succeeds even when 'tldr --update' fails (best-effort seed)" {
    _load_module
    _mock_apt_defaults
    _fake_bin tldr 'exit 1'
    PATH="${INIT_UBUNTU_TEST_SCRATCH}/bin:${PATH}" run install
    assert_success
}

@test "install succeeds when tldr is not on PATH yet (best-effort seed)" {
    _load_module
    _mock_apt_defaults
...
@test "install writes the sidecar with the dpkg-reported version" {
    _load_module
    _mock_apt_defaults
    MOCK_PKG_VERSION="1.6.1"
    _mock_dpkg_query
    module_standalone_main install
    [[ -f "${INIT_UBUNTU_STATE_DIR}/versions/tealdeer" ]]
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/tealdeer")" == "1.6.1" ]]
}
...
@test "remove deletes the sidecar" {
    ...
    printf '1.6.1\n' > "${INIT_UBUNTU_STATE_DIR}/versions/tealdeer"
    _mock_apt_defaults
...
@test "doctor passes when the tldr probe binary is available" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
    _fake_bin tldr 'printf "tealdeer 1.6.1\n"'
    PATH="${INIT_UBUNTU_TEST_SCRATCH}/bin:${PATH}" run doctor
    assert_success
}

@test "doctor fails when the tldr probe binary is missing even though installed" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
...
@test "is_outdated returns zero when apt reports the package upgradable" {
    _load_module
    MOCK_APT_UPGRADABLE='tealdeer/noble-updates 1.6.1 amd64 [upgradable from: 1.6.0]'
    _mock_apt_list
    run is_outdated
    assert_success
}

@test "is_outdated returns nonzero when the package is not in the upgradable list" {
    _load_module
...
@test "standalone: --version prints NAME + VERSION_PROVIDED" {
    ...
    assert_success
    assert_output --partial "tealdeer"
}
...
@test "standalone: info prints metadata" {
    ...
    assert_success
    assert_output --partial "name:        tealdeer"
