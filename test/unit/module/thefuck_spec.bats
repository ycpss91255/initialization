#!/usr/bin/env bats
# test/unit/module/thefuck_spec.bats — module/thefuck.module.sh
#
...
# thefuck's doctor()/verify() use `command -v thefuck` (presence on PATH) because
# `thefuck --version` needs the shell alias/context to behave.
...
_load_module() {
    ...
    # shellcheck source=../../../module/thefuck.module.sh
    source "${MODULE_DIR}/thefuck.module.sh"
}

# _standalone_module runs the module as a self-contained CLI (the same entry
# users hit when they type `bash module/thefuck.module.sh ...`).
_standalone_module() {
    bash "${MODULE_DIR}/thefuck.module.sh" "$@"
}
...
@test "thefuck module file parses (bash -n)" {
    run bash -n "${MODULE_DIR}/thefuck.module.sh"
    assert_success
}

@test "thefuck module sources cleanly in engine mode (MODULE_STANDALONE=false)" {
    _load_module
    [[ "${MODULE_STANDALONE}" == "false" ]]
}

@test "thefuck module defines all 10 lifecycle functions" {
    _load_module
    local _fn
...
@test "thefuck module declares NAME=thefuck" {
    _load_module
    [[ "${NAME}" == "thefuck" ]]
}

@test "thefuck module CATEGORY=optional" {
    _load_module
    [[ "${CATEGORY}" == "optional" ]]
}

@test "thefuck module TAGS contains shell" {
    _load_module
    [[ " ${TAGS[*]} " == *" shell "* ]]
}

@test "thefuck module DEPENDS_ON contains pipx" {
    _load_module
    [[ " ${DEPENDS_ON[*]} " == *" pipx "* ]]
}

@test "thefuck DESCRIPTION is associative with en + zh-TW entries" {
    _load_module
...
@test "thefuck module_get_description returns language-specific text" {
    _load_module
    [[ "$(module_get_description en)" == *"command"* ]]
    [[ -n "$(module_get_description zh-TW)" ]]
    # Unknown language falls back to en.
    [[ "$(module_get_description xx)" == "$(module_get_description en)" ]]
}

@test "thefuck module VERSION_PROVIDED=pipx-managed" {
    _load_module
    [[ "${VERSION_PROVIDED}" == "pipx-managed" ]]
}

@test "thefuck module INSTALL_TARGET_DEFAULT=user-home (pipx is user-level)" {
    _load_module
    [[ "${INSTALL_TARGET_DEFAULT}" == "user-home" ]]
}

@test "thefuck module SUPPORTS_USER_HOME=true" {
    _load_module
    [[ "${SUPPORTS_USER_HOME}" == "true" ]]
}

@test "thefuck module RISK_LEVEL=low" {
    _load_module
    [[ "${RISK_LEVEL}" == "low" ]]
}

@test "thefuck module SUPPORTED_UBUNTU covers 22.04 / 24.04 / 26.04" {
    _load_module
...
@test "thefuck HOMEPAGE points at the nvbn/thefuck project" {
    _load_module
    [[ "${HOMEPAGE}" == *"nvbn/thefuck"* ]]
}

@test "thefuck POST_INSTALL_MESSAGE explains the required shell alias" {
    _load_module
    [[ "$(module_get_post_install_message en)" == *"alias"* ]]
    [[ "$(module_get_post_install_message en)" == *"pipx"* ]]
    [[ -n "$(module_get_post_install_message zh-TW)" ]]
}

@test "TEST_VERIFY_CMD probes thefuck presence (command -v, not --version)" {
    _load_module
...
@test "is_installed returns zero when pipx list shows thefuck" {
    _load_module
    MOCK_PIPX_LIST="thefuck 3.32"
    _mock_pipx
    run is_installed
    assert_success
}
...
@test "dry-run install writes nothing under the state dir" {
    ...
    INIT_UBUNTU_DRY_RUN=true run install
    assert_success
    [[ ! -e "${INIT_UBUNTU_STATE_DIR}/versions/thefuck" ]]
...
@test "install runs pipx install thefuck" {
    _load_module
    MOCK_PIPX_LIST=""
    _mock_pipx
    install
    grep -q '^install thefuck$' "${MOCK_PIPX_LOG}"
}
...
@test "install skips when already pipx-managed (idempotent fast path)" {
    ...
    run grep -q '^install thefuck$' "${MOCK_PIPX_LOG}"
    assert_failure
}
...
@test "upgrade uses pipx upgrade when already pipx-managed" {
    ...
    run upgrade
    assert_success
    grep -q '^upgrade thefuck$' "${MOCK_PIPX_LOG}"
}
...
@test "remove uses pipx uninstall" {
    ...
    run remove
    assert_success
    grep -q '^uninstall thefuck$' "${MOCK_PIPX_LOG}"
}

@test "remove on a not-installed thefuck is a no-op" {
    _load_module
    MOCK_PIPX_LIST=""
    _mock_pipx
    run remove
...
@test "install writes the sidecar with the pipx-reported version" {
    ...
    [[ -f "${INIT_UBUNTU_STATE_DIR}/versions/thefuck" ]]
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/thefuck")" == "3.32" ]]
}
...
@test "install sidecar falls back to pipx-managed when pipx list is empty" {
    ...
    _mock_pipx
    module_standalone_main install
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/thefuck")" == "pipx-managed" ]]
}
...
@test "remove deletes the sidecar" {
    ...
    printf '3.32\n' > "${INIT_UBUNTU_STATE_DIR}/versions/thefuck"
    MOCK_PIPX_LIST="thefuck 3.32"
    _mock_pipx
...
@test "verify passes when installed and thefuck is on PATH (command -v probe)" {
    _load_module
    MOCK_PIPX_LIST="thefuck 3.32"
    _mock_pipx
    _fake_bin thefuck
...
@test "doctor passes when thefuck is on PATH" {
    _load_module
    MOCK_PIPX_LIST="thefuck 3.32"
    _mock_pipx
    _fake_bin thefuck
...
@test "doctor fails when the thefuck binary is not on PATH" {
    _load_module
    MOCK_PIPX_LIST="thefuck 3.32"
    _mock_pipx
...
@test "standalone: info prints metadata" {
    ...
    assert_success
    assert_output --partial "name:        thefuck"
