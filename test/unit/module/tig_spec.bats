#!/usr/bin/env bats
# test/unit/module/tig_spec.bats — module/tig.module.sh
#
# apt archetype-A module (git text-mode UI). Mirrors the htop archetype-A
# spec: smoke / metadata / lifecycle dry-run / no-side-fx / idempotency /
# standalone CLI / sidecar lifecycle (ADR-0001) + a REAL doctor() probe.
...
_load_module() {
    ...
    # shellcheck source=../../../module/tig.module.sh
    source "${MODULE_DIR}/tig.module.sh"
}

_standalone_module() {
    bash "${MODULE_DIR}/tig.module.sh" "$@"
}
...
@test "tig module file parses (bash -n)" {
    run bash -n "${MODULE_DIR}/tig.module.sh"
    assert_success
}

@test "tig module sources cleanly in engine mode (MODULE_STANDALONE=false)" {
    _load_module
    [[ "${MODULE_STANDALONE}" == "false" ]]
}

@test "tig module defines all 10 lifecycle functions" {
    _load_module
    local _fn
...
@test "tig module declares NAME=tig" {
    _load_module
    [[ "${NAME}" == "tig" ]]
}

@test "tig module CATEGORY=optional" {
    _load_module
    [[ "${CATEGORY}" == "optional" ]]
}

@test "tig module TAGS contains git" {
    _load_module
    [[ " ${TAGS[*]} " == *" git "* ]]
}

@test "tig module DEPENDS_ON contains git" {
    _load_module
    [[ " ${DEPENDS_ON[*]} " == *" git "* ]]
}

@test "tig DESCRIPTION is associative with en + zh-TW entries" {
    _load_module
...
@test "tig module_get_description returns language-specific text + en fallback" {
    _load_module
    [[ -n "$(module_get_description en)" ]]
    [[ "$(module_get_description zh-TW)" == *"文字模式"* ]]
    [[ "$(module_get_description xx)" == "$(module_get_description en)" ]]
}

@test "tig SUPPORTED_UBUNTU includes 22.04 24.04 26.04" {
    _load_module
...
@test "tig module RISK_LEVEL=low and VERSION_PROVIDED=apt-managed" {
    _load_module
    [[ "${RISK_LEVEL}" == "low" ]]
    [[ "${VERSION_PROVIDED}" == "apt-managed" ]]
}

@test "tig archetype data installs the tig apt package" {
    _load_module
    [[ " ${APT_PKGS[*]} " == *" tig "* ]]
    [[ "${#APT_PKGS[@]}" -eq 1 ]]
    [[ -z "${APT_PPA}" ]]
...
@test "is_installed returns nonzero when dpkg does not report tig" {
    _load_module
    MOCK_DPKG_RC=1
    _mock_dpkg
    run is_installed
...
@test "is_installed returns zero when dpkg reports tig as ii" {
    _load_module
    MOCK_DPKG_OUTPUT='ii  tig  2.5.8  amd64  pkg'
    _mock_dpkg
    run is_installed
    assert_success
}
...
@test "dry-run install writes nothing under the state dir" {
    ...
    INIT_UBUNTU_DRY_RUN=true run install
    assert_success
    [[ ! -e "${INIT_UBUNTU_STATE_DIR}/versions/tig" ]]
...
@test "install writes the sidecar with the dpkg-reported version" {
    _load_module
    _mock_apt_defaults
    MOCK_PKG_VERSION="2.5.8"
    _mock_dpkg_query
    module_standalone_main install
    [[ -f "${INIT_UBUNTU_STATE_DIR}/versions/tig" ]]
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/tig")" == "2.5.8" ]]
}
...
@test "remove deletes the sidecar" {
    ...
    printf '1.0.0\n' > "${INIT_UBUNTU_STATE_DIR}/versions/tig"
    _mock_apt_defaults
...
@test "doctor passes when the tig binary answers --version" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
    _fake_bin tig 'printf "tig version 2.5.8\n"'
    PATH="${INIT_UBUNTU_TEST_SCRATCH}/bin:${PATH}" run doctor
    assert_success
}

@test "doctor fails when tig binary is missing even though installed" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
...
@test "is_outdated returns zero when apt reports tig upgradable" {
    _load_module
    MOCK_APT_UPGRADABLE='tig/noble-updates 2.5.9 amd64 [upgradable from: 2.5.8]'
    _mock_apt_list
    run is_outdated
    assert_success
}

@test "is_outdated returns nonzero when tig is not in the upgradable list" {
    _load_module
...
@test "standalone: info prints metadata" {
    ...
    assert_success
    assert_output --partial "name:        tig"
