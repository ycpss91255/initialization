#!/usr/bin/env bats
# test/unit/module/tree_spec.bats — module/tree.module.sh
#
...
_load_module() {
    ...
    # shellcheck source=../../../module/tree.module.sh
    source "${MODULE_DIR}/tree.module.sh"
}

_standalone_module() {
    bash "${MODULE_DIR}/tree.module.sh" "$@"
}
...
@test "tree module file parses (bash -n)" {
    run bash -n "${MODULE_DIR}/tree.module.sh"
    assert_success
}

@test "tree module sources cleanly in engine mode (MODULE_STANDALONE=false)" {
    _load_module
    [[ "${MODULE_STANDALONE}" == "false" ]]
}

@test "tree module defines all 10 lifecycle functions" {
    _load_module
    local _fn
...
@test "tree module declares NAME=tree" {
    _load_module
    [[ "${NAME}" == "tree" ]]
}

@test "tree module CATEGORY=optional" {
    _load_module
    [[ "${CATEGORY}" == "optional" ]]
}

@test "tree module TAGS contains cli" {
    _load_module
    [[ " ${TAGS[*]} " == *" cli "* ]]
}

@test "tree module DEPENDS_ON is empty" {
    _load_module
    [[ "${#DEPENDS_ON[@]}" -eq 0 ]]
}

@test "tree DESCRIPTION is associative with en + zh-TW entries" {
    _load_module
...
@test "tree module_get_description returns language-specific text + en fallback" {
    _load_module
    [[ -n "$(module_get_description en)" ]]
    [[ "$(module_get_description zh-TW)" == *"樹狀圖"* ]]
    [[ "$(module_get_description xx)" == "$(module_get_description en)" ]]
}

@test "tree SUPPORTED_UBUNTU includes 22.04 24.04 26.04" {
    _load_module
...
@test "tree module RISK_LEVEL=low and VERSION_PROVIDED=apt-managed" {
    _load_module
    [[ "${RISK_LEVEL}" == "low" ]]
    [[ "${VERSION_PROVIDED}" == "apt-managed" ]]
}

@test "tree archetype data installs the tree apt package" {
    _load_module
    [[ " ${APT_PKGS[*]} " == *" tree "* ]]
    [[ "${#APT_PKGS[@]}" -eq 1 ]]
    [[ -z "${APT_PPA}" ]]
...
@test "is_installed returns nonzero when dpkg does not report tree" {
    _load_module
    MOCK_DPKG_RC=1
    _mock_dpkg
    run is_installed
...
@test "is_installed returns zero when dpkg reports tree as ii" {
    _load_module
    MOCK_DPKG_OUTPUT='ii  tree  2.1.1  amd64  pkg'
    _mock_dpkg
    run is_installed
    assert_success
}
...
@test "dry-run install writes nothing under the state dir" {
    ...
    INIT_UBUNTU_DRY_RUN=true run install
    assert_success
    [[ ! -e "${INIT_UBUNTU_STATE_DIR}/versions/tree" ]]
...
@test "install writes the sidecar with the dpkg-reported version" {
    _load_module
    _mock_apt_defaults
    MOCK_PKG_VERSION="2.1.1"
    _mock_dpkg_query
    module_standalone_main install
    [[ -f "${INIT_UBUNTU_STATE_DIR}/versions/tree" ]]
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/tree")" == "2.1.1" ]]
}
...
@test "remove deletes the sidecar" {
    ...
    printf '1.0.0\n' > "${INIT_UBUNTU_STATE_DIR}/versions/tree"
    _mock_apt_defaults
...
@test "doctor passes when the tree probe binary is available" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
    _fake_bin tree 'printf "tree v2.1.1\n"'
    PATH="${INIT_UBUNTU_TEST_SCRATCH}/bin:${PATH}" run doctor
    assert_success
}

@test "doctor fails when the tree probe binary is missing even though installed" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
...
@test "is_outdated returns zero when apt reports tree upgradable" {
    _load_module
    MOCK_APT_UPGRADABLE='tree/noble-updates 2.1.1 amd64 [upgradable from: 0.0.0]'
    _mock_apt_list
    run is_outdated
    assert_success
}

@test "is_outdated returns nonzero when tree is not in the upgradable list" {
    _load_module
...
@test "standalone: info prints metadata" {
    ...
    assert_success
    assert_output --partial "name:        tree"
