#!/usr/bin/env bats
# test/unit/module/unzip_spec.bats — module/unzip.module.sh
#
# Base apt module split out of apt-essentials (ADR-0026). Mirrors the ripgrep
# archetype-A spec: smoke / metadata / lifecycle dry-run / no-side-fx /
# idempotency / standalone CLI / sidecar lifecycle (ADR-0001).
...
_load_module() {
    ...
    # shellcheck source=../../../module/unzip.module.sh
    source "${MODULE_DIR}/unzip.module.sh"
}

_standalone_module() {
    bash "${MODULE_DIR}/unzip.module.sh" "$@"
}
...
@test "unzip module file parses (bash -n)" {
    run bash -n "${MODULE_DIR}/unzip.module.sh"
    assert_success
}

@test "unzip module sources cleanly in engine mode (MODULE_STANDALONE=false)" {
    _load_module
    [[ "${MODULE_STANDALONE}" == "false" ]]
}

@test "unzip module defines all 10 lifecycle functions" {
    _load_module
    local _fn
...
@test "unzip module declares NAME=unzip" {
    _load_module
    [[ "${NAME}" == "unzip" ]]
}

@test "unzip module CATEGORY=base" {
    _load_module
    [[ "${CATEGORY}" == "base" ]]
}

@test "unzip module TAGS contains archive" {
    _load_module
    [[ " ${TAGS[*]} " == *" archive "* ]]
}

@test "unzip module DEPENDS_ON is empty (base tool)" {
    _load_module
    [[ "${#DEPENDS_ON[@]}" -eq 0 ]]
}

@test "unzip DESCRIPTION is associative with en + zh-TW entries" {
    _load_module
...
@test "unzip module_get_description returns language-specific text + en fallback" {
    _load_module
    [[ -n "$(module_get_description en)" ]]
    [[ "$(module_get_description zh-TW)" == *"解壓"* ]]
    [[ "$(module_get_description xx)" == "$(module_get_description en)" ]]
}

@test "unzip SUPPORTED_UBUNTU includes 22.04 24.04 26.04" {
    _load_module
...
@test "unzip SUPPORTED_PLATFORMS covers all five form factors" {
    _load_module
    local _p
...
@test "unzip module RISK_LEVEL=low and VERSION_PROVIDED=apt-managed" {
    _load_module
    [[ "${RISK_LEVEL}" == "low" ]]
    [[ "${VERSION_PROVIDED}" == "apt-managed" ]]
}

@test "unzip archetype data installs the unzip apt package" {
    _load_module
    [[ " ${APT_PKGS[*]} " == *" unzip "* ]]
    [[ "${#APT_PKGS[@]}" -eq 1 ]]
    [[ -z "${APT_PPA}" ]]
...
@test "is_installed returns nonzero when dpkg does not report unzip" {
    _load_module
    MOCK_DPKG_RC=1
    _mock_dpkg
    run is_installed
...
@test "is_installed returns zero when dpkg reports unzip as ii" {
    _load_module
    MOCK_DPKG_OUTPUT='ii  unzip  99.9.9  amd64  pkg'
    _mock_dpkg
    run is_installed
    assert_success
}
...
@test "dry-run install writes nothing under the state dir" {
    ...
    INIT_UBUNTU_DRY_RUN=true run install
    assert_success
    [[ ! -e "${INIT_UBUNTU_STATE_DIR}/versions/unzip" ]]
...
@test "install writes the sidecar with the dpkg-reported version" {
    ...
    _mock_dpkg_query
    install
    [[ -f "${INIT_UBUNTU_STATE_DIR}/versions/unzip" ]]
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/unzip")" == "1.2.3" ]]
}
...
@test "install sidecar falls back to apt-managed when dpkg-query is empty" {
    ...
    _mock_dpkg_query
    install
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/unzip")" == "apt-managed" ]]
}
...
@test "upgrade refreshes the sidecar version" {
    ...
    printf '1.0.0\n' > "${INIT_UBUNTU_STATE_DIR}/versions/unzip"
    _mock_apt_defaults
    MOCK_PKG_VERSION="1.2.3"
    _mock_dpkg_query
    upgrade
...
@test "doctor passes when the unzip binary answers --version" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
    mkdir -p "${INIT_UBUNTU_TEST_SCRATCH}/bin"
    printf '#!/usr/bin/env bash\nprintf "unzip 1.2.3\n"\n' \
        > "${INIT_UBUNTU_TEST_SCRATCH}/bin/unzip"
    chmod +x "${INIT_UBUNTU_TEST_SCRATCH}/bin/unzip"
    PATH="${INIT_UBUNTU_TEST_SCRATCH}/bin:${PATH}" run doctor
    assert_success
}

@test "is_outdated returns zero when apt reports unzip upgradable" {
    _load_module
    MOCK_APT_UPGRADABLE='unzip/noble-updates 1.2.3 amd64 [upgradable from: 1.2.2]'
    _mock_apt_list
    run is_outdated
    assert_success
}

@test "is_outdated returns nonzero when unzip is not in the upgradable list" {
    _load_module
...
@test "standalone: info prints metadata" {
    ...
    assert_success
    assert_output --partial "name:        unzip"
