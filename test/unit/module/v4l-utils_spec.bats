#!/usr/bin/env bats
# test/unit/module/v4l-utils_spec.bats — module/v4l-utils.module.sh
#
...
_load_module() {
    ...
    # shellcheck source=../../../module/v4l-utils.module.sh
    source "${MODULE_DIR}/v4l-utils.module.sh"
}

_standalone_module() {
    bash "${MODULE_DIR}/v4l-utils.module.sh" "$@"
}
...
@test "v4l-utils module file parses (bash -n)" {
    run bash -n "${MODULE_DIR}/v4l-utils.module.sh"
    assert_success
}

@test "v4l-utils module sources cleanly in engine mode (MODULE_STANDALONE=false)" {
    _load_module
    [[ "${MODULE_STANDALONE}" == "false" ]]
}

@test "v4l-utils module defines all 10 lifecycle functions (ADR-0002 / #305)" {
    _load_module
    local _fn
...
@test "v4l-utils module declares NAME=v4l-utils" {
    _load_module
    [[ "${NAME}" == "v4l-utils" ]]
}

@test "v4l-utils module CATEGORY=optional" {
    _load_module
    [[ "${CATEGORY}" == "optional" ]]
}

@test "v4l-utils module TAGS contains media" {
    _load_module
    [[ " ${TAGS[*]} " == *" media "* ]]
}

@test "v4l-utils module DEPENDS_ON is empty (standalone tool)" {
    _load_module
    [[ "${#DEPENDS_ON[@]}" -eq 0 ]]
}

@test "v4l-utils DESCRIPTION is associative with en + zh-TW entries" {
    _load_module
...
@test "v4l-utils module_get_description returns language-specific text + en fallback" {
    _load_module
    [[ -n "$(module_get_description en)" ]]
    [[ "$(module_get_description zh-TW)" == *"工具組"* ]]
    [[ "$(module_get_description xx)" == "$(module_get_description en)" ]]
}

@test "v4l-utils SUPPORTED_UBUNTU includes 22.04 24.04 26.04" {
    _load_module
...
@test "v4l-utils SUPPORTED_PLATFORMS is desktop-only" {
    _load_module
    [[ "${#SUPPORTED_PLATFORMS[@]}" -eq 1 ]]
    [[ "${SUPPORTED_PLATFORMS[0]}" == "desktop" ]]
}

@test "v4l-utils module RISK_LEVEL=low and VERSION_PROVIDED=apt-managed" {
    _load_module
    [[ "${RISK_LEVEL}" == "low" ]]
    [[ "${VERSION_PROVIDED}" == "apt-managed" ]]
}

@test "v4l-utils archetype data installs the v4l-utils apt package" {
    _load_module
    [[ " ${APT_PKGS[*]} " == *" v4l-utils "* ]]
    [[ "${#APT_PKGS[@]}" -eq 1 ]]
    [[ -z "${APT_PPA}" ]]
...
@test "v4l-utils TEST_VERIFY_CMD probes the v4l2-ctl binary" {
    _load_module
    [[ "${TEST_VERIFY_CMD}" == *"v4l2-ctl"* ]]
}

# ── is_installed ─────────────────────────────────────────────────────────────

@test "is_installed returns nonzero when dpkg does not report v4l-utils" {
    _load_module
    MOCK_DPKG_RC=1
    _mock_dpkg
    run is_installed
...
@test "is_installed returns zero when dpkg reports v4l-utils as ii" {
    _load_module
    MOCK_DPKG_OUTPUT='ii  v4l-utils  99.9.9  amd64  pkg'
    _mock_dpkg
    run is_installed
    assert_success
}
...
@test "dry-run install writes nothing under the state dir" {
    ...
    INIT_UBUNTU_DRY_RUN=true run install
    assert_success
    [[ ! -e "${INIT_UBUNTU_STATE_DIR}/versions/v4l-utils" ]]
...
@test "install writes the sidecar with the dpkg-reported version" {
    ...
    [[ -f "${INIT_UBUNTU_STATE_DIR}/versions/v4l-utils" ]]
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/v4l-utils")" == "1.2.3" ]]
}
...
@test "install sidecar falls back to apt-managed when dpkg-query is empty" {
    ...
    _mock_dpkg_query
    module_standalone_main install
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/v4l-utils")" == "apt-managed" ]]
}
...
@test "upgrade refreshes the sidecar version" {
    ...
    printf '1.0.0\n' > "${INIT_UBUNTU_STATE_DIR}/versions/v4l-utils"
    _mock_apt_defaults
    MOCK_PKG_VERSION="1.2.3"
    _mock_dpkg_query
...
@test "doctor fails when installed but the v4l2-ctl binary is not runnable" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
...
@test "doctor passes when installed and the v4l2-ctl binary runs" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
    _stub_bin v4l2-ctl
...
@test "is_outdated returns zero when apt reports v4l-utils upgradable" {
    _load_module
    MOCK_APT_UPGRADABLE='v4l-utils/noble-updates 7.94 amd64 [upgradable from: 7.93]'
    _mock_apt_list
    run is_outdated
    assert_success
}

@test "is_outdated returns nonzero when v4l-utils is not in the upgradable list" {
    _load_module
...
@test "standalone: --version prints NAME + VERSION_PROVIDED" {
    ...
    assert_success
    assert_output --partial "v4l-utils"
}
...
@test "standalone: info prints metadata" {
    ...
    assert_success
    assert_output --partial "name:        v4l-utils"
