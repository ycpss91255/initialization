#!/usr/bin/env bats
# test/unit/module/vim_spec.bats — module/vim.module.sh
#
...
_load_module() {
    ...
    # shellcheck source=../../../module/vim.module.sh
    source "${MODULE_DIR}/vim.module.sh"
}

_standalone_module() {
    bash "${MODULE_DIR}/vim.module.sh" "$@"
}
...
@test "vim module file parses (bash -n)" {
    run bash -n "${MODULE_DIR}/vim.module.sh"
    assert_success
}

@test "vim module sources cleanly in engine mode (MODULE_STANDALONE=false)" {
    _load_module
    [[ "${MODULE_STANDALONE}" == "false" ]]
}

@test "vim module defines all 10 lifecycle functions" {
    _load_module
    local _fn
...
@test "vim module declares NAME=vim" {
    _load_module
    [[ "${NAME}" == "vim" ]]
}

@test "vim module CATEGORY=base" {
    _load_module
    [[ "${CATEGORY}" == "base" ]]
}

@test "vim module TAGS contains editor" {
    _load_module
    [[ " ${TAGS[*]} " == *" editor "* ]]
}

@test "vim module DEPENDS_ON is empty (base tool)" {
    _load_module
    [[ "${#DEPENDS_ON[@]}" -eq 0 ]]
}

@test "vim DESCRIPTION is associative with en + zh-TW entries" {
    _load_module
...
@test "vim module_get_description returns language-specific text + en fallback" {
    _load_module
    [[ -n "$(module_get_description en)" ]]
    [[ "$(module_get_description zh-TW)" == *"編輯器"* ]]
    [[ "$(module_get_description xx)" == "$(module_get_description en)" ]]
}

@test "vim SUPPORTED_UBUNTU includes 22.04 24.04 26.04" {
    _load_module
...
@test "vim SUPPORTED_PLATFORMS covers all five form factors" {
    _load_module
    local _p
...
@test "vim module RISK_LEVEL=low and VERSION_PROVIDED=apt-managed" {
    _load_module
    [[ "${RISK_LEVEL}" == "low" ]]
    [[ "${VERSION_PROVIDED}" == "apt-managed" ]]
}

@test "vim archetype data installs the vim apt package" {
    _load_module
    [[ " ${APT_PKGS[*]} " == *" vim "* ]]
    [[ "${#APT_PKGS[@]}" -eq 1 ]]
    [[ -z "${APT_PPA}" ]]
...
@test "is_installed returns nonzero when dpkg does not report vim" {
    _load_module
    MOCK_DPKG_RC=1
    _mock_dpkg
    run is_installed
...
@test "is_installed returns zero when dpkg reports vim as ii" {
    _load_module
    MOCK_DPKG_OUTPUT='ii  vim  99.9.9  amd64  pkg'
    _mock_dpkg
    run is_installed
    assert_success
}
...
@test "doctor passes when the vim binary answers --version" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
    mkdir -p "${INIT_UBUNTU_TEST_SCRATCH}/bin"
    printf '#!/usr/bin/env bash\nprintf "vim 1.2.3\n"\n' \
        > "${INIT_UBUNTU_TEST_SCRATCH}/bin/vim"
    chmod +x "${INIT_UBUNTU_TEST_SCRATCH}/bin/vim"
    PATH="${INIT_UBUNTU_TEST_SCRATCH}/bin:${PATH}" run doctor
    assert_success
}

@test "is_outdated returns zero when apt reports vim upgradable" {
    _load_module
    MOCK_APT_UPGRADABLE='vim/noble-updates 1.2.3 amd64 [upgradable from: 1.2.2]'
    _mock_apt_list
    run is_outdated
    assert_success
}

@test "is_outdated returns nonzero when vim is not in the upgradable list" {
    _load_module
...
@test "standalone: info prints metadata" {
    ...
    assert_success
    assert_output --partial "name:        vim"
