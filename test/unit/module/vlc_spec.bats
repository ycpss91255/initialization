#!/usr/bin/env bats
# test/unit/module/vlc_spec.bats — module/vlc.module.sh
#
...
_load_module() {
    ...
    # shellcheck source=../../../module/vlc.module.sh
    source "${MODULE_DIR}/vlc.module.sh"
}

_standalone_module() {
    bash "${MODULE_DIR}/vlc.module.sh" "$@"
}
...
@test "vlc module file parses (bash -n)" {
    run bash -n "${MODULE_DIR}/vlc.module.sh"
    assert_success
}

@test "vlc module sources cleanly in engine mode (MODULE_STANDALONE=false)" {
    _load_module
    [[ "${MODULE_STANDALONE}" == "false" ]]
}

@test "vlc module defines all 10 lifecycle functions (ADR-0002 / #305)" {
    _load_module
    local _fn
...
@test "vlc module declares NAME=vlc" {
    _load_module
    [[ "${NAME}" == "vlc" ]]
}

@test "vlc module CATEGORY=optional" {
    _load_module
    [[ "${CATEGORY}" == "optional" ]]
}

@test "vlc module TAGS contains media" {
    _load_module
    [[ " ${TAGS[*]} " == *" media "* ]]
}

@test "vlc module DEPENDS_ON is empty (standalone tool)" {
    _load_module
    [[ "${#DEPENDS_ON[@]}" -eq 0 ]]
}

@test "vlc DESCRIPTION is associative with en + zh-TW entries" {
    _load_module
...
@test "vlc module_get_description returns language-specific text + en fallback" {
    _load_module
    [[ -n "$(module_get_description en)" ]]
    [[ "$(module_get_description zh-TW)" == *"媒體播放器"* ]]
    [[ "$(module_get_description xx)" == "$(module_get_description en)" ]]
}

@test "vlc SUPPORTED_UBUNTU includes 22.04 24.04 26.04" {
    _load_module
...
@test "vlc SUPPORTED_PLATFORMS is desktop-only" {
    _load_module
    [[ "${#SUPPORTED_PLATFORMS[@]}" -eq 1 ]]
    [[ "${SUPPORTED_PLATFORMS[0]}" == "desktop" ]]
}

@test "vlc module RISK_LEVEL=low and VERSION_PROVIDED=apt-managed" {
    _load_module
    [[ "${RISK_LEVEL}" == "low" ]]
    [[ "${VERSION_PROVIDED}" == "apt-managed" ]]
}

@test "vlc archetype data installs the vlc apt package" {
    _load_module
    [[ " ${APT_PKGS[*]} " == *" vlc "* ]]
    [[ "${#APT_PKGS[@]}" -eq 1 ]]
    [[ -z "${APT_PPA}" ]]
...
@test "vlc TEST_VERIFY_CMD probes the vlc binary" {
    _load_module
    [[ "${TEST_VERIFY_CMD}" == *"vlc"* ]]
}
...
@test "is_installed returns nonzero when dpkg does not report vlc" {
    _load_module
    MOCK_DPKG_RC=1
    _mock_dpkg
    run is_installed
...
@test "is_installed returns zero when dpkg reports vlc as ii" {
    _load_module
    MOCK_DPKG_OUTPUT='ii  vlc  99.9.9  amd64  pkg'
    _mock_dpkg
    run is_installed
    assert_success
}
...
@test "dry-run install writes nothing under the state dir" {
    ...
    INIT_UBUNTU_DRY_RUN=true run install
    assert_success
    [[ ! -e "${INIT_UBUNTU_STATE_DIR}/versions/vlc" ]]
...
@test "install writes the sidecar with the dpkg-reported version" {
    ...
    [[ -f "${INIT_UBUNTU_STATE_DIR}/versions/vlc" ]]
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/vlc")" == "1.2.3" ]]
}
...
@test "install sidecar falls back to apt-managed when dpkg-query is empty" {
    ...
    _mock_dpkg_query
    module_standalone_main install
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/vlc")" == "apt-managed" ]]
}
...
@test "upgrade refreshes the sidecar version" {
    ...
    printf '1.0.0\n' > "${INIT_UBUNTU_STATE_DIR}/versions/vlc"
    _mock_apt_defaults
    MOCK_PKG_VERSION="1.2.3"
    _mock_dpkg_query
...
@test "doctor fails when installed but the vlc binary is not runnable" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
...
@test "doctor passes when installed and the vlc binary runs" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
    _stub_bin vlc
...
@test "is_outdated returns zero when apt reports vlc upgradable" {
    _load_module
    MOCK_APT_UPGRADABLE='vlc/noble-updates 7.94 amd64 [upgradable from: 7.93]'
    _mock_apt_list
    run is_outdated
    assert_success
}

@test "is_outdated returns nonzero when vlc is not in the upgradable list" {
    _load_module
...
@test "standalone: info prints metadata" {
    ...
    assert_success
    assert_output --partial "name:        vlc"
