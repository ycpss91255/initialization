#!/usr/bin/env bats
# test/unit/module/vscode_spec.bats — module/vscode.module.sh
#
# Per Q29: smoke / metadata / lifecycle dry-run / no-side-fx / idempotency /
# standalone CLI / module-specific (apt archetype + Microsoft vendor repo,
...
_load_module() {
    ...
    # shellcheck source=../../../module/vscode.module.sh
    source "${MODULE_DIR}/vscode.module.sh"
}

# _standalone_module runs the module as a self-contained CLI (the same
# entry users hit when they type `bash module/vscode.module.sh ...`).
_standalone_module() {
    bash "${MODULE_DIR}/vscode.module.sh" "$@"
}
...
#   MOCK_DPKG_OUTPUT (line printed, e.g. "ii  code ...") / MOCK_DPKG_RC.
_mock_dpkg() {
    dpkg() {
...
# Microsoft repo setup mock: MOCK_REPO_RC (0 = success), and when
# MOCK_REPO_MARKER is set, touch that file so ordering can be asserted.
_mock_repo_setup() {
    _vscode_setup_apt_repo() {
        [[ -n "${MOCK_REPO_MARKER:-}" ]] && : > "${MOCK_REPO_MARKER}"
        return "${MOCK_REPO_RC:-0}"
    }
}

# sudo passthrough mock so repo-file removal can run against scratch paths.
...
@test "vscode module file parses (bash -n)" {
    run bash -n "${MODULE_DIR}/vscode.module.sh"
    assert_success
}

@test "vscode module sources cleanly in engine mode (MODULE_STANDALONE=false)" {
    _load_module
    [[ "${MODULE_STANDALONE}" == "false" ]]
}

@test "vscode module defines all 10 lifecycle functions" {
    _load_module
    local _fn
...
@test "vscode module declares NAME=vscode" {
    _load_module
    [[ "${NAME}" == "vscode" ]]
}

@test "vscode module CATEGORY=optional (demoted from recommended)" {
    _load_module
    [[ "${CATEGORY}" == "optional" ]]
}

@test "vscode module TAGS contains editor" {
    _load_module
    [[ " ${TAGS[*]} " == *" editor "* ]]
}
...
@test "vscode module_get_description returns language-specific text" {
    _load_module
    [[ "$(module_get_description en)" == *"Visual Studio Code"* ]]
    [[ -n "$(module_get_description zh-TW)" ]]
    # Unknown language falls back to en.
    [[ "$(module_get_description xx)" == "$(module_get_description en)" ]]
}

@test "vscode SUPPORTED_UBUNTU includes 22.04 24.04 26.04" {
    _load_module
...
@test "vscode module RISK_LEVEL=low" {
    _load_module
    [[ "${RISK_LEVEL}" == "low" ]]
}

@test "vscode module VERSION_PROVIDED=apt-managed" {
    _load_module
    [[ "${VERSION_PROVIDED}" == "apt-managed" ]]
}

@test "vscode HOMEPAGE points at code.visualstudio.com" {
    _load_module
    [[ "${HOMEPAGE}" == *"code.visualstudio.com"* ]]
}

@test "vscode archetype data installs the code apt package" {
    _load_module
    [[ " ${APT_PKGS[*]} " == *" code "* ]]
}

@test "vscode declares the Microsoft vendor repo paths (deb822 + keyring)" {
    _load_module
...
@test "is_installed returns nonzero when dpkg does not report code" {
    _load_module
    MOCK_DPKG_RC=1
    _mock_dpkg
    run is_installed
...
@test "is_installed returns zero when dpkg reports code as ii" {
    _load_module
    MOCK_DPKG_OUTPUT='ii  code  1.100.0  amd64  Code editing. Redefined.'
    _mock_dpkg
    run is_installed
    assert_success
}
...
@test "dry-run purge leaves scratch repo files in place" {
    _load_module
...
# ── Microsoft vendor repo (module-specific) ──────────────────────────────────

@test "install runs the Microsoft repo setup before installing code" {
    _load_module
    _mock_apt_defaults
    MOCK_REPO_MARKER="${INIT_UBUNTU_TEST_SCRATCH}/repo-was-set-up"
    _mock_repo_setup
...
@test "install fails when the repo setup fails (no sidecar left behind)" {
    _load_module
    _mock_apt_defaults
    MOCK_REPO_RC=1
    _mock_repo_setup
...
@test "install short-circuits before repo setup when already installed" {
    _load_module
...
@test "remove keeps the Microsoft repo files (only purge drops them)" {
    _load_module
    _mock_apt_defaults
    VSCODE_APT_SOURCE="${INIT_UBUNTU_TEST_SCRATCH}/vscode.sources"
    VSCODE_APT_KEYRING="${INIT_UBUNTU_TEST_SCRATCH}/microsoft.gpg"
...
@test "purge deletes the Microsoft repo source + keyring" {
    _load_module
    _mock_apt_defaults
    VSCODE_APT_SOURCE="${INIT_UBUNTU_TEST_SCRATCH}/vscode.sources"
    VSCODE_APT_KEYRING="${INIT_UBUNTU_TEST_SCRATCH}/microsoft.gpg"
...
@test "doctor passes when the code binary answers --version" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
    mkdir -p "${INIT_UBUNTU_TEST_SCRATCH}/bin"
    printf '#!/usr/bin/env bash\nprintf "1.100.0\\n"\n' \
        > "${INIT_UBUNTU_TEST_SCRATCH}/bin/code"
    chmod +x "${INIT_UBUNTU_TEST_SCRATCH}/bin/code"
...
@test "is_outdated returns zero when apt reports code upgradable" {
    _load_module
    MOCK_APT_UPGRADABLE='code/stable 1.101.0-1 amd64 [upgradable from: 1.100.0-1]'
    _mock_apt_list
    run is_outdated
    assert_success
}

@test "is_outdated returns nonzero when code is not in the upgradable list" {
    _load_module
