#!/usr/bin/env bats
# test/unit/module/wget_spec.bats — module/wget.module.sh
#
...
_load_module() {
    ...
    # shellcheck source=../../../module/wget.module.sh
    source "${MODULE_DIR}/wget.module.sh"
}

_standalone_module() {
    bash "${MODULE_DIR}/wget.module.sh" "$@"
}
...
@test "wget module file parses (bash -n)" {
    run bash -n "${MODULE_DIR}/wget.module.sh"
    assert_success
}

@test "wget module sources cleanly in engine mode (MODULE_STANDALONE=false)" {
    _load_module
    [[ "${MODULE_STANDALONE}" == "false" ]]
}

@test "wget module defines all 10 lifecycle functions" {
    _load_module
    local _fn
...
@test "wget module declares NAME=wget" {
    _load_module
    [[ "${NAME}" == "wget" ]]
}

@test "wget module CATEGORY=base" {
    _load_module
    [[ "${CATEGORY}" == "base" ]]
}

@test "wget module TAGS contains http" {
    _load_module
    [[ " ${TAGS[*]} " == *" http "* ]]
}

@test "wget module DEPENDS_ON is empty (base tool)" {
    _load_module
    [[ "${#DEPENDS_ON[@]}" -eq 0 ]]
}

@test "wget DESCRIPTION is associative with en + zh-TW entries" {
    _load_module
...
@test "wget module_get_description returns language-specific text + en fallback" {
    _load_module
    [[ -n "$(module_get_description en)" ]]
    [[ "$(module_get_description zh-TW)" == *"下載"* ]]
    [[ "$(module_get_description xx)" == "$(module_get_description en)" ]]
}

@test "wget SUPPORTED_UBUNTU includes 22.04 24.04 26.04" {
    _load_module
...
@test "wget SUPPORTED_PLATFORMS covers all five form factors" {
    _load_module
    local _p
...
@test "wget module RISK_LEVEL=low and VERSION_PROVIDED=apt-managed" {
    _load_module
    [[ "${RISK_LEVEL}" == "low" ]]
    [[ "${VERSION_PROVIDED}" == "apt-managed" ]]
}

@test "wget archetype data installs the wget apt package" {
    _load_module
    [[ " ${APT_PKGS[*]} " == *" wget "* ]]
    [[ "${#APT_PKGS[@]}" -eq 1 ]]
    [[ -z "${APT_PPA}" ]]
...
@test "is_installed returns nonzero when dpkg does not report wget" {
    _load_module
    MOCK_DPKG_RC=1
    _mock_dpkg
    run is_installed
...
@test "is_installed returns zero when dpkg reports wget as ii" {
    _load_module
    MOCK_DPKG_OUTPUT='ii  wget  99.9.9  amd64  pkg'
    _mock_dpkg
    run is_installed
    assert_success
}
...
@test "dry-run install writes nothing under the state dir" {
    ...
    INIT_UBUNTU_DRY_RUN=true run install
    assert_success
    [[ ! -e "${INIT_UBUNTU_STATE_DIR}/versions/wget" ]]
...
@test "install writes the sidecar with the dpkg-reported version" {
    ...
    _mock_dpkg_query
    install
    [[ -f "${INIT_UBUNTU_STATE_DIR}/versions/wget" ]]
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/wget")" == "1.2.3" ]]
}
...
@test "install sidecar falls back to apt-managed when dpkg-query is empty" {
    ...
    _mock_dpkg_query
    install
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/wget")" == "apt-managed" ]]
}
...
@test "upgrade refreshes the sidecar version" {
    ...
    printf '1.0.0\n' > "${INIT_UBUNTU_STATE_DIR}/versions/wget"
    _mock_apt_defaults
    MOCK_PKG_VERSION="1.2.3"
    _mock_dpkg_query
    upgrade
...
@test "doctor passes when the wget binary answers --version" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
    mkdir -p "${INIT_UBUNTU_TEST_SCRATCH}/bin"
    printf '#!/usr/bin/env bash\nprintf "wget 1.2.3\n"\n' \
        > "${INIT_UBUNTU_TEST_SCRATCH}/bin/wget"
    chmod +x "${INIT_UBUNTU_TEST_SCRATCH}/bin/wget"
    PATH="${INIT_UBUNTU_TEST_SCRATCH}/bin:${PATH}" run doctor
    assert_success
}

@test "is_outdated returns zero when apt reports wget upgradable" {
    _load_module
    MOCK_APT_UPGRADABLE='wget/noble-updates 1.2.3 amd64 [upgradable from: 1.2.2]'
    _mock_apt_list
    run is_outdated
    assert_success
}

@test "is_outdated returns nonzero when wget is not in the upgradable list" {
    _load_module
...
@test "standalone: info prints metadata" {
    ...
    assert_success
    assert_output --partial "name:        wget"
