#!/usr/bin/env bats
# test/unit/module/xclip_spec.bats — module/xclip.module.sh
#
# apt archetype-A module (desktop-only: needs an X display). Mirrors the tree
# archetype-A spec: smoke / metadata / lifecycle dry-run / no-side-fx /
# idempotency / standalone CLI / sidecar lifecycle (ADR-0001) + a REAL doctor()
# presence probe.
...
_load_module() {
    ...
    # shellcheck source=../../../module/xclip.module.sh
    source "${MODULE_DIR}/xclip.module.sh"
}

_standalone_module() {
    bash "${MODULE_DIR}/xclip.module.sh" "$@"
}
...
@test "xclip module file parses (bash -n)" {
    run bash -n "${MODULE_DIR}/xclip.module.sh"
    assert_success
}

@test "xclip module sources cleanly in engine mode (MODULE_STANDALONE=false)" {
    _load_module
    [[ "${MODULE_STANDALONE}" == "false" ]]
}

@test "xclip module defines all 10 lifecycle functions" {
    _load_module
    local _fn
...
@test "xclip module declares NAME=xclip" {
    _load_module
    [[ "${NAME}" == "xclip" ]]
}

@test "xclip module CATEGORY=optional" {
    _load_module
    [[ "${CATEGORY}" == "optional" ]]
}

@test "xclip module TAGS contains cli" {
    _load_module
    [[ " ${TAGS[*]} " == *" cli "* ]]
}

@test "xclip module DEPENDS_ON is empty" {
    _load_module
    [[ "${#DEPENDS_ON[@]}" -eq 0 ]]
}

@test "xclip module SUPPORTED_PLATFORMS is desktop-only" {
    _load_module
    [[ "${#SUPPORTED_PLATFORMS[@]}" -eq 1 ]]
    [[ "${SUPPORTED_PLATFORMS[0]}" == "desktop" ]]
}

@test "xclip DESCRIPTION is associative with en + zh-TW entries" {
    _load_module
...
@test "xclip module_get_description returns language-specific text + en fallback" {
    _load_module
    [[ -n "$(module_get_description en)" ]]
    [[ "$(module_get_description zh-TW)" == *"剪貼簿"* ]]
    [[ "$(module_get_description xx)" == "$(module_get_description en)" ]]
}

@test "xclip SUPPORTED_UBUNTU includes 22.04 24.04 26.04" {
    _load_module
...
@test "xclip module RISK_LEVEL=low and VERSION_PROVIDED=apt-managed" {
    _load_module
    [[ "${RISK_LEVEL}" == "low" ]]
    [[ "${VERSION_PROVIDED}" == "apt-managed" ]]
}

@test "xclip archetype data installs the xclip apt package" {
    _load_module
    [[ " ${APT_PKGS[*]} " == *" xclip "* ]]
    [[ "${#APT_PKGS[@]}" -eq 1 ]]
    [[ -z "${APT_PPA}" ]]
...
@test "is_installed returns zero when dpkg reports the package as ii" {
    _load_module
    MOCK_DPKG_OUTPUT='ii  xclip  0.13  amd64  pkg'
    _mock_dpkg
    run is_installed
    assert_success
}
...
@test "dry-run install writes nothing under the state dir" {
    ...
    INIT_UBUNTU_DRY_RUN=true run install
    assert_success
    [[ ! -e "${INIT_UBUNTU_STATE_DIR}/versions/xclip" ]]
...
@test "install writes the sidecar with the dpkg-reported version" {
    _load_module
    _mock_apt_defaults
    MOCK_PKG_VERSION="0.13"
    _mock_dpkg_query
    module_standalone_main install
    [[ -f "${INIT_UBUNTU_STATE_DIR}/versions/xclip" ]]
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/xclip")" == "0.13" ]]
}
...
@test "remove deletes the sidecar" {
    ...
    printf '0.13\n' > "${INIT_UBUNTU_STATE_DIR}/versions/xclip"
    _mock_apt_defaults
...
@test "doctor passes when the xclip probe binary is on PATH" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
    _fake_bin xclip 'exit 0'
    PATH="${INIT_UBUNTU_TEST_SCRATCH}/bin:${PATH}" run doctor
    assert_success
}

@test "doctor fails when the xclip probe binary is missing even though installed" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
...
@test "is_outdated returns zero when apt reports the package upgradable" {
    _load_module
    MOCK_APT_UPGRADABLE='xclip/noble-updates 0.13 amd64 [upgradable from: 0.12]'
    _mock_apt_list
    run is_outdated
    assert_success
}
...
@test "standalone: info prints metadata" {
    ...
    assert_success
    assert_output --partial "name:        xclip"
