#!/usr/bin/env bats
# test/unit/module/xdg-utils_spec.bats — module/xdg-utils.module.sh
#
# apt archetype-A module. Mirrors the tree/figlet archetype-A spec: smoke /
# metadata / lifecycle dry-run / no-side-fx / idempotency / standalone CLI /
# sidecar lifecycle (ADR-0001) + a REAL doctor() probe (`xdg-open`).
...
_load_module() {
    ...
    # shellcheck source=../../../module/xdg-utils.module.sh
    source "${MODULE_DIR}/xdg-utils.module.sh"
}

_standalone_module() {
    bash "${MODULE_DIR}/xdg-utils.module.sh" "$@"
}
...
@test "xdg-utils module file parses (bash -n)" {
    run bash -n "${MODULE_DIR}/xdg-utils.module.sh"
    assert_success
}

@test "xdg-utils module sources cleanly in engine mode (MODULE_STANDALONE=false)" {
    _load_module
    [[ "${MODULE_STANDALONE}" == "false" ]]
}

@test "xdg-utils module defines all 10 lifecycle functions" {
    _load_module
    local _fn
...
@test "xdg-utils module declares NAME=xdg-utils" {
    _load_module
    [[ "${NAME}" == "xdg-utils" ]]
}

@test "xdg-utils module CATEGORY=optional" {
    _load_module
    [[ "${CATEGORY}" == "optional" ]]
}

@test "xdg-utils module TAGS contains desktop" {
    _load_module
    [[ " ${TAGS[*]} " == *" desktop "* ]]
}

@test "xdg-utils module DEPENDS_ON is empty" {
    _load_module
    [[ "${#DEPENDS_ON[@]}" -eq 0 ]]
}

@test "xdg-utils DESCRIPTION is associative with en + zh-TW entries" {
    _load_module
...
@test "xdg-utils module_get_description returns language-specific text + en fallback" {
    _load_module
    [[ -n "$(module_get_description en)" ]]
    [[ "$(module_get_description zh-TW)" == *"桌面"* ]]
    [[ "$(module_get_description xx)" == "$(module_get_description en)" ]]
}

@test "xdg-utils SUPPORTED_UBUNTU includes 22.04 24.04 26.04" {
    _load_module
...
@test "xdg-utils module RISK_LEVEL=low and VERSION_PROVIDED=apt-managed" {
    _load_module
    [[ "${RISK_LEVEL}" == "low" ]]
    [[ "${VERSION_PROVIDED}" == "apt-managed" ]]
}

@test "xdg-utils archetype data installs the xdg-utils apt package" {
    _load_module
    [[ " ${APT_PKGS[*]} " == *" xdg-utils "* ]]
    [[ "${#APT_PKGS[@]}" -eq 1 ]]
    [[ -z "${APT_PPA}" ]]
...
@test "is_installed returns nonzero when dpkg does not report xdg-utils" {
    _load_module
    MOCK_DPKG_RC=1
    _mock_dpkg
    run is_installed
...
@test "is_installed returns zero when dpkg reports xdg-utils as ii" {
    _load_module
    MOCK_DPKG_OUTPUT='ii  xdg-utils  1.1.3  all  pkg'
    _mock_dpkg
    run is_installed
    assert_success
}
...
@test "dry-run install writes nothing under the state dir" {
    ...
    INIT_UBUNTU_DRY_RUN=true run install
    assert_success
    [[ ! -e "${INIT_UBUNTU_STATE_DIR}/versions/xdg-utils" ]]
...
@test "install writes the sidecar with the dpkg-reported version" {
    _load_module
    _mock_apt_defaults
    MOCK_PKG_VERSION="1.1.3"
    _mock_dpkg_query
    module_standalone_main install
    [[ -f "${INIT_UBUNTU_STATE_DIR}/versions/xdg-utils" ]]
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/xdg-utils")" == "1.1.3" ]]
}
...
@test "remove deletes the sidecar" {
    ...
    printf '1.0.0\n' > "${INIT_UBUNTU_STATE_DIR}/versions/xdg-utils"
    _mock_apt_defaults
...
@test "doctor passes when the xdg-open probe binary is available" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
    _fake_bin xdg-open 'exit 0'
    PATH="${INIT_UBUNTU_TEST_SCRATCH}/bin:${PATH}" run doctor
    assert_success
}

@test "doctor fails when the xdg-open probe binary is missing even though installed" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
...
@test "is_outdated returns zero when apt reports xdg-utils upgradable" {
    _load_module
    MOCK_APT_UPGRADABLE='xdg-utils/noble-updates 1.1.3 all [upgradable from: 1.1.2]'
    _mock_apt_list
    run is_outdated
    assert_success
}

@test "is_outdated returns nonzero when xdg-utils is not in the upgradable list" {
    _load_module
...
@test "standalone: --version prints NAME + VERSION_PROVIDED" {
    ...
    assert_success
    assert_output --partial "xdg-utils"
}
...
@test "standalone: info prints metadata" {
    ...
    assert_success
    assert_output --partial "name:        xdg-utils"
