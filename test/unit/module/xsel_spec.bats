#!/usr/bin/env bats
# test/unit/module/xsel_spec.bats — module/xsel.module.sh
#
...
_load_module() {
    ...
    # shellcheck source=../../../module/xsel.module.sh
    source "${MODULE_DIR}/xsel.module.sh"
}

_standalone_module() {
    bash "${MODULE_DIR}/xsel.module.sh" "$@"
}
...
@test "xsel module file parses (bash -n)" {
    run bash -n "${MODULE_DIR}/xsel.module.sh"
    assert_success
}

@test "xsel module sources cleanly in engine mode (MODULE_STANDALONE=false)" {
    _load_module
    [[ "${MODULE_STANDALONE}" == "false" ]]
}

@test "xsel module defines all 10 lifecycle functions" {
    _load_module
    local _fn
...
@test "xsel module declares NAME=xsel" {
    _load_module
    [[ "${NAME}" == "xsel" ]]
}

@test "xsel module CATEGORY=optional" {
    _load_module
    [[ "${CATEGORY}" == "optional" ]]
}

@test "xsel module TAGS contains cli" {
    _load_module
    [[ " ${TAGS[*]} " == *" cli "* ]]
}

@test "xsel module DEPENDS_ON is empty" {
    _load_module
    [[ "${#DEPENDS_ON[@]}" -eq 0 ]]
}

@test "xsel module SUPPORTED_PLATFORMS is desktop-only" {
    _load_module
    [[ "${#SUPPORTED_PLATFORMS[@]}" -eq 1 ]]
    [[ "${SUPPORTED_PLATFORMS[0]}" == "desktop" ]]
}

@test "xsel DESCRIPTION is associative with en + zh-TW entries" {
    _load_module
...
@test "xsel module_get_description returns language-specific text + en fallback" {
    _load_module
    [[ -n "$(module_get_description en)" ]]
    [[ "$(module_get_description zh-TW)" == *"選取"* ]]
    [[ "$(module_get_description xx)" == "$(module_get_description en)" ]]
}

@test "xsel SUPPORTED_UBUNTU includes 22.04 24.04 26.04" {
    _load_module
...
@test "xsel module RISK_LEVEL=low and VERSION_PROVIDED=apt-managed" {
    _load_module
    [[ "${RISK_LEVEL}" == "low" ]]
    [[ "${VERSION_PROVIDED}" == "apt-managed" ]]
}

@test "xsel archetype data installs the xsel apt package" {
    _load_module
    [[ " ${APT_PKGS[*]} " == *" xsel "* ]]
    [[ "${#APT_PKGS[@]}" -eq 1 ]]
    [[ -z "${APT_PPA}" ]]
...
@test "is_installed returns zero when dpkg reports the package as ii" {
    _load_module
    MOCK_DPKG_OUTPUT='ii  xsel  1.2.0  amd64  pkg'
    _mock_dpkg
    run is_installed
    assert_success
}
...
@test "dry-run install writes nothing under the state dir" {
    ...
    INIT_UBUNTU_DRY_RUN=true run install
    assert_success
    [[ ! -e "${INIT_UBUNTU_STATE_DIR}/versions/xsel" ]]
...
@test "install writes the sidecar with the dpkg-reported version" {
    _load_module
    _mock_apt_defaults
    MOCK_PKG_VERSION="1.2.0"
    _mock_dpkg_query
    module_standalone_main install
    [[ -f "${INIT_UBUNTU_STATE_DIR}/versions/xsel" ]]
    [[ "$(cat "${INIT_UBUNTU_STATE_DIR}/versions/xsel")" == "1.2.0" ]]
}
...
@test "remove deletes the sidecar" {
    ...
    printf '1.2.0\n' > "${INIT_UBUNTU_STATE_DIR}/versions/xsel"
    _mock_apt_defaults
...
@test "doctor passes when the xsel probe binary is on PATH" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
    _fake_bin xsel 'exit 0'
    PATH="${INIT_UBUNTU_TEST_SCRATCH}/bin:${PATH}" run doctor
    assert_success
}

@test "doctor fails when the xsel probe binary is missing even though installed" {
    _load_module
    MOCK_IS_INSTALLED_RC=0
    _mock_is_installed
...
@test "is_outdated returns zero when apt reports the package upgradable" {
    _load_module
    MOCK_APT_UPGRADABLE='xsel/noble-updates 1.2.0 amd64 [upgradable from: 1.1.0]'
    _mock_apt_list
    run is_outdated
    assert_success
}
...
@test "standalone: info prints metadata" {
    ...
    assert_success
    assert_output --partial "name:        xsel"
